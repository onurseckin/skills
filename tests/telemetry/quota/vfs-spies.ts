...
import type { QuotaVirtualFs, VirtualNode } from "./vfs-harness.ts";

export const strData = (d: string | NodeJS.ArrayBufferView): string =>
  typeof d === "string" ? d : new TextDecoder().decode(d as Uint8Array);

export function getDirents(
  vfs: Map<string, VirtualNode>,
  pref: string,
  isBuf = false,
): fs.Dirent[] {
...
export async function fspOpenMock(self: QuotaVirtualFs, p: fs.PathLike): Promise<fsp.FileHandle> {
  const s = String(p);
  const n = self.vfs.get(s);
...
export function mockSyncHandlers(self: QuotaVirtualFs): void {
  ...
  m(cp, "spawnSync", (_cmd: unknown, args: unknown, opts: unknown) => {
    ...
    const isString =
      ...
      opts !== null &&
      "encoding" in opts &&
      typeof (opts as { encoding?: unknown }).encoding === "string" &&
      (opts as { encoding?: string }).encoding !== "buffer";
    return {
      status,
  ...
  m(fs, "readSync", (fd: number, buf: Buffer, off: number, len: number, pos: number | null) => {
    ...
    const n = p ? self.vfs.get(p) : undefined;
    if (!n || n.content === undefined) return 0;
    const data = Buffer.from(n.content, "utf8");
    const at =
      pos === null || pos === undefined ? (self.fdOffsets.get(Number(fd)) ?? 0) : Number(pos);
