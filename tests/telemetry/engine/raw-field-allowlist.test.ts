...
describe("collector-boundary field allowlist", () => {
  it("drops a distinctive secret hidden under innocuously-named keys at every nesting depth", async () => {
  ...
  it("the same payload leaks the secret through the plain denylist redactor", () => {
    const payload = buildLeakyPayload();
