...
import { categorizeDefect } from "../../../olt/scripts/src/mind/defects/index.ts";
...
describe("Diagnostics Defect Categorization Engine", () => {
  describe("Explicit Category Preservation & Aliasing", () => {
    ...
    test("maps role_confusion alias strictly to boundary_violation", () => {
      expect(categorizeDefect({ category: "role_confusion" })).toBe("boundary_violation");
      expect(categorizeDefect({ category: "ROLE_CONFUSION" })).toBe("boundary_violation");
      expect(categorizeDefect({ type: "role_confusion_detected" })).toBe("boundary_violation");
    });
  });
  ...
  describe("Boundary Violation Categorization", () => {
    test("identifies boundary violations from type keywords", () => {
      ...
      for (const type of boundaryTypes) {
        expect(categorizeDefect({ type })).toBe("boundary_violation");
      }
    });

    test("identifies boundary violations from ID keywords", () => {
      expect(categorizeDefect({ id: "defect-20260821-08-orch-role-leak" })).toBe(
        "boundary_violation",
      );
      expect(categorizeDefect({ id: "defect-20260821-10-identity-and-role-amnesia" })).toBe(
        "boundary_violation",
      );
      expect(categorizeDefect({ id: "defect-main-thread-spillover" })).toBe("boundary_violation");
    });
    ...
    test("identifies boundary violations from observation and remediation phrases", () => {
      ...
      for (const phrase of phrases) {
        expect(categorizeDefect(phrase)).toBe("boundary_violation");
      }
    });
  });
  ...
  describe("Model Reasoning Error Categorization", () => {
    test("identifies model reasoning errors from type keywords", () => {
      ...
      for (const type of reasoningTypes) {
        expect(categorizeDefect({ type })).toBe("model_reasoning_error");
      }
    });

    test("identifies model reasoning errors from ID keywords", () => {
      expect(categorizeDefect({ id: "defect-20260821-09-mind-plan-revision-paralysis" })).toBe(
        "model_reasoning_error",
      );
      expect(categorizeDefect({ id: "defect-hallucination-api-key" })).toBe(
        "model_reasoning_error",
      );
      expect(categorizeDefect({ id: "defect-intent-drift-task-3" })).toBe("model_reasoning_error");
      expect(categorizeDefect({ id: "defect-idle-death-loop" })).toBe("model_reasoning_error");
    });
    ...
    test("identifies model reasoning errors from observation and remediation text", () => {
      ...
      for (const c of cases) {
        expect(categorizeDefect(c)).toBe("model_reasoning_error");
      }
    });
  });
  ...
  describe("Code Defect Categorization", () => {
    test("defaults all technical and syntax defects to code_defect", () => {
      ...
      for (const type of codeTypes) {
        expect(categorizeDefect({ type })).toBe("code_defect");
      }
    });

    test("defaults empty or unrecognized records to code_defect", () => {
      expect(categorizeDefect({})).toBe("code_defect");
      expect(categorizeDefect({ type: "generic_issue" })).toBe("code_defect");
      expect(categorizeDefect({ observation: "File not found at /tmp/foo.txt" })).toBe(
        "code_defect",
      );
    });
  });
  ...
  describe("Precedence & Disambiguation", () => {
    test("explicit valid category overrides contradictory text", () => {
      ...
      expect(categorizeDefect(entry)).toBe("code_defect");
    ...
    test("boundary violation takes precedence over reasoning error in ambiguous multi-signal defects", () => {
      ...
      expect(categorizeDefect(ambiguousEntry)).toBe("boundary_violation");
    });
  });
});
