/**
 * @file behavioral-forensics.test.ts
 * Unit tests for Behavioral Forensics Heuristics, Token Burn, and Auto-Injection.
 */
...
describe("Behavioral Forensics: 7 Core Heuristics Suites", () => {
  ...
  it("detects FALSE_SERIALIZATION when disjoint write scopes are executed in sequence", () => {
    ...
    expect(result.isClean).toBe(false);
    expect(result.metrics.sequentialWaveBottlenecks).toBeGreaterThanOrEqual(2);
  ...
  it("detects ROLE_BOUNDARY_DEVIATION when supervisor executes direct code modification", () => {
    ...
    const result = analyzeBehavioralForensics({
      runId: "run-coord-breach",
      allToolCalls: toolCalls,
    });

    expect(result.isClean).toBe(false);
    expect(result.metrics.boundaryDeviationsCount).toBe(1);
    const inc = result.incidents.find((i) => i.category === "ROLE_BOUNDARY_DEVIATION");
    expect(inc?.severity).toBe("CRITICAL");
  });

  it("detects ROLE_BOUNDARY_DEVIATION when validator attempts non-permitted write or execution", () => {
    const toolCalls: ExtractedToolCall[] = [
      {
        agentId: "cognitive-validator-1",
        agentRole: "validator",
        name: "write_to_file",
        isRead: false,
        isWrite: true,
  ...
  it("detects POLLING_WASTE when agent polls status repeatedly in loops", () => {
    const toolCalls: ExtractedToolCall[] = Array.from({ length: 7 }, () => ({
      agentId: "worker-1",
      name: "manage_task",
      isRead: false,
      isWrite: false,
