...
// event log with no time or actor bound, so a "supervisor-dead-agent-reclaimed" event a different
// orchestrator actor wrote hours ago on this same run keeps counting on every later, unrelated
// report. These fixtures reproduce that with genuinely stale leases (claimTask + an expiry moved
// into the past), not synthetic event rows, so a fix aimed at the wrong mechanism cannot pass.
...
describe("RunSupervisor session reporting — scoped by actor and time window", () => {
  test("a later, idle pass by the same actor does not inherit a reclaim from hours earlier", async () => {
    ...
    // Sanity: the reclaim genuinely happened and was genuinely counted once.
