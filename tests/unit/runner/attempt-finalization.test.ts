...
import { readBoundedBytes, sha256Bytes } from "../../../orchestrating-long-tasks/scripts/src/core/json.ts";
import { recoverAggregateFromAttempts } from "../../../orchestrating-long-tasks/scripts/src/integration/reconcile-command-attempts.ts";
...
import type { AttemptResult, BunSpawnApi, NormalizedCommandOptions } from "../../../orchestrating-long-tasks/scripts/src/runner/types.ts";
...
function binding(marker: string): RepositoryBinding {
  return {
    schema: "harness.repository-binding", version: 1, inspection_sha256: digest(marker),
    git_identity_sha256: digest(marker), content_sha256: digest(marker), file_count: 1, total_bytes: 17,
  };
}

function metadata(root: string, path: string) {
  const bytes = readBoundedBytes(path, 1024 * 1024);
  return { path: relative(root, path).split(sep).join("/"), bytes: bytes.byteLength, sha256: sha256Bytes(bytes) };
}

async function successfulAttempt(runRoot: string, commandRoot: string, id: string, attempt: number): Promise<AttemptResult> {
  const attemptRoot = join(commandRoot, `attempt-${attempt}`);
  await mkdir(attemptRoot);
  const stdoutPath = join(attemptRoot, "stdout.log"), stderrPath = join(attemptRoot, "stderr.log"), activityPath = join(attemptRoot, "activity.json");
  const startedAt = "2026-08-14T00:00:00.000Z", finishedAt = "2026-08-14T00:00:01.000Z";
  await writeFile(stdoutPath, "raw success\n");
  await writeFile(stderrPath, "");
  atomicWriteJson(activityPath, {
    schema: "harness.command-activity", version: 1, command_id: id, attempt, status: "completed",
    started_at: startedAt, heartbeat_at: finishedAt, last_output_at: startedAt, stdout_bytes: 12, stderr_bytes: 0, finished_at: finishedAt,
  }, 0o600);
  const record: CommandAttemptRecord = {
    id, attempt, status: "succeeded", started_at: startedAt, finished_at: finishedAt, exit_code: 0, signal: null, signals_sent: [], timeout_kind: null, failure_class: null,
...
describe("two-phase command attempt finalization", () => {
  test("persists a started marker before invoking the spawn dependency", async () => {
    ...
    const options: NormalizedCommandOptions = {
      argv: ["injected"], cwd: root, repositoryRoot: root, commandDir: join(root, "commands"), runRoot: root, actor: "validator",
      wallTimeoutMs: 1000, idleTimeoutMs: 1000, graceMs: 10, drainTimeoutMs: 10, heartbeatIntervalMs: 1000, maxOutputBytes: 1024, retries: 0, idempotent: false,
    ...
    runtime.spawn = (() => {
      marker = JSON.parse(readFileSync(join(commandRoot, "attempt-1", "attempt-started.json"), "utf8"));
      throw new Error("injected spawn stop");
    }) as BunSpawnApi["spawn"];
    try {
      await expect(runAttempt(options, 1, "C-started", commandRoot, createCommandSigningCapability())).rejects.toThrow("injected spawn stop");
    } finally {
      runtime.spawn = original;
    }
    expect(marker).toMatchObject({
      schema: "harness.command-attempt-started", version: 1, command_id: "C-started", attempt: 1, status: "running", root_pid_identity: null,
    });
  });
  ...
  test("leaves an unreturned started attempt running for conservative reconciliation", async () => {
    ...
    const runner = createInternalCommandRunner({
      ...
      attempt: async (options, attempt, id, commandRoot, signer) => {
        ...
        writeAttemptStarted(attemptRoot, id, attempt, "2026-08-14T00:00:00.000Z", options.environment[OWNERSHIP_ENV]!, signer);
        throw new Error("injected attempt interruption");
      },
    });
    const prepared = await runner.prepareCommand({ argv: ["tool"], cwd: root, runRoot, commandDir: join(runRoot, "commands"), actor: "validator" });
    await expect(runner.executePreparedCommand(prepared)).rejects.toThrow("injected attempt interruption");
    ...
    expect(
      recoverAggregateFromAttempts(runRoot, prepared.record, {
        probeProcess: () => { throw new Error("missing identity must not be probed"); },
      }),
    ).toBeUndefined();
  });
  ...
  test("retains raw attempt evidence when the repository changes after the child exits", async () => {
    ...
    let observations = 0, recordPath = "", rawAggregate: Record<string, unknown> | undefined;
    const runner = createInternalCommandRunner({
      inspectRepository: () => {
        observations += 1;
      ...
      attempt: async (_options, attempt, id, commandRoot) => successfulAttempt(runRoot, commandRoot, id, attempt),
    });
    const prepared = await runner.prepareCommand({
      argv: ["./bin/verify"], cwd: root, runRoot, commandDir: join(runRoot, "commands"), actor: "validator", gateId: "G-finalize", idempotent: true,
    });
    recordPath = prepared.recordPath;

    await expect(runner.executePreparedCommand(prepared)).rejects.toThrow(/repository.*changed|post-attempt.*integrity/i);
    expect(rawAggregate).toMatchObject({ status: "running", repository_after: null, attempts: [{ exit_code: 0 }] });
    const aggregate = JSON.parse(await readFile(prepared.recordPath, "utf8"));
    const terminalAttempt = JSON.parse(await readFile(join(prepared.commandRoot, "attempt-1", "record.json"), "utf8"));
  ...
  test("captures repository_after for a successful gate terminalization", async () => {
    ...
    const prepared = await runner.prepareCommand({
      argv: ["./bin/verify"], cwd: root, runRoot, commandDir: join(runRoot, "commands"), actor: "validator", gateId: "G-success-after",
    });
