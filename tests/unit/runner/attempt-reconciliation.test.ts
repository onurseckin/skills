...
import type { CommandAttemptStartedRecord } from "../../../orchestrating-long-tasks/scripts/src/contracts/commands.ts";
...
describe("incomplete command attempt reconciliation", () => {
  test("leaves a raw initial marker stranded even when its root later appears absent", async () => {
    ...
    expect(
      recoverAggregateFromAttempts(runRoot, prepared.record, { probeProcess: () => "absent" }),
    ).toBeUndefined();
  });
  ...
  test("leaves a base-only marker from an initial disposition failure stranded", async () => {
    ...
    expect(
      recoverAggregateFromAttempts(runRoot, prepared.record, {
        probeProcess: () => {
          throw new Error("base-only marker must preclude a root process probe");
        },
      }),
    ).toBeUndefined();
  });
  ...
  test("rejects a stripped terminal-proof disposition before reconciliation", async () => {
    ...
    expect(() =>
      recoverAggregateFromAttempts(runRoot, prepared.record, { probeProcess: () => "absent" }),
    ).toThrow(/disposition|hash|history/i);
  });
  ...
  test("strands a marker truncated to an earlier valid signed uncertainty", async () => {
    ...
    const uncertainIndex = marker.cleanup_history.findLastIndex(
      (entry) => entry.status === "uncertain",
    );
    ...
    expect(
      attemptStartedIssues(
        ...
        1,
        prepared.record.environment![OWNERSHIP_ENV],
        prepared.record.attempt_signing_public_key,
      ),
    ).toEqual([]);
    expect(
      recoverAggregateFromAttempts(runRoot, prepared.record, {
        probeProcess: () => {
          throw new Error("truncated uncertainty must preclude process probing");
        },
      }),
    ).toBeUndefined();
  });
