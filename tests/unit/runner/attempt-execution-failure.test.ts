...
import { AttemptExecutionError } from "../../../orchestrating-long-tasks/scripts/src/runner/attempt-execution-error.ts";
import {
  settledAttemptTerminalProof,
  startAttemptIntent,
  strongAttemptTerminalProof,
} from "../../../orchestrating-long-tasks/scripts/src/runner/attempt-intent.ts";
import { settleBounded } from "../../../orchestrating-long-tasks/scripts/src/runner/attempt-support.ts";
...
import type { AttemptResult, FailureClass, NormalizedCommandOptions } from "../../../orchestrating-long-tasks/scripts/src/runner/types.ts";
import { verifyCommandRecord } from "../../../orchestrating-long-tasks/scripts/src/runner/verify-command.ts";
import type { CommandSigningCapability } from "../../../orchestrating-long-tasks/scripts/src/runner/attempt-disposition-capability.ts";
...
async function attemptResult(
  ...
  const startedAt = `2026-08-14T00:00:0${attempt}.000Z`, finishedAt = `2026-08-14T00:00:1${attempt}.000Z`;
  const output = `attempt ${attempt}\n`;
  await writeFile(stdoutPath, output);
  await writeFile(stderrPath, "");
  const controller = startAttemptIntent(attemptRoot, id, attempt, startedAt, options.environment[OWNERSHIP_ENV]!, () => undefined, signer);
  ...
  controller.markTerminalProof("injected terminal process proof", failureClass === "evidence_failure" ? strongAttemptTerminalProof(rootIdentity) : settledAttemptTerminalProof(undefined));
  ...
  const record: CommandAttemptRecord = {
    id, attempt, status: "failed", started_at: startedAt, finished_at: finishedAt, exit_code: 1, signal: null, signals_sent: [], timeout_kind: null, failure_class: failureClass,
    activity_path: portable(options.runRoot, activityPath),
    activity: metadata(options.runRoot, activityPath),
    logs: { stdout: metadata(options.runRoot, stdoutPath), stderr: metadata(options.runRoot, stderrPath) },
    ...(integrityFailure === undefined ? {} : { integrity_failure: integrityFailure }),
  };
  return { record, attempt, failureClass, stdoutPath, stderrPath, activityPath, outputTail: output };
}
...
describe("terminal attempt execution failures", () => {
  ...
  test("publishes first-attempt evidence failure before rethrowing its original error", async () => {
    ...
    const runner = createInternalCommandRunner({
      inspectRepository: () => { throw new Error("non-gate observer must not run"); },
      attempt: async (options, attempt, id, commandRoot, signer) => {
        calls += 1;
        const result = await attemptResult(options, commandRoot, id, attempt, "evidence_failure", signer, original.message);
        throw new AttemptExecutionError(original, result);
      },
    });
    const prepared = await runner.prepareCommand({ argv: ["tool"], cwd: repositoryRoot, runRoot, commandDir: join(runRoot, "commands"), actor: "validator", retries: 2, idempotent: true });
    let caught: unknown;
    try { await runner.executePreparedCommand(prepared); } catch (error) { caught = error; }
    expect(caught).toBe(original);
    expect(calls).toBe(1);
    const stored = JSON.parse(await readFile(prepared.recordPath, "utf8"));
    expect(stored).toMatchObject({
      status: "failed", evidence_error: original.message, retry_exhausted: false, attempts: [{ attempt: 1, failure_class: "evidence_failure" }],
    });
    ...
    expect(embeddedCommandIssues(mismatched)).toContain("aggregate evidence error does not match its final attempt integrity failure");
  });
  ...
  test("preserves a later evidence failure instead of recovering it as interrupted", async () => {
    ...
    const runner = createInternalCommandRunner({
      inspectRepository: () => { throw new Error("non-gate observer must not run"); },
      attempt: async (options, attempt, id, commandRoot, signer) => {
        if (attempt === 1) return attemptResult(options, commandRoot, id, attempt, "network_transient", signer);
        const result = await attemptResult(options, commandRoot, id, attempt, "evidence_failure", signer, original.message);
        throw new AttemptExecutionError(original, result);
      },
    });
    ...
    const stored = JSON.parse(await readFile(prepared.recordPath, "utf8"));
    expect(stored).toMatchObject({
      status: "failed", evidence_error: original.message, retry_exhausted: false,
      attempts: [
    ...
    const recovered = recoverAggregateFromAttempts(runRoot, prepared.record, {
      probeProcess: () => { throw new Error("terminal attempts must not probe process identity"); },
    });
    expect(recovered).toMatchObject({
      status: "failed", evidence_error: original.message,
      attempts: [
        { attempt: 1, failure_class: "network_transient" },
