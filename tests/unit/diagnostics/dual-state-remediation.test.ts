...
describe("Diagnostics Dual-State Remediation Engine", () => {
  describe("Scenario 1: Main Thread Boundary Violation (Direct Execution)", () => {
    const unhandledBlunder: BlunderEntry = {
      id: "blunder-dual-main-thread-01",
    ...
    test("State 1 (Defect State): Flags open boundary violation, blocks closure without proof", () => {
      ...
      expect(unhandledBlunder.status).toBe("open");
      expect(unhandledBlunder.category).toBe("boundary_violation");
      expect(unhandledBlunder.resolution).toBeUndefined();

      // 2. Candidate Proposal generation: Unresolved blunder generates actionable candidate proposal
      const candidates = formulateBlunderCandidates([unhandledBlunder], ["G1", "G2"]);
      expect(candidates.length).toBe(1);
      const cand = candidates[0];
      expect(cand !== undefined).toBeTrue();
      if (cand !== undefined) {
        expect(cand.id).toBe("cand-blunder-dual-main-thread-01");
      ...
      const invalidProof: BlunderResolutionProof = {
        task_id: "",
        test_assertion: "bun test",
        resolved_at: "2026-08-22T08:15:00.000Z",
      };
      expect(() => resolveBlunder(unhandledBlunder, invalidProof)).toThrow(HarnessError);
    });

    test("State 2 (Remediated State): Resolves blunder with Tier 3 delegation proof and verified commit", () => {
      ...
      const resolved = resolveBlunder(unhandledBlunder, validProof);
      ...
      // 2. Candidate proposals should now be empty for resolved blunder
      const candidates = formulateBlunderCandidates([resolved], ["G1", "G2"]);
      expect(candidates.length).toBe(0);

      // 3. Immutability: Original blunder remains unchanged
      expect(unhandledBlunder.status).toBe("open");
    });
  });

  describe("Scenario 2: Model Reasoning Error (Plan Revision Paralysis)", () => {
    const unhandledReasoningError: BlunderEntry = {
      id: "blunder-dual-paralysis-02",
    ...
    test("State 1 (Defect State): Identifies model reasoning error and generates G1 candidate", () => {
      expect(categorizeBlunder(unhandledReasoningError)).toBe("model_reasoning_error");
      expect(unhandledReasoningError.status).toBe("open");

      const candidates = formulateBlunderCandidates([unhandledReasoningError], ["G1", "G2"]);
      expect(candidates.length).toBe(1);
      const cand = candidates[0];
      expect(cand !== undefined).toBeTrue();
      if (cand !== undefined) {
        expect(cand.id).toBe("cand-blunder-dual-paralysis-02");
        expect(cand.kind).toBe("proposal");
    ...
    test("State 2 (Remediated State): Proves autonomic rollover and attaches resolution proof", () => {
      ...
      const resolved = resolveBlunder(unhandledReasoningError, proof);
      expect(resolved.status).toBe("resolved");
  ...
  describe("Scenario 3: Code Defect (Type Error / Implicit Any)", () => {
    const unhandledCodeDefect: BlunderEntry = {
      id: "blunder-dual-type-defect-03",
      type: "type_error",
      severity: "critical",
    ...
    test("State 1 (Defect State): Flags defect kind candidate with G1 goal", () => {
      expect(categorizeBlunder(unhandledCodeDefect)).toBe("code_defect");
      expect(unhandledCodeDefect.status).toBe("open");

      const candidates = formulateBlunderCandidates([unhandledCodeDefect], ["G1", "G2"]);
      expect(candidates.length).toBe(1);
    ...
    test("State 2 (Remediated State): Attaches tsc zero-emit proof and closes defect", () => {
      ...
      const resolved = resolveBlunder(unhandledCodeDefect, proof);
      expect(resolved.status).toBe("resolved");
  ...
  describe("Scenario 4: Multi-Capsule Audit Transition from Defect to Remediated State", () => {
    test("tracks audit aggregation transition across capsule lifecycle", () => {
      ...
      const openEntry: BlunderEntry = {
        id: "blunder-audit-trans-1",
        type: "role_amnesia",
        severity: "high",
      ...
      // State 1: Write initial open blunder log
      writeFileSync(join(capsuleDir, "blunders.jsonl"), `${JSON.stringify(openEntry)}\n`);

      const auditState1 = auditBlunderLog([capsuleDir]);
      expect(auditState1.total_blunders).toBe(1);
      ...
      const resolvedEntry = resolveBlunder(openEntry, {
      ...
      const auditState2 = auditBlunderLog([capsuleDir]);
      expect(auditState2.total_blunders).toBe(1); // Deduplicated by ID
      expect(auditState2.open_count).toBe(0);
      expect(auditState2.resolved_count).toBe(1);
      expect(auditState2.blunders[0]?.status).toBe("resolved");
      expect(auditState2.blunders[0]?.resolution?.task_id).toBe("task-whoami-enforcement");
    });
  });
});
