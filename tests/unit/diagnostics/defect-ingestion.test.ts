import { resolveDefectsPath, resolveBacklogPath } from "../../../olt/scripts/src/core/shared/paths.ts";
...
describe("Diagnostics Defect Ingestion Engine", () => {
  ...
  describe("Historical .capsules/defects.jsonl Ingestion", () => {
    test("successfully parses all historical defect records without crashing", () => {
      if (!existsSync(historicalDefectsPath)) {
        return;
      }

      const content = readFileSync(historicalDefectsPath, "utf8");
    ...
    test("correctly parses historical record: defect-20260821-08-orch-role-leak", () => {
      if (!existsSync(historicalDefectsPath)) {
        return;
      }
      ...
      const content = readFileSync(historicalDefectsPath, "utf8");
      const parsed = parseDefectLog(content);
      const entry = parsed.find((b) => b.id === "defect-20260821-08-orch-role-leak");
      if (!entry) return;

      expect(entry.id).toBe("defect-20260821-08-orch-role-leak");
    ...
    test("correctly parses historical record: defect-20260821-09-mind-plan-revision-paralysis", () => {
      if (!existsSync(historicalDefectsPath)) {
        return;
      }
      ...
      const content = readFileSync(historicalDefectsPath, "utf8");
      const parsed = parseDefectLog(content);
      const entry = parsed.find((b) => b.id === "defect-20260821-09-mind-plan-revision-paralysis");
      if (!entry) return;

      expect(entry.id).toBe("defect-20260821-09-mind-plan-revision-paralysis");
    ...
    test("correctly parses historical record: defect-20260821-10-identity-and-role-amnesia with role_confusion mapping", () => {
      if (!existsSync(historicalDefectsPath)) {
        return;
      }
      ...
      const content = readFileSync(historicalDefectsPath, "utf8");
      const parsed = parseDefectLog(content);
      const entry = parsed.find((b) => b.id === "defect-20260821-10-identity-and-role-amnesia");
      if (!entry) return;

      expect(entry.id).toBe("defect-20260821-10-identity-and-role-amnesia");
      expect(entry.category).toBe("boundary_violation");
      expect(entry.severity).toBe("high");
    ...
    test("correctly parses historical record: defect-20260821-12-mind-self-termination-and-idle-death", () => {
      if (!existsSync(historicalDefectsPath)) {
        return;
      }
      ...
      const entry = parsed.find(
        (b) => b.id === "defect-20260821-12-mind-self-termination-and-idle-death",
      );
      if (!entry) return;

      expect(entry.id).toBe("defect-20260821-12-mind-self-termination-and-idle-death");
      expect(entry.category).toBe("boundary_violation");
  ...
  describe("Synthetic Schema Variations & Edge Cases", () => {
    ...
    test("skips malformed and corrupted JSON lines", () => {
      ...
      const results = parseDefectLog(input);
    ...
    test("supports observation falling back to message and vice versa", () => {
      ...
      const parsedMsg = parseDefectLog(lineMsgOnly);
      ...
      const parsedObs = parseDefectLog(lineObsOnly);
    ...
    test("normalizes severity to lowercase canonical string", () => {
      ...
      const parsed = parseDefectLog(lines);
    ...
    test("preserves context metadata and extracts role and process hierarchy", () => {
      ...
      const parsed = parseDefectLog(JSON.stringify(entryObj));
    ...
    test("round-trips entries via serializeDefectLog without data loss", () => {
