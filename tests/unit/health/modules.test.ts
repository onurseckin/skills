...
describe("the module graph reads what the file actually declares", () => {
  ...
  test("every export form is recorded with its kind", () => {
    expect(leaf?.exports.map((entry) => `${entry.kind}:${entry.name}`).sort()).toEqual([
      "class:Holder",
      "enum:Level",
      "function:used",
      "interface:Shape",
...
describe("a default export is a symbol the graph must carry", () => {
  const tree = loadTree("default-export", {
  ...
  test("the declaring module records it under the name an importer binds", () => {
    const record = tree.modules.get(join(tree.root, "config.ts"));
    expect(record?.exports.map((entry) => [entry.name, entry.kind])).toEqual([
      ["default", "default"],
    ]);
  });
  ...
  test("the importing module binds it as `default`, so the export has a consumer", () => {
    const entry = tree.modules.get(join(tree.root, "entry.ts"));
    expect(entry?.imports.map((binding) => binding.imported)).toContain("default");
  });
});
