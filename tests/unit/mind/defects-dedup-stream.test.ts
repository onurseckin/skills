...
import { toAggregatedDefect } from "../../../olt/scripts/src/mind/defects/aggregator.ts";
import type {
  AggregatedDefect,
  DefectRecordInput,
...
describe("mind/defects/dedup-stream", () => {
  ...
  describe("deduplicateDefectLog", () => {
    it("returns empty array for empty inputs", () => {
      ...
      expect(deduplicateDefectLog(null as unknown as DefectRecordInput[])).toEqual([]);
    });
  ...
  describe("parseAndDeduplicateDefectJsonl and serializeAggregatedDefectLog", () => {
    ...
    it("returns empty array on empty input", () => {
      ...
      expect(parseAndDeduplicateDefectJsonl(null as unknown as string)).toEqual([]);
    });
  ...
  describe("streamDeduplicateDefects", () => {
    it("streams and deduplicates strings and object chunks with window and maxEntries", async () => {
      async function* generateChunks() {
        ...
        yield "invalid json";
        yield "   ";
        yield sampleDefects[1]!; // duplicates sampleDefects[0]
        yield sampleDefects[2]!; // new defect
        yield { type: "third_defect", observation: "third", timestamp: "2026-08-24T10:05:00Z" }; // causes eviction
      }
    ...
    it("transforms chunks and evicts oldest when exceeding maxEntries", async () => {
      ...
      const inputStream = new ReadableStream<string | DefectRecordInput>({
        start(controller) {
          ...
          controller.enqueue(sampleDefects[1]!); // dedup with defect 0
          controller.enqueue(sampleDefects[2]!); // entry 2
          controller.enqueue({ type: "third_defect", observation: "third" }); // entry 3, triggers eviction of entry 1
          controller.close();
        },
      });
  ...
  describe("filterDefectStream", () => {
    it("filters defects by category, status, minSeverity, agentId", () => {
