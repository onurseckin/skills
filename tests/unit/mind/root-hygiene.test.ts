...
import {
  ...
  RootHygieneEngine,
  scanRootHygiene,
} from "../../../olt/scripts/src/mind/root-hygiene.ts";
...
describe("mind/root-hygiene scanner and quarantine engine", () => {
  ...
  test("root-hygiene.ts satisfies all strict repository invariants", () => {
    const filePath = join(process.cwd(), "olt/scripts/src/mind/root-hygiene.ts");
    ...
    const testFilePath = join(process.cwd(), "tests/unit/mind/root-hygiene.test.ts");
