...
const restrictedEnvironment = {
  ...
  GIT_OPTIONAL_LOCKS: "0", GIT_PAGER: "cat", GIT_TERMINAL_PROMPT: "0", PAGER: "cat",
};

const restrictedPrefix = [
  "-c", "core.hooksPath=/dev/null", "-c", "core.fsmonitor=false",
  "-c", "diff.external=", "-c", "pager.diff=false", "--no-pager",
];
...
describe("restricted Git policy", () => {
  test("builds an exact packet Git environment without ambient helper injection", () => {
    expect(
      repositoryGitEnvironment({
        PATH: "/usr/bin:/bin", LANG: "C", GIT_DIR: "/poison", GIT_EXTERNAL_DIFF: "/poison/diff",
        GIT_PAGER: "/poison/pager", GIT_CONFIG_COUNT: "1", GIT_NO_REPLACE_OBJECTS: "0", GIT_REPLACE_REF_BASE: "refs/poison/",
      }),
  ...
  test("uses exact restricted argv and environment at the packet Git spawn seam", () => {
    ...
    expect(command("/repo", ["status", "--porcelain=v2"], 64)).toEqual({ status: 0, bytes: Buffer.from("ok\n") });
    expect(calls).toEqual([
      {
        executable: "git",
        argv: [...restrictedPrefix, "-C", "/repo", "status", "--porcelain=v2"],
        options: {
          encoding: "buffer", env: { ...restrictedEnvironment, LANG: "C", PATH: "/usr/bin:/bin" },
          shell: false, maxBuffer: 65, timeout: 15_000, killSignal: "SIGKILL",
        },
      },
    ]);
  });
  ...
  test("executes both accepted Git gates with canonical diff restrictions", () => {
    expect(restrictedGateGitArgv(["/usr/bin/git", "diff", "--check"])).toEqual(["/usr/bin/git", ...restrictedPrefix, "diff", "--no-ext-diff", "--no-textconv", "--check"]);
    expect(restrictedGateGitArgv(["/usr/bin/git", "diff", "--cached", "--check"])).toEqual(["/usr/bin/git", ...restrictedPrefix, "diff", "--no-ext-diff", "--no-textconv", "--cached", "--check"]);
    expect(restrictedGateGitArgv(["Env.EXE", "Git", "diff", "--check"])).toEqual(["Env.EXE", "Git", ...restrictedPrefix, "diff", "--no-ext-diff", "--no-textconv", "--check"]);
    expect(restrictedGateGitArgv(["COMMAND.EXE", "git.EXE", "diff", "--cached", "--check"])).toEqual(["COMMAND.EXE", "git.EXE", ...restrictedPrefix, "diff", "--no-ext-diff", "--no-textconv", "--cached", "--check"]);
  });
  ...
  test("adds exact noninteractive Git restrictions to every gate environment", () => {
    const environment = captureGateEnvironment(
      { PATH: "/usr/bin:/bin", GIT_NO_REPLACE_OBJECTS: "0", GIT_PAGER: "/poison/pager", PAGER: "less" },
  ...
  test("persists bounded Git execution argv and rejects record drift", async () => {
    ...
    try {
      const binding = {
        schema: "harness.repository-binding" as const, version: 1 as const,
        inspection_sha256: "a".repeat(64), git_identity_sha256: "b".repeat(64), content_sha256: "c".repeat(64),
        file_count: 0, total_bytes: 0,
      };
      const runner = createInternalCommandRunner({
        inspectRepository: () => binding,
        attempt: async () => { throw new Error("not executed"); },
      });
      const prepared = await runner.prepareCommand({
        argv: ["git", "diff", "--check"], cwd: repo, runRoot: join(repo, ".capsules"), commandDir: join(repo, ".capsules", "commands"), actor: "validator", gateId: "G-diff",
      });
      expect(prepared.record.execution_argv).toEqual([realpathSync(git), ...restrictedPrefix, "diff", "--no-ext-diff", "--no-textconv", "--check"]);
      ...
      expect(verifyCommandRecord(join(repo, ".capsules"), forged)).toContain("Git gate execution argv does not match its restricted policy");
      ...
      expect(verifyCommandRecord(join(repo, ".capsules"), noncanonical)).toContain("Git gate command is not an accepted restricted diff check");
    } finally {
  ...
  test("rejects non-grammar Git gates before publishing or attempting them", async () => {
    ...
    try {
      ...
      const runner = createInternalCommandRunner({
        inspectRepository: () => ({
          schema: "harness.repository-binding", version: 1,
          inspection_sha256: "a".repeat(64), git_identity_sha256: "b".repeat(64), content_sha256: "c".repeat(64),
          file_count: 0, total_bytes: 0,
        }),
        attempt: async () => { attempted = true; throw new Error("must not execute"); },
      });
      for (const argv of [
        ["git", "status"], ["git", "diff", "--check", "HEAD"], ["git", "-c", "core.fsmonitor=/poison", "status"],
        ["env", "git", "status"], ["command", "git", "status"], ["Git", "status"], ["env", "GIT", "diff", "--check", "HEAD"],
        ["command", "git.EXE", "status"], ["./scripts/git", "diff", "--check"], ["./scripts/env", "git", "diff", "--check"],
        ["./scripts/command", "git", "diff", "--cached", "--check"],
      ]) {
        await expect(
          runner.prepareCommand({ argv, cwd: root, runRoot, commandDir: join(runRoot, "commands"), actor: "validator", gateId: "G-reject" }),
