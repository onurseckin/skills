...
describe("Multi-Theme Contrast Matrix & Dynamic Color Scheme Visual Reporting Engine", () => {
  describe("Color Parsing & Validation", () => {
    ...
    it("parses hsl and hsla functional syntax with various angle units", () => {
      ...
      // Degrees, turns, radians, gradians, and unitless
      ...
      const hslRad = parseRgb("hsl(3.14159rad 100% 50%)");
      expect(hslRad.r).toBe(0);
      expect(hslRad.g).toBe(255);
      expect(hslRad.b).toBe(255);

      const hslGrad = parseRgb("hsl(200grad 100% 50%)");
      expect(hslGrad).toEqual({ r: 251, g: 0, b: 255, a: 1 });

      const hslUnitless = parseRgb("hsl(180 100% 50%)");
      expect(hslUnitless).toEqual({ r: 0, g: 255, b: 255, a: 1 });

      const hslNan = parseRgb("hsl(invalid 100% 50%)");
      expect(hslNan).toEqual({ r: 255, g: 0, b: 0, a: 1 });
    ...
    it("safely handles malformed inputs without throwing", () => {
      ...
      expect(parseRgb("   ")).toEqual({ r: 0, g: 0, b: 0, a: 0 });
      expect(parseRgb(null as unknown as string)).toEqual({ r: 0, g: 0, b: 0, a: 0 });
  ...
  describe("Relative Luminance & WCAG 2.1 Contrast Calculation", () => {
    ...
    it("evaluates WCAG contrast with semi-transparent foregrounds and backgrounds", () => {
      ...
      const whiteOnTranslucentBlack = calculateWcagContrast("#ffffff", "rgba(0, 0, 0, 0.5)");
      expect(whiteOnTranslucentBlack).toBeGreaterThan(1.0);
  ...
  describe("Multi-Theme Matrix Evaluation", () => {
    ...
    it("evaluates large text detection through font-weight strings", () => {
      const elements: ElementThemePair[] = [
        {
      ...
      const report = evaluateThemeContrastMatrix(elements, ["wcag-aa"]);
      expect(report.overallPassed).toBe(true);
    });
    ...
    it("detects and flags invalid color expressions gracefully", () => {
      const invalidElements: ElementThemePair[] = [
        ...
        {
          selector: "div.both-invalid",
          theme: "light",
          foregroundColor: "invalid-fg",
          backgroundColor: "invalid-bg",
        },
        {
          selector: "div.bg-invalid",
          theme: "light",
          foregroundColor: "#ffffff",
          backgroundColor: "invalid-bg",
        },
  ...
  describe("Theme Regression Detection", () => {
    ...
    it("detects high-contrast inverted regressions where high-contrast theme has less contrast than standard theme", () => {
      const invertedElements: ElementThemePair[] = [
        ...
        {
          selector: "span.badge-dark",
          theme: "dark",
          foregroundColor: "#ffffff",
          backgroundColor: "#000000", // CR = 21:1
        },
        {
          selector: "span.badge-dark",
          theme: "high-contrast-dark",
          foregroundColor: "#999999",
          backgroundColor: "#000000", // CR ~ 7.5:1
        },
      ];
      ...
      const hcDarkRegression = report.findings.find((f) => f.id.includes("REGRESS-HCDARK"));
      expect(hcDarkRegression).toBeDefined();
      expect(hcDarkRegression?.severity).toBe("moderate");
      expect(hcDarkRegression?.message).toContain("High-contrast dark mode inverted contrast");
    });
  });
  ...
  describe("Markdown Report Formatting & ASCII/Unicode Matrix Tables", () => {
    ...
    it("renders missing themes gracefully in detailed matrix table", () => {
      const report: MultiThemeComparisonReport = {
        timestamp: new Date().toISOString(),
        totalElements: 1,
        evaluatedThemes: ["light", "dark"],
        evaluatedStandards: ["wcag-aa"],
        matrices: [
          {
            ...
            themes: {
              light: {
                foregroundColor: "#ffffff",
                backgroundColor: "#000000",
                effectiveForeground: { r: 255, g: 255, b: 255, a: 1 },
                effectiveBackground: { r: 0, g: 0, b: 0, a: 1 },
                wcagRatio: 21,
                apcaLc: -106,
                evaluations: [
                  {
      ...
      expect(md).toContain(
        "| `header.banner` | dark | *none* | *none* | N/A | N/A | ✗ FAIL | ✗ FAIL | ✗ FAIL | FAIL |",
      );
    });
  });
});
