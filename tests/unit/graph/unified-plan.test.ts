...
describe("unified-plan: compileUnifiedHighLeveragePlan", () => {
  test("compiles atomic high-leverage plan with requirements, decoupling, and topology", () => {
    ...
    expect(result.topology.waves).toHaveLength(1); // Decoupled into 1 parallel wave
    ...
    const tasks = [
      {
        ...
        label: "Task A",
        writeScope: ["src/a.ts"],
        gate: "bun test tests/common.test.ts", // Identical gate over disjoint scopes!
      },
      {
        id: "task-b",
        label: "Task B",
  ...
  test("rejects plan with illegal transitive bypass edge", () => {
    const repo = fixtureRepo(roots);
    // Task 3 depends on Task 2, Task 2 depends on Task 1, but Task 3 also depends directly on Task 1
    const tasks = [
      {
        id: "task-1",
        label: "Task 1",
      ...
      },
      {
        ...
        deps: ["task-2", "task-1"], // Direct bypass of task-2!
      },
    ];
