...
import { parseCoverageTable } from "../../../orchestrating-long-tasks/scripts/src/cli/commands/coverage-check.ts";
...
describe("coverage sweep gap tests: git runner edge cases", () => {
  ...
  test("git helper throws with exit status when stderr is empty", () => {
    ...
    expect(() => git("/tmp", ["status"], failingRunnerEmptyStderr)).toThrow(
      /exit status 1/,
    );
  });
});
  ...
  test("provisionWorktrees refuses worktree_root inside repo with PATH_SAFETY error", () => {
    const input: ProvisionWorktreesInput = {
      ...
      loadState: () => ({
        ...
        tasks: {},
        task_order: [],
        graph: { version: 1, schema: "harness.graph", revision: 1, nodes: [], edges: [], gates: [] },
        topology: { revision: 1, max_parallel: 1, waves: [], decisions: [] },
        requirements: { version: 1, schema: "harness.requirements", prompt_sha256: "0".repeat(64), requirements: [], dispositions: [] },
      }),
    };
