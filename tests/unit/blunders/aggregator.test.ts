...
describe("Blunder Entry Aggregator", () => {
  describe("toAggregatedBlunder", () => {
    test("initializes an aggregated blunder record from raw input", () => {
  ...
  describe("aggregateBlunderEntries", () => {
    test("increments count and updates timestamps correctly", () => {
      const base: BlunderRecordInput = {
        id: "b-base",
        type: "type_error",
        severity: "warning",
        timestamp: "2026-08-22T08:00:00.000Z",
      ...
      const target = toAggregatedBlunder(base);
      ...
      const updated = aggregateBlunderEntries(target, incoming);
    ...
    test("upgrades severity when incoming is higher severity", () => {
      const target = toAggregatedBlunder({
        type: "syntax_defect",
        severity: "warning",
    ...
    test("bounds occurrences list to maximum limit", () => {
      let current = toAggregatedBlunder({
        type: "spam_error",
        observation: "Rapid loop error",
      });

      for (let i = 0; i < 60; i += 1) {
        current = aggregateBlunderEntries(
          current,
          {
            type: "spam_error",
            observation: "Rapid loop error",
