...
describe("Automated Visual Validation & Screenshot Pipeline", () => {
  test("run:exec detects screenshots in test-results, copies to evidence and reports, updates manifest", async () => {
    ...
    const exec = await execute([
      "run:exec", "--run", run, "--task", "task-core", "--cwd", repo, "--", "echo", "run",
    ]);
  ...
  test("run:exec ingests screenshots referenced in command stdout", async () => {
    ...
    writeFileSync(join(repo, "script.ts"), `console.log("Screenshot: screenshots/modal.png");\n`, "utf-8");

    const exec = await execute([
      "run:exec", "--run", run, "--task", "task-core", "--cwd", repo, "--", "bun", "script.ts",
    ]);
    const screenshots = exec.screenshots as string[];
  ...
  test("evidence:screenshots queries and filters captured UI screenshots", async () => {
    ...
    const exec = await execute([
      "run:exec", "--run", run, "--task", "task-core", "--actor", "actor-a", "--cwd", repo, "--", "echo", "done",
    ]);
    const cmdId = String(exec.command_id);
    ...
    const byTaskActor = await execute([
      "evidence:screenshots", "--run", run, "--task", "task-core", "--actor", "actor-a",
    ]);
    expect(Number(byTaskActor.count)).toBeGreaterThanOrEqual(1);

    const wrongActor = await execute([
      "evidence:screenshots", "--run", run, "--task", "task-core", "--actor", "wrong-actor",
    ]);
  ...
  test("evidence:get and report:get support --screenshots inspection flag and handle empty screenshots", async () => {
    ...
    const { valToken } = await submitAndStartValidation({
      run, repo, taskId: "task-core", worker: "w1", validator: "v1",
    });

    const getSubRep = await execute([
      "report:get", "--run", run, "--task", "task-core", "--submission", "--screenshots",
    ]);
    ...
    await execute([
      "task:review", "--run", run, "--task", "task-core", "--validator", "v1",
      "--token", valToken, "--evidence", cmdId, "--status", "pass", "--summary", "passed",
    ]);
    ...
    const getRep = await execute(["report:get", "--run", run, "--task", "task-core", "--screenshots"]);
  ...
  test("task:reject records screenshots in findings and reports during failure review", async () => {
    ...
    const reject = await execute([
      "task:reject", "--run", run, "--task", "task-core", "--validator", "v1",
      "--token", valToken, "--evidence", cmdId, "--reason", "Visual overflow",
    ]);
  ...
  test("deterministic in-place screenshot overwrite updates files and deduplicates manifest.json", async () => {
    ...
    const originalFile = createMockScreenshot(join(repo, "test-results"), "drawer-overview.png", "content-v1");
    ...
    const manifestBefore = readJsonFile<{ screenshots: Array<{ name: string; size_bytes?: number }> }>(
      join(run, "evidence", "manifest.json"),
    );
    expect(manifestBefore.screenshots.filter((s) => s.name === "cmd-fixed-drawer-overview.png").length).toBe(1);
    ...
    const manifestAfter = readJsonFile<{ screenshots: Array<{ name: string; size_bytes?: number }> }>(
      join(run, "evidence", "manifest.json"),
    );
    const matchedEntries = manifestAfter.screenshots.filter((s) => s.name === "cmd-fixed-drawer-overview.png");
  ...
  test("run:exec discovers and ingests visual-report.json into evidence and reports", async () => {
    ...
    const mockReport: VisualMetricsReport = {
      ...
      layoutOverflows: [{ element: "nav-bar", selector: ".top-nav", scrollWidth: 410, clientWidth: 375, delta: 35, viewport: "mobile" }],
      textClippings: [{ element: "heading", selector: "h1.title", text: "Very long title truncated", scrollWidth: 400, clientWidth: 350, viewport: "mobile" }],
      collisions: [{ elements: ["button#save", "div#overlay"], selectors: ["#save", "#overlay"], zIndex: 10, overlapArea: 120, viewport: "desktop" }],
    };

    mkdirSync(join(repo, "test-results"), { recursive: true });
    writeFileSync(join(repo, "test-results", "visual-report.json"), JSON.stringify(mockReport, null, 2), "utf-8");

    const exec = await execute([
      "run:exec", "--run", run, "--task", "task-core", "--cwd", repo, "--", "echo", "visual-pass",
    ]);
  ...
  test("task:review attaches visual_report to task review report", async () => {
    ...
    const review = await execute([
      "task:review", "--run", run, "--task", "task-core", "--validator", "v1",
      "--token", valToken, "--evidence", cmdId, "--status", "pass", "--summary", "Visual validation clean",
    ]);
