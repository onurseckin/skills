...
// graph.json completeness contract. MediaAsset.sizeBytes / MediaAsset.dimensions have existed on
// the schema since the first rewrite, and asset-measure.ts already reads real image headers, but
// nothing before this file drove a REAL file through the graph generator to prove the two survive
// export. A fixture that never writes bytes to disk cannot catch a regression in that path.
...
/** A real, parseable PNG file: an IHDR chunk carrying width/height, padded past the 32-byte header read. */
...
describe("captured asset dimensions and byte size reach graph.json", () => {
  ...
  test("a harness-recorded screenshot capture keeps its recorded byte count and gains real pixel dimensions from the file", () => {
    ...
    const recordedBytes = 999999; // deliberately not the file's real size on disk
    recordCaptures(root, [
      capture({
    ...
    const asset = dataset.nodes
      .find((node) => node.id === "node-validator-T-1")
      ?.assets?.find((candidate) => candidate.url === path);
    // The byte count the harness recorded at capture time is authoritative and is never silently
    // replaced by a re-measurement of the file; the pixel dimensions have no other source, so they
    // still come from actually reading the file's header.
