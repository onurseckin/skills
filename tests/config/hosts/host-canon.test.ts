...
describe("parseHostProfiles", () => {
  ...
  test("canonicalizes a claude key onto claude-code", () => {
    const parsed = parseHostProfiles(
      { claude: { timer_arming_mechanism: "none" } },
      "harness.config.json",
    );
    expect(parsed["claude-code"]).toBeDefined();
    expect(parsed["claude"]).toBeUndefined();
    expect(parsed["claude-code"]?.timer_arming_mechanism).toEqual({
      value: "none",
      source: "config_override",
    });
  });
  ...
  test("B4 — an absent models_available is absent, distinguishable from an explicit empty array", () => {
    const parsed = parseHostProfiles({ "claude-code": {} }, "harness.config.json");
    expect(parsed["claude-code"]?.models_available).toEqual({
      value: [],
      source: "absent",
    });
  });
  ...
  test("B4 — a non-empty models_available is config_override and filters non-string entries", () => {
    const parsed = parseHostProfiles(
      { "claude-code": { models_available: ["sonnet", 42, "opus", null] } },
      "harness.config.json",
    );
    expect(parsed["claude-code"]?.models_available).toEqual({
      value: ["sonnet", "opus"],
  ...
  test("attests wake_driver_present as config_override only when the file states it explicitly", () => {
    const parsedExplicit = parseHostProfiles(
      { "claude-code": { wake_driver_present: true } },
      "harness.config.json",
    );
    expect(parsedExplicit["claude-code"]?.wake_driver_present).toEqual({
      value: true,
      source: "config_override",
    });

    const parsedImplicit = parseHostProfiles({ "claude-code": {} }, "harness.config.json");
    expect(parsedImplicit["claude-code"]?.wake_driver_present).toEqual({
      value: false,
      source: "absent",
    });
  });
  ...
  test("B5 — an absent wake_driver_present is absent, distinguishable from unreadable or explicit false", () => {
    const parsed = parseHostProfiles({ "claude-code": {} }, "harness.config.json");
    expect(parsed["claude-code"]?.wake_driver_present).toEqual({
      value: false,
      source: "absent",
    });
  });
  ...
  test("B3/B5 — an invalid timer_arming_mechanism is unreadable, distinguishable from absent or an explicit none", () => {
    const parsed = parseHostProfiles(
      { "claude-code": { timer_arming_mechanism: "cron-job" } },
      "harness.config.json",
    );
    expect(parsed["claude-code"]?.timer_arming_mechanism).toEqual({
      value: "none",
  ...
  test("B3 — an absent self_wake_supported is absent, distinguishable from an explicit false", () => {
    const parsed = parseHostProfiles({ "claude-code": {} }, "harness.config.json");
    expect(parsed["claude-code"]?.self_wake_supported).toEqual({
      value: false,
      source: "absent",
    });
  });
  ...
  test("returns an empty record for a non-object value rather than throwing", () => {
    ...
    expect(parseHostProfiles(undefined, "harness.config.json")).toEqual({});
    expect(parseHostProfiles(["array"], "harness.config.json")).toEqual({});
    expect(parseHostProfiles("string", "harness.config.json")).toEqual({});
  });
});
