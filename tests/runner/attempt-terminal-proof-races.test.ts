...
import { writeAttemptFailureEvidence } from "../../orchestrating-long-tasks/scripts/src/runner/attempt-failure-evidence.ts";
import type { AttemptProcessProof } from "../../orchestrating-long-tasks/scripts/src/runner/attempt-intent.ts";
import { DescendantTracker } from "../../orchestrating-long-tasks/scripts/src/runner/descendant-tracker.ts";
...
function cleanup(overrides: Record<string, unknown> = {}) {
  return cleanupFailedAttempt({
    ...
    graceMs: 0,
    drainTimeoutMs: 1,
    probeProcess: () => { throw new Error("unexpected process probe"); },
    terminateGroup: async () => { throw new Error("unexpected process signal"); },
    ...overrides,
  } as never);
}
...
describe("failed attempt descendant tracking and races", () => {
  ...
  test("withholds absence proof for a live reparented token owner without pipe anchors", async () => {
    const runner: ProcessIdentity = { pid: 30, parent: 20, group: 30, birth: "runner" };
    const escaped: ProcessIdentity = { pid: 50, parent: 1, group: rootIdentity.group, birth: "escaped" };
    ...
    const tracker = new DescendantTracker(rootIdentity.pid, new Set(), "token", {
      ...
      ownedPids: () => new Set(),
      tokenOwners: () => [escaped],
      kill: () => { throw new Error("absence proof must not signal a token owner"); },
  ...
  test("fails closed when the bounded token-owner scan is unavailable", async () => {
    ...
    const tracker = new DescendantTracker(rootIdentity.pid, new Set(), "token", {
      ...
      identify: (pid) => processes.get(pid),
      ownedPids: () => new Set(),
      tokenOwners: () => { throw new Error("token-owner scan unavailable"); },
      kill: () => { throw new Error("uncertain ownership must not signal a process"); },
      probe: () => "absent",
    });
  ...
  test("persists normal-phase and cleanup signals after a later evidence failure", async () => {
    ...
    const stdoutPath = join(attemptDir, "stdout.log"), stderrPath = join(attemptDir, "stderr.log"), activityPath = join(attemptDir, "activity.json");
