...
import type { PreparedCommand } from "../../orchestrating-long-tasks/scripts/src/runner/types.ts";
...
async function writeGateAttempt(
  ...
  atomicWriteJson(activityPath, { ...activity, status: "completed", finished_at: finishedAt }, 0o600);
  ...
  atomicWriteJson(
    join(attemptRoot, "record.json"),
    {
      ...
      evidence_issues: [],
      ...(repositoryAfter ? { repository_after: repositoryAfter, gate_finalized_at: finishedAt } : {}),
    },
    0o600,
  );
}
...
describe("interrupted command attempt reconciliation", () => {
  ...
  test("fails a finalized gate whose same-attempt repository binding drifted", async () => {
    ...
    const recovered = recoverAggregateFromAttempts(runRoot, prepared.record, {
      inspectRepository: () => { throw new Error("must not substitute a recovery-time binding"); },
    });
  ...
  test("rejects an oversized durable attempt before aggregate publication", async () => {
    ...
    expect(() => recoverAggregateFromAttempts(runRoot, prepared.record)).toThrow(/maximum|size|large/i);
    expect(JSON.parse(await readFile(prepared.recordPath, "utf8")).status).toBe("running");
  });
  ...
  test("rejects an oversized recovered aggregate before publication", async () => {
    ...
    expect(() => recoverAggregateFromAttempts(runRoot, prepared.record)).toThrow(/maximum|size|limit/i);
  });
});
