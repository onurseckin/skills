...
} from "../../orchestrating-long-tasks/scripts/src/runner/gate-path-bindings.ts";
import { gateControlBindingsOverlapWriteScopes } from "../../orchestrating-long-tasks/scripts/src/runner/gate-path-overlap.ts";
...
describe("dynamic gate path bindings", () => {
  test("rejects repeated canonical directories before any recursive open or read", async () => {
    ...
    expect({ opens, directoryReads, fileReads }).toEqual({ opens: 0, directoryReads: 0, fileReads: 0 });
  });
  ...
  test("shares the recursive byte and digest-work budget across distinct operands", async () => {
    ...
    const first = join(root, "suite-a"), second = join(root, "suite-b");
    await mkdir(first);
    await mkdir(second);
    const firstFile = join(first, "large.test.ts"), secondFile = join(second, "large.test.ts");
  ...
  test("protects interpreter programs and configs from every task write scope", async () => {
    ...
    const bindings = captureGatePathBindings(root, root, ["bun", "tools/verify.ts", "--config=tools/gate.json"]);
    expect(bindings.filter(({ scope }) => scope === "repository").map(({ role }) => role)).toEqual([
      "program",
      "config",
    ]);
    expect(gateControlBindingsOverlapWriteScopes(bindings, [["src"], ["tools/verify.ts"]])).toBeTrue();
  ...
  test("rejects capture-to-spawn mutation before invoking the attempt runner", async () => {
    ...
    await writeFile(executable, "#!/bin/sh\nexit 1\n", { mode: 0o700 });
    await expect(runner.executePreparedCommand(prepared)).rejects.toThrow(/identity|digest|changed/i);
