...
import { terminateProcessGroup } from "../../../../olt/scripts/src/engine/runner/process/process-group.ts";
...
describe("successful signal delivery ledger", () => {
  ...
  test("resumes process-group cleanup after a delivered TERM without sending TERM twice", async () => {
    ...
    const result = await terminateProcessGroup(40, 0, new Promise(() => undefined), root, {
      inspect: () => root,
      kill: (_pid, signal) => {
        sent.push(signal);
        return true;
