...
describe("Defect Synthesizer Unit Tests", () => {
  it("normalizes Finding contracts and infers file paths from text", () => {
    const finding: Finding = {
      ...
      evidence: [],
      remediation: "Add null check in src/auth/token.ts and tests/auth/token.test.ts",
    ...
    expect(detail.file_paths).toContain("tests/auth/token.test.ts");
    expect(detail.file_paths_evidence_class).toBe("derived");
    expect(detail.revalidation_gate).toBe("bun test tests/auth/token.test.ts");
  });
  ...
  it("synthesizes structured prompt with categorized findings, critic feedback, and failed gates", () => {
    const findings: FindingDetail[] = [
      {
        ...
        revalidation_gate: "bun test tests/db",
      },
      {
        id: "imp-01",
        ...
        revalidation_gate: "bun test tests/api",
      },
      {
        id: "min-01",
    ...
    const synthesis = synthesizeNextRoundPrompt({
      ...
      gateFailures: ["bun test tests/db", "bun test tests/api"],
    });
    ...
    expect(synthesis.synthesizedPrompt).toContain("❌ `bun test tests/db`");
  });
});
