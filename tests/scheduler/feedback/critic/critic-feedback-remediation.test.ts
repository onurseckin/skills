...
describe("Critic Feedback: Routing & Autonomous Remediation", () => {
  ...
  describe("routeCriticFeedback", () => {
    ...
    test("verifies strict escalation budget boundary at maxRepairRounds threshold", () => {
      const state = workflowState();
      state.tasks["T-SUB"] = {
        ...state.tasks["T-1"]!,
        id: "T-SUB",
        status: "changes_requested",
        requirement_ids: ["R-BOUND"],
        repair_round: 1,
        findings: [],
      };
      state.tasks["T-EXHAUST"] = {
        ...state.tasks["T-1"]!,
        id: "T-EXHAUST",
        status: "changes_requested",
        requirement_ids: ["R-EXHAUST"],
        repair_round: 2,
        findings: [],
      };
      ...
      const findings = [
        {
          id: "F-SUB",
          requirement_id: "R-BOUND",
          observation: "Sub-boundary issue",
          remediation: "Fix sub",
        },
        {
          id: "F-EXHAUST",
          requirement_id: "R-EXHAUST",
          observation: "Boundary issue",
          remediation: "Fix exhaust",
        },
      ];

      const port = new TestPort(state);
      const result = routeCriticFeedback(port, { actor: "val-1", role: "validator" }, findings, {
        maxRepairRounds: 3,
      });

      expect(result.changesRequestedTaskIds).toContain("T-SUB");
      expect(result.escalatedTaskIds).toContain("T-EXHAUST");
      expect(result.totalTasksEscalated).toBe(1);

      const updated = port.read();
      expect(updated.tasks["T-SUB"]?.status).toBe("changes_requested");
      expect(updated.tasks["T-SUB"]?.repair_round).toBe(2);
      expect(updated.tasks["T-EXHAUST"]?.status).toBe("escalated");
      expect(updated.tasks["T-EXHAUST"]?.repair_round).toBe(3);
    });
  });
});
