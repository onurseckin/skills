...
describe("restricted Git policy", () => {
  ...
  test("persists bounded Git execution argv and rejects record drift", async () => {
    ...
    vfs.mkdirSync(join(repo, ".olt", "capsules", "commands"), { recursive: true });
    const git = join(tools, "git");
    vfs.writeFileSync(git, "not executed\n");
    session.chmodSync(git, 0o700);
    ...
    try {
      ...
      session.rmSync(root, { recursive: true, force: true });
    }
  });
