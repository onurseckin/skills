...
import { createSampleRuntimeSummary, RANKING_SUITES } from "./index.ts";
...
describe("Test Runtime Telemetry & Pareto Ranking", () => {
  ...
  describe("parseTestRuntimeOutput", () => {
    test("parses multi-file Bun test output with ANSI codes and summary line", () => {
      const output = `
\u001b[1mtests/engine/store/store.test.ts:\u001b[0m
(pass) store > initialize [10.50ms]
(pass) store > execute [20.50ms]

\u001b[1mtests/testing/runner/test-runner.test.ts:\u001b[0m
...
Ran 5 tests across 2 files. [250.00ms]
      ...
      expect(summary.files[0]?.file).toBe("tests/testing/runner/test-runner.test.ts");
      expect(summary.files[0]?.durationMs).toBe(200);
      expect(summary.files[0]?.passed).toBe(false);
      expect(summary.files[1]?.file).toBe("tests/engine/store/store.test.ts");
