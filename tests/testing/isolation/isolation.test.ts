...
import { createSampleIsolationContext, ISOLATION_SUITES } from "./index.ts";
...
describe("testing subsystem isolation sandbox", () => {
  ...
  it("runs with runWithIsolation wrapper helper and validates facade fixture", async () => {
    ...
    const sample = createSampleIsolationContext("fixture-test");
    expect(sample.id).toBeDefined();
    await sample.cleanup();
    expect(ISOLATION_SUITES.length).toBe(1);
  });
});
