...
describe("VirtualMemoryFS Core Engine", () => {
  ...
  describe("deletion operations (unlinkSync & rmSync)", () => {
    ...
    it("removes directories and files with rmSync", () => {
      ...
      expect(() => fs.rmSync("/nest")).toThrow(VirtualFSError);
  ...
  describe("working directory, snapshots & reset", () => {
    ...
    it("provides working singleton virtualFS instance and fixture helper", () => {
      ...
      const testFs = createTestVirtualMemoryFS();
      expect(testFs.readFileSync("/fixtures/sample.txt", "utf-8")).toBe("sample virtual content");
