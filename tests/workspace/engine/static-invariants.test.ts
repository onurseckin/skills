...
describe("Static Invariant Verification: Zero TypeScript any, Zero Suppressions & <= 300 LOC", () => {
  ...
  it("verifies all workspace test files satisfy strict monorepo rules", () => {
    const workspaceDir = join(import.meta.dir, "..");
    const files = getTsFilesRecursively(workspaceDir);
  ...
  it("enforces strict directory density (<= 10 items) per folder in tests/workspace/", () => {
