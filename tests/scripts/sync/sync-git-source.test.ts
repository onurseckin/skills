...
describe("getDirtyOltPaths", () => {
  ...
  function makeTempDir(label: string): string {
    const dir = realpathSync(mkdtempSync(join(tmpdir(), `sync-git-${label}-`)));
    roots.push(dir);
    return dir;
  }
  ...
  test("ignores changes outside the olt/ subtree", () => {
    const root = makeTempDir("outside-scope-repo");
    initFixtureRepo(root);
...
describe("materializeOltFromHead", () => {
  ...
  function makeTempDir(label: string): string {
    const dir = realpathSync(mkdtempSync(join(tmpdir(), `sync-mat-${label}-`)));
    roots.push(dir);
    return dir;
  }
  ...
  test("materializes the committed content, not the dirty worktree edit", () => {
    const root = makeTempDir("materialize-repo");
    const tmpParent = makeTempDir("materialize-tmp-parent");
    initFixtureRepo(root);
...
describe("resolveOltSyncSource", () => {
  ...
  function makeTempDir(label: string): string {
    const dir = realpathSync(mkdtempSync(join(tmpdir(), `sync-res-${label}-`)));
    roots.push(dir);
    return dir;
  }
  ...
  test("clean tree without --allow-dirty materializes an isolated HEAD copy", () => {
    const root = makeTempDir("resolve-clean-repo");
    initFixtureRepo(root);
  ...
  test("dirty tree without --allow-dirty refuses and names the dirty paths in the error", () => {
    const root = makeTempDir("resolve-dirty-repo");
    initFixtureRepo(root);
  ...
  test("dirty tree with --allow-dirty deploys the live worktree unchanged", () => {
    const root = makeTempDir("resolve-allow-dirty-repo");
    initFixtureRepo(root);
