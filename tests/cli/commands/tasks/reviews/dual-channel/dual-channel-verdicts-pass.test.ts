...
describe("Task Review Dual-Channel - Verdicts Pass With Deep Evidence", () => {
  beforeEach(() => {
    setupVirtualCliFS();
  });
  ...
  const setupTestCapsule = async (): Promise<{
    ...
    const testDir = `/virtual/cli/test-dual-chan-verdicts-pass-${Math.random().toString(36).slice(2)}`;
    roots.push(testDir);
  ...
  it("taskReviewCommand passes and records review when companion manifest contains deep quantitative evidence", async () => {
