/**
 * @file fixture.ts
 * In-memory virtual test sandbox fixture and harness for tests/capture domain.
...
export function setupVirtualCaptureFS(): VirtualMemoryFS {
  if (!currentSession) {
...
export function cleanupVirtualCaptureFS(): void {
  if (currentSession) {
...
/**
 * Creates an in-memory virtual scratch sandbox directory for capture tests.
 * Zero physical disk writes occur.
 */
export function scratchRoot(callerPath = "capture-test", label = "test"): string {
  const vfs = setupVirtualCaptureFS();
  counter += 1;
  ...
  const dirName = `${fileTag}-${labelTag}-${counter}-${digest}`
    ...
    .replace(/-+$/, "");
  const repoRoot = normPath(process.cwd());
  const root = path.join(repoRoot, ".olt", "scratch", dirName);
  vfs.mkdirSync(root, { recursive: true });
  return root;
}

export function getVirtualCaptureFS(): VirtualMemoryFS {
  return currentVfs;
}
