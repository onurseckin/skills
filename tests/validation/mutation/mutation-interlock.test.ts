...
describe("Mutation Interlock Enforcement", () => {
  ...
  it("permits file mutation when target file is inside the leased write scope", () => {
    ...
    const session = registerSessionGrant({
      ...
      pid: 91105,
      ppid: 91104,
      writeScope: ["olt/scripts/src/workflow/lease/", "tests/workflow/lease/guard.test.ts"],
    });
    ...
    expect(() =>
      assertLeaseTokenForFileMutation("tests/workflow/lease/guard.test.ts", session.token, {
        runRoot: sandboxDir,
      }),
    ).not.toThrow();
