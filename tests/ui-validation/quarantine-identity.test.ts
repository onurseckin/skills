...
describe("IdentityGovernanceEngine", () => {
    ...
    it("generates Playwright-compatible browser storage state", () => {
      ...
      const authStorage = storageState.origins[0]?.localStorage.find((e) => e.name === "auth_token");
      expect(authStorage).toBeDefined();
      expect(authStorage?.value).toBeDefined();

      const userStorage = storageState.origins[0]?.localStorage.find((e) => e.name === "current_user");
    ...
    it("simulates multi-role permission boundaries and catches privilege leakage", () => {
      ...
      const auditResult = engine.simulatePermissionBoundary(
        "/admin/settings",
        ["*"],
        personas,
      );
