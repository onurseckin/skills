...
describe("11. End-to-End 5-Round Progressive Validation Convergence", () => {
    it("should execute a full 5-round dialectic cycle with adversarial challenges, arbitration, and lock integrity", () => {
      ...
      engine.submitDefense({
        ...
        rationale: "ARIA landmarks <header>, <main>, <footer> verified with zero duplicate banner roles.",
        evidenceReferences: ["dom:aria-snapshot"],
      });
      const r1Advance = engine.advanceRound({
        statePayload: { "layout.grid": "fluid-12", "layout.landmarks": ["header", "main", "footer"] },
      });
        ...
        candidateResolutions: [
          {
            ...
            score: 98,
            tradeoffs: "Optimal balance between headline prominence and viewport wrapping prevention.",
          },
        ],
      });
      ...
      const r3c2 = engine.raiseChallenge({
        ...
        thesis: "Modal dialog surface elevation does not cast ambient occlusion shadow in dark mode.",
      });

      // Defend both
      engine.submitDefense({
        challengeId: r3c1.challengeId,
        rationale: "Button text color shifted to token neutral-50 achieving APCA Lc 78.4 contrast against dark surface.",
        evidenceReferences: ["token:color.neutral-50", "audit:apca-contrast"],
      });
      engine.submitDefense({
        challengeId: r3c2.challengeId,
        rationale: "Dark mode elevation tier 3 applies 1px border highlight (rgba(255,255,255,0.12)) plus 16px soft shadow.",
      ...
      engine.submitDefense({
        ...
        rationale: "Focus indicator converted to focus-visible outline-offset: 2px within parent boundary.",
