...
export const commandLockSuiteName = "checkCognitiveValidatorCommandLock & checkCommandLockIntegrity";
...
describe(commandLockSuiteName, () => {
  ...
  test("detects cognitive validator executing command in state.commands", () => {
    const res = checkCognitiveValidatorCommandLock({
      state: {
        ...
        commands: [{ agent_id: "val-1", command: "bun test tests/doctor/checks/git-index-engine.test.ts" }],
      },
    });
  ...
  test("detects corrupted state.json, validator violations, and implementer whole-suite violations", () => {
    const scratch = mkdtempSync(join(tmpdir(), "cmd-lock-test-"));
  ...
  test("audits direct capsule directory path correctly", () => {
    const scratch = mkdtempSync(join(tmpdir(), "cmd-lock-direct-"));
    mkdirSync(scratch, { recursive: true });

    const state = {
      agents: { "worker-1": { role: "implementer" } },
      commands: [{ agent_id: "worker-1", command: "bun test tests/doctor/checks/pushback-quotas-engine.test.ts" }],
    };
