/**
 * @file fixture.ts
 * In-memory virtual test sandbox fixture and harness for tests/doctor domain.
...
export function setupVirtualDoctorFS(): VirtualMemoryFS {
  if (!currentSession) {
    ...
    currentVfs.mkdirSync("/virtual/doctor-scratch", { recursive: true });
...
export function cleanupVirtualDoctorFS(): void {
  if (currentSession) {
...
/**
 * Creates an in-memory virtual scratch sandbox directory for doctor tests.
 * Zero physical disk writes occur.
 */
export function scratchRoot(callerPath = "doctor-test", label = "test"): string {
  ...
  const dirName = `${fileTag}-${labelTag}-${counter}-${digest}`
    ...
    .slice(0, 50)
    .replace(/-+$/, "");
  const root = `/virtual/doctor-scratch/${dirName}`;
  vfs.mkdirSync(root, { recursive: true });
  return root;
}

export function getVirtualDoctorFS(): VirtualMemoryFS {
  return currentVfs;
}
