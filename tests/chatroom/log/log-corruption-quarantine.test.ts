...
describe("log corruption quarantine and doctor integration", () => {
  ...
  it("quarantines torn log line, readers skip it gracefully, deduplicates repeated reads, and doctor reports degraded health", async () => {
    ...
    const existing = vfs.readFileSync(segmentPath, "utf8");
    vfs.writeFileSync(
      segmentPath,
      `${typeof existing === "string" ? existing : new TextDecoder().decode(existing)}${tornLine}`,
    );
    ...
    const qDir = roomQuarantineDir(room, context.homeDir);
    const qFiles = vfs.readdirSync(qDir).filter((file) => !file.startsWith("."));
    expect(qFiles).toHaveLength(1);

    const firstQFile = qFiles[0];
    expect(firstQFile).toBeDefined();
    if (!firstQFile) {
      return;
    }
    const quarantineRecordPath = join(qDir, firstQFile);
    const rawRecord = vfs.readFileSync(quarantineRecordPath, "utf8");
    const quarantineRecord = JSON.parse(
      typeof rawRecord === "string" ? rawRecord : new TextDecoder().decode(rawRecord),
    ) as {
      readonly ts: string;
      readonly seq: number;
    ...
    const qFilesAfterRepeat = vfs.readdirSync(qDir).filter((file) => !file.startsWith("."));
  ...
  it("deduplicates identical corrupt lines in quarantineCorruptLine", () => {
    ...
    const qFilesAfter = vfs.readdirSync(qDir).filter((file) => !file.startsWith("."));
