/**
 * @file handlers.ts
 * Handlers for stats, permissions, rw, and directory traversal in defects virtual filesystem.
 */
...
export function isVirtualPath(p: string): boolean {
  ...
  return (
    ...
    s.includes("/virtual/") ||
    s.includes("defects-scratch") ||
    s.includes("test-defects")
  );
}

export function getInode(s: VirtualDefectsState, targetPath: string): number {
...
function getNlink(s: VirtualDefectsState, targetPath: string): number {
  const t = norm(targetPath);
...
export function makeSymlinkStats(s: VirtualDefectsState, targetPath: string): fs.Stats {
  const t = norm(targetPath);
  const ino = getInode(s, t);
...
export function checkReadPermission(s: VirtualDefectsState, targetPath: string): void {
