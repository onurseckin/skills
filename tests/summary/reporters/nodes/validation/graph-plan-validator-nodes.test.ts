...
describe("buildPlanValidatorNodes", () => {
  ...
  test("an expired assignment with no review renders skipped node with neutral badge and no verdict edge", () => {
    ...
    expect(result.edges.some((edge) => edge.kind === "signoff" || edge.kind === "pushback")).toBe(
      false,
    );
  });
  ...
  test("a 3-round sequence forms an acyclic pushback-to-signoff chain without bypassing rounds", () => {
          ...
          findings: [
            {
              id: "PV-1",
              severity: "critical",
              observation: "Shallow plan.",
              remediation: "Expand vectors.",
            },
            {
              id: "PV-2",
              severity: "critical",
              observation: "Missing tasks.",
              remediation: "Add tasks.",
            },
          ],
        }),
        review({
          ...
          findings: [
            {
              id: "PV-3",
              severity: "warning",
              observation: "Minor ordering issue.",
              remediation: "Reorder tasks.",
            },
          ],
        }),
        review({
    ...
    const pb1 = result.edges.find(
      (e) => e.source === "node-plan-validator-r1" && e.kind === "pushback",
    );
    expect(pb1).toBeDefined();
    expect(pb1?.target).toBe("node-plan-validator-r2");

    const pb2 = result.edges.find(
      (e) => e.source === "node-plan-validator-r2" && e.kind === "pushback",
    );
    expect(pb2).toBeDefined();
    expect(pb2?.target).toBe("node-plan-validator-r3");
    expect(pb2?.exchanges?.[0]?.finding?.id).toBe("PV-3");

    const signoff = result.edges.find(
      (e) => e.source === "node-plan-validator-r3" && e.kind === "signoff",
    );
    ...
    const directToPlan = result.edges.filter(
      (e) =>
        (e.source === "node-plan-validator-r1" || e.source === "node-plan-validator-r2") &&
        e.target === "node-orchestrator-plan" &&
        e.kind === "pushback",
    );
