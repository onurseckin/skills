...
/**
 * What gets drawn: nodes, edges and sections. One asset lives in one place among those, which is the
 * rule that stopped a single screenshot appearing on seventeen nodes. `dataset.run` is outside it on
 * purpose — it carries the records the capsule wrote, verbatim, and a record that names a screenshot
 * keeps naming it.
 */
...
describe("critic finding ownership", () => {
  test("leaves a validator's review findings on the gate instead of republishing them", () => {
    ...
    // The finding the validator actually filed stays on the gate node, and the review-recorded
    // event mints nothing of its own: it states a verdict, not a defect.
