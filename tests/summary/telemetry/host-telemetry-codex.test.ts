...
describe("host identity detection", () => {
  ...
  test("never reports a model, because the machine's config is not an agent's telemetry", () => {
    withTempHome((home) => {
      ...
      const keys = identity ? Object.keys(identity).sort() : [];
      expect(keys).toEqual(["evidenceClass", "hostTool"]);
  ...
  test("recognises the claude-code, cursor and custom hosts", () => {
    withTempHome((home) => {
      ...
      vfs.writeFileSync(join(home, ".claude.json"), JSON.stringify({ currentModel: "claude-3-5" }));
  ...
  test("treats a corrupt or empty config as no identity at all", () => {
    for (const content of ["invalid-json {{{", "", "   \n\t ", JSON.stringify({ model: "  " })]) {
      withTempHome((home) => {
        const dir = join(home, ".gemini", "antigravity-cli");
        vfs.mkdirSync(dir, { recursive: true });
        vfs.writeFileSync(join(dir, "settings.json"), content);
        expect(detectHostIdentity({ homeDir: home, env: {} })).toBeNull();
      });
    }
  });
  ...
  test("a config path that is a directory, not a file, is read as absent rather than throwing", () => {
    withTempHome((home) => {
      vfs.mkdirSync(join(home, ".gemini", "antigravity-cli", "settings.json"), { recursive: true });
      expect(detectHostIdentity({ homeDir: home, env: {} })).toBeNull();
    });
  });
});
...
describe("host telemetry probing: codex", () => {
  ...
  test("codex: reads a per-agent model, provider and reasoning effort keyed by the exact agent id", () => {
    withTempHome((home) => {
      const agentsDir = join(home, ".codex", "agents");
      vfs.mkdirSync(agentsDir, { recursive: true });
      vfs.writeFileSync(join(home, ".codex", "config.toml"), "[features]\nmulti_agent = true\n");
      vfs.writeFileSync(
      ...
      expect(probe?.provider).toEqual({ value: "openai", evidence_class: "derived" });
    });
  });
  ...
  test("codex: an unrecognised reasoning effort is left absent rather than guessed at", () => {
    withTempHome((home) => {
      ...
      vfs.mkdirSync(agentsDir, { recursive: true });
      vfs.writeFileSync(join(home, ".codex", "config.toml"), "[features]\nmulti_agent = false\n");
      vfs.writeFileSync(join(agentsDir, "worker-1.toml"), 'reasoning_effort = "extreme"\n');
      expect(
  ...
  test("Probe 3: codex agent configuration with corrupted TOML leaves agent telemetry absent", () => {
    withTempHome((home) => {
      const agentsDir = join(home, ".codex", "agents");
      vfs.mkdirSync(agentsDir, { recursive: true });
      vfs.writeFileSync(join(home, ".codex", "config.toml"), "[features]\nmulti_agent = true\n");
      vfs.writeFileSync(join(agentsDir, "corrupt-agent.toml"), "[not valid toml");

      const probe = detectHostTelemetry("corrupt-agent", { homeDir: home, env: {} });
      expect(probe?.model).toBeUndefined();
      expect(probe?.thinking_level).toBeUndefined();
      expect(probe?.provider).toBeUndefined();
    });
  });
});
