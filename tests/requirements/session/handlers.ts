/**
 * @file handlers.ts
 * Handlers for stats, permissions, rw, and directory traversal in requirements virtual filesystem.
 */
...
export function isVirtualPath(p: string): boolean {
  ...
  return (
    ...
    s.includes("requirements-scratch") ||
    s.includes("virtual-enhanced-plan-run") ||
    s.includes("planning") ||
    s.includes(".olt") ||
    s.includes("scratch") ||
    s.includes("tmp")
...
export function getInode(s: VirtualRequirementsState, targetPath: string): number {
...
function getNlink(s: VirtualRequirementsState, targetPath: string): number {
  const t = norm(targetPath);
...
export function makeSymlinkStats(s: VirtualRequirementsState, targetPath: string): fs.Stats {
  const t = norm(targetPath);
  const ino = getInode(s, t);
...
export function checkReadPermission(s: VirtualRequirementsState, targetPath: string): void {
