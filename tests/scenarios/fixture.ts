/**
 * @file fixture.ts
 * In-memory virtual test sandbox fixture and harness for tests/scenarios domain.
...
export function setupVirtualScenariosFS(): VirtualMemoryFS {
  if (!currentSession) {
    ...
    currentVfs.mkdirSync("/virtual/scenarios-scratch", { recursive: true });
...
export function cleanupVirtualScenariosFS(): void {
  if (currentSession) {
...
/**
 * Creates an in-memory virtual scratch sandbox directory for scenario tests.
 * Zero physical disk writes occur.
 */
export function scratchRoot(callerPath = "scenarios-test", label = "test"): string {
  const vfs = setupVirtualScenariosFS();
  counter += 1;
  ...
  const dirName = `${fileTag}-${labelTag}-${counter}-${digest}`
    ...
    .slice(0, 50)
    .replace(/-+$/, "");
  const root = `/virtual/scenarios-scratch/${dirName}`;
  vfs.mkdirSync(root, { recursive: true });
  return root;
}

export function getVirtualScenariosFS(): VirtualMemoryFS {
  return currentVfs;
}
