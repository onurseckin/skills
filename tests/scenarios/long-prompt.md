...
Create `.harness/local-automation-20260813/` immediately. The capsule is already covered by the repository’s `.harness/` ignore rule.
...
Before implementation, inspect `git status`, instruction files, recent changes, `scripts/README.md`, current script/test conventions, and `.gitignore`. Preserve all pre-existing changes. Record results in the capsule and verify `git check-ignore -q .harness/example`.
...
- Product state: `.harness/local-automation/state.sqlite3`, with logs beneath the same directory. No authoritative state may live only in memory or PID files.
...
| Run state gitignored                  | All workflow and product state under `.harness/`                                           | `git check-ignore`; tracked-file audit                          |
...
copied `.harness/<run>/runtime/harness.ts`, never the mutable installed skill.
...
packaging conventions, and all pre-existing dirty paths. I will prove `.harness/` is ignored before
