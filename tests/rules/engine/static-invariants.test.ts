...
describe("Static Invariant Verification: Zero TypeScript any, Zero Suppressions & <= 300 LOC", () => {
  ...
  it("verifies all rules test files satisfy strict monorepo rules", () => {
    const rulesDir = join(import.meta.dir, "..");
    const files = getTsFilesRecursively(rulesDir);
  ...
  it("enforces strict directory density (<= 10 items) per folder in tests/rules/", () => {
