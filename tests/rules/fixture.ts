/**
 * @file fixture.ts
 * In-memory virtual test sandbox fixture and harness for tests/rules domain.
...
export function setupVirtualRulesFS(): VirtualMemoryFS {
  if (!currentSession) {
    ...
    currentVfs.mkdirSync("/virtual/rules-scratch", { recursive: true });
...
export function cleanupVirtualRulesFS(): void {
  if (currentSession) {
...
/**
 * Creates an in-memory virtual scratch sandbox directory for rules tests.
 * Zero physical disk writes occur.
 */
export function scratchRoot(callerPath = "rules-test", label = "test"): string {
  const vfs = setupVirtualRulesFS();
  counter += 1;
  ...
  const dirName = `${fileTag}-${labelTag}-${counter}-${digest}`
    ...
    .slice(0, 50)
    .replace(/-+$/, "");
  const root = `/virtual/rules-scratch/${dirName}`;
  vfs.mkdirSync(root, { recursive: true });
  return root;
}

export function getVirtualRulesFS(): VirtualMemoryFS {
  return currentVfs;
}
