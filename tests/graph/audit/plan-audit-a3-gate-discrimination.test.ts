...
describe("A3-gate-discrimination", () => {
  ...
  describe("consulting a recorded gate:prove verdict (DESIGN.md C3)", () => {
    ...
    test("a proof recorded as NOT falsifiable does not satisfy discrimination — refusal stands", () => {
      const runState = runStateWithProofs([
        gateProof({
          task_id: "task-d1",
          gate_argv: ["bun", "run", "typecheck"],
    ...
    test("a proof recorded for a different scope than the task currently carries does not satisfy", () => {
      const runState = runStateWithProofs([
        gateProof({
          task_id: "task-d1",
          gate_argv: ["bun", "run", "typecheck"],
          write_scope: ["src/d1", "src/d1/new-file.ts"],
        }),
        gateProof({
          task_id: "task-d2",
          gate_argv: ["bun", "run", "typecheck"],
    ...
    test("a proof recorded for a different gate argv than the task currently carries does not satisfy", () => {
      const runState = runStateWithProofs([
        gateProof({
          task_id: "task-d1",
          gate_argv: ["bun", "test", "tests/d1.test.ts"],
          write_scope: ["src/d1"],
        }),
        gateProof({
          task_id: "task-d2",
