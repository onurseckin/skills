...
describe("Mind Init Lifecycle Suite (mind-init.ts)", () => {
  ...
  beforeEach(() => {
    ...
    testDir = `/virtual/mind-init-cov-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
  ...
  describe("Validation Errors & Pre-condition Guards", () => {
    ...
    it("throws INVALID_ARGUMENT when repo root is a file rather than directory", () => {
      ...
      vfs.writeFileSync(filePath, "hello", "utf-8");
      expect(() =>
        initializeMindLifecycle({
          repo: filePath,
          charter: "charter.yaml",
    ...
    it("throws INVALID_ARGUMENT when charter file is empty or blank whitespace", () => {
      ...
      vfs.writeFileSync(emptyCharter, "   \n\t  \n", "utf-8");
      expect(() =>
        initializeMindLifecycle({
          repo: testDir,
    ...
    it("throws INVALID_STATE when capsule directory already exists", () => {
      ...
      vfs.writeFileSync(charterPath, SAMPLE_CHARTER, "utf-8");
  ...
  describe("Successful Mind Initialization Execution", () => {
    it("initializes mind lifecycle with default parameters and creates policy/capsule", () => {
      ...
      expect(vfs.existsSync(join(testDir, ".olt", "policy.json"))).toBe(true);
      expect(vfs.existsSync(join(result.run_root, "last_pulse.json"))).toBe(true);

      const loaded = loadRun(result.run_root);
      const state = loaded.state as Record<string, unknown>;
      const mindState = state.mind as { generation: number };
      const budgetState = state.budget as { pulses_per_day: number | null; quiet_hours: string };
      const pulseState = state.pulse as { counter: number };
      const agents = state.agents as Array<{ id: string; role: string }>;
      expect(mindState.generation).toBe(1);
      expect(budgetState.pulses_per_day).toBe(50);
      expect(budgetState.quiet_hours).toBe("false");
      expect(pulseState.counter).toBe(0);
    ...
    it("handles custom generation (> 1), custom mindId, actor matching mindId, and minimal charter", () => {
      ...
      vfs.writeFileSync(charterPath, MINIMAL_CHARTER, "utf-8");
      ...
      const state = loaded.state as Record<string, unknown>;
      const mindState = state.mind as { generation: number };
      const budgetState = state.budget as { pulses_per_day: number | null };
      const agents = state.agents as Array<{ id: string; role: string }>;
      expect(mindState.generation).toBe(2);
      expect(budgetState.pulses_per_day).toBeNull();
      expect(agents.some((g) => g.id === "mind-gen-2")).toBe(true);
      expect(agents.some((g) => g.role === "mind-auditor")).toBe(true);
      expect(agents.some((g) => g.role === "skill-auditor")).toBe(true);
    });
    ...
    it("adds distinct actor grant when actor !== mindId", () => {
      ...
      const state = loaded.state as Record<string, unknown>;
      const agents = state.agents as Array<{ id: string; role: string }>;
      expect(agents.some((g) => g.id === "human-operator-alice")).toBe(true);
      expect(agents.some((g) => g.id === "mind-primary")).toBe(true);
    });
  });
});
