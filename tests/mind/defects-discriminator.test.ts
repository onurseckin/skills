...
describe("mind/defects/discriminator", () => {
  describe("normalizeObservationSignature", () => {
    ...
    it("returns empty string for non-string input", () => {
      expect(normalizeObservationSignature(undefined as unknown as string)).toBe("");
      expect(normalizeObservationSignature(null as unknown as string)).toBe("");
      expect(normalizeObservationSignature(123 as unknown as string)).toBe("");
    });
  });
  ...
  describe("extractDefectKeywords", () => {
    ...
    it("returns empty array for non-string input", () => {
      expect(extractDefectKeywords(null as unknown as string)).toEqual([]);
      expect(extractDefectKeywords(123 as unknown as string)).toEqual([]);
    });
  });
