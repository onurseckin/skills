...
it("enforces L1 commitment accountability gate when unfulfilled commitments lack justification", () => {
    ...
    memory.recordCommitment({
      id: "comm-unfulfilled",
      topic: "Telemetry Pipeline",
      agreedResolution: "Add zero-drop buffer",
      targetMilestone: "M1",
      status: "pending",
      createdAt: "2026-09-01T00:00:00.000Z",
      updatedAt: "2026-09-01T00:00:00.000Z",
    ...
    // When evaluated, state should lock to L1 and inquiry should demand accountability
    const exchange = engine.evaluateCycle("cycle-10", "New Feature Proposal");

    expect(exchange.level).toBe(DIALECTICAL_LEVELS.L1_TRADE_OFF_VERIFICATION);
    expect(exchange.unfulfilledCommitmentId).toBe("comm-unfulfilled");
    expect(exchange.inquiry).toContain("Accountability Gate");
    expect(exchange.inquiry).toContain("comm-unfulfilled");
    expect(engine.getState().currentLevel).toBe(DIALECTICAL_LEVELS.L1_TRADE_OFF_VERIFICATION);

    // Submitting satisfactory response while still unfulfilled without memory update stays on L1
    const state = engine.submitResponse("cycle-10", "Addressed in discussion", {
      isSatisfactory: true,
    });
    expect(state.currentLevel).toBe(DIALECTICAL_LEVELS.L1_TRADE_OFF_VERIFICATION);

    // Once justified or fulfilled, laddering can advance
    memory.updateCommitmentStatus("comm-unfulfilled", "fulfilled", "Buffer added and tested");
    const unblockedState = engine.submitResponse("cycle-10", "Commitment fulfilled", {
      isSatisfactory: true,
    });
    expect(unblockedState.currentLevel).toBe(DIALECTICAL_LEVELS.L2_SECOND_ORDER_IMPLICATIONS);
  });
});
