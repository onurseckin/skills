...
describe("Task 1.35: Named Export 'planTasksForDefect' in mind/tasks/index.ts", () => {
  ...
  test("5. planTasksForDefect supports custom planning options", () => {
    ...
    const options: PlanTasksForDefectOptions = {
      ...
      priority: "MEDIUM",
      writeScope: ["olt/scripts/src/cli/commands/audit.ts", "tests/unit/cli/audit.test.ts"],
      gate: "bun test tests/unit/cli/audit.test.ts && bun run typecheck",
      charterGoals: ["G1", "G2"],
    ...
    const defects: readonly DefectTaskTarget[] = [
      {
        ...
        write_scope: ["olt/scripts/src/mind/core.ts", "tests/unit/mind/core.test.ts"],
        severity: "critical",
      },
      {
        id: "defect-alpha-2",
