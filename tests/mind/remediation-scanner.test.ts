...
describe("Remediation Scanner Engine", () => {
  ...
  it("transforms DefectEntry into a canonical DiscoveryItem", () => {
    ...
    expect(item.targetFiles).toEqual(["olt/scripts/src/mind/", "tests/unit/mind/"]);
    expect(item.writeScope).toEqual(["olt/scripts/src/mind/", "tests/unit/mind/"]);
  ...
  it("verifies static invariants of zero any and zero suppressions", () => {
    const filesToAudit = [
      ...
      join(process.cwd(), "tests/unit/mind/remediation-scanner.test.ts"),
    ];
