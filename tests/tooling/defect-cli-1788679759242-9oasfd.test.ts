...
describe("Defect Remediation: defect-cli-1788679759242-9oasfd", () => {
  test("exports constants and defect metadata", () => {
    ...
    expect(DEFECT_TITLE.includes("C-f87dc8e8-ef4d-4e56-927a-3a4c23c2b063")).toBe(
      true,
    );
  });
  ...
  test("flags running command intents lacking terminal evidence", () => {
    ...
    const result: TerminalEvidenceValidationResult =
      validateCommandTerminalEvidence(ctx);
    expect(result.remediated).toBe(true);
    expect(result.valid).toBe(false);
    expect(result.errorCode).toBe(ERROR_CODE);
    expect(result.missingEvidenceCommandIds).toContain(
      "C-f87dc8e8-ef4d-4e56-927a-3a4c23c2b063",
    );
