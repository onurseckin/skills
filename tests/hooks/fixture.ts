/**
 * @file fixture.ts
 * In-memory virtual test sandbox fixture and harness for tests/hooks domain.
...
export function setupVirtualHooksFS(): VirtualMemoryFS {
  if (!currentSession) {
...
export function cleanupVirtualHooksFS(): void {
  if (currentSession) {
...
/**
 * Creates an in-memory virtual scratch sandbox directory for hooks tests.
 * Zero physical disk writes occur.
 */
export function scratchRoot(callerPath = "hooks-test", label = "test"): string {
  const vfs = setupVirtualHooksFS();
  counter += 1;
  ...
  const dirName = `${fileTag}-${labelTag}-${counter}-${digest}`
    ...
    .slice(0, 50)
    .replace(/-+$/, "");
  const root = `/virtual/hooks-scratch/${dirName}`;
  vfs.mkdirSync(root, { recursive: true });
  return root;
}

export function getVirtualHooksFS(): VirtualMemoryFS {
  return currentVfs;
}
