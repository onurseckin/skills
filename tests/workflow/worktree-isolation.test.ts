...
describe("Worktree Isolation - Disjoint Write Scopes", () => {
  it("validates mutually disjoint domain write scopes as isolated", () => {
    const domains: readonly DomainScopeEntry[] = [
      { domain: "auth", writeScope: ["src/auth/**", "tests/unit/auth/**"] },
      { domain: "billing", writeScope: ["src/billing/**", "tests/unit/billing/**"] },
      { domain: "reporting", writeScope: ["src/reporting/**", "tests/unit/reporting/**"] },
    ];
...
describe("Worktree Isolation - Write Scope Path Confinement", () => {
  const allowedScope = ["src/workflow/worktree/**", "tests/unit/workflow/worktree*.ts"];
  ...
  it("matches exact, directory prefix, and glob patterns accurately", () => {
    ...
    expect(isPathInWriteScope("tests/unit/workflow/worktree-isolation.test.ts", allowedScope)).toBe(
      true,
    );
  ...
  it("filters paths strictly to assigned scope", () => {
    const paths = [
      ...
      "tests/unit/workflow/worktree.test.ts",
      "docs/readme.md",
    ];
...
describe("Worktree Isolation - Domain Worktree Lifecycle & Commits", () => {
  ...
  beforeEach(() => {
    testRoot = scratchRoot(import.meta.path, "worktree-isolation-test");
