...
describe("Task Queue Dequeue Engine & Anti-Batching Guard", () => {
  ...
  it("admitTask, renewTaskLease, releaseTaskLease, startTaskValidation lifecycle", () => {
    ...
    const sampleLease = createSampleTaskLease();
    expect(sampleLease.leaseId).toBe("lease-sample-01");
    const receipts = createSampleCompletionReceipts();
    expect(receipts.validatorVerdict).toBe("passed");
    expect(TASK_DEQUEUE_SUITES.length).toBe(3);
  });
});
