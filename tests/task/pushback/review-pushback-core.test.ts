...
describe("Strict 1:1 Individual Task Verification Scepticism", () => {
  ...
  describe("Domain Batching Rejection", () => {
    it("detects and rejects multi-domain claims sharing identical duplicate evidence", () => {
      const domains = ["code-quality", "security", "ui-design", "performance"] as const;
      const batchedEvidence = {
        "code-quality": { summary: "All checks passed", exit_code: 0 },
        security: { summary: "All checks passed", exit_code: 0 },
        "ui-design": { summary: "All checks passed", exit_code: 0 },
        performance: { summary: "All checks passed", exit_code: 0 },
      };
      ...
      expect(result.violatingDomains).toContain("performance");
  ...
  describe("Counterfactual Falsifiability Evidence", () => {
    ...
    it("rejects verification when counterfactual hypothesis and checks are empty", () => {
      const emptyEntry = [
        {
          hypothesis: "",
          negativeCheck: "",
      ...
      const result = evaluateCounterfactualEvidence(emptyEntry);
      expect(result.isSufficient).toBe(false);
      expect(result.hypothesisCount).toBe(0);
      expect(result.falsificationCheckCount).toBe(0);
    });
