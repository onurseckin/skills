...
describe("task pushback path unit tests", () => {
  ...
  it("validates pushback evidence invariants", () => {
    ...
    expect(() =>
      validatePushbackEvidence("invalid" as unknown as CoordinatorPushbackCause, "Obs", "Rem"),
  ...
  it("executes coordinator pushback for procedural cause", () => {
    ...
    expect(() =>
      executeCoordinatorPushback(port, "task-alpha", "coordinator-1", {
        validatorId: "unrecorded-val",
        domain: "code-quality",
        cause: "procedural",
        observation: "Obs",
        remediation: "Rem",
  ...
  it("contests validator verdict using contestValidatorVerdict helper", () => {
    ...
    expect(() =>
      contestValidatorVerdict(port, {
        ...options,
        cause: "invalid-cause" as unknown as CoordinatorPushbackCause,
      }),
    ).toThrow(HarnessError);
