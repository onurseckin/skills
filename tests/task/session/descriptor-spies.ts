 ...
 * Spies for open descriptors, truncation, fstat, and platform flock.
 */
...
export function createDescriptorSpies(s: VirtualTaskState): Array<{ mockRestore: () => void }> {
  const spy = <K extends keyof typeof fs>(k: K, fn: unknown) =>
    spyOn(fs, k).mockImplementation(fn as never) as unknown as Mock<
      (...args: unknown[]) => unknown
    >;

  const spawnMock = (cmd: unknown, args?: unknown, opts?: unknown) => {
    const isUtf8 =
      typeof opts === "object" &&
      opts !== null &&
      (opts as { encoding?: string }).encoding === "utf8";
    const argList = Array.isArray(args) ? args : [];
    let stdoutStr = "";
    if (cmd === "git" && argList.includes("status")) {
      stdoutStr = "A  file.txt\n";
    }
    return {
      status: 0,
      stdout: isUtf8 ? stdoutStr : Buffer.from(stdoutStr),
      stderr: isUtf8 ? "" : Buffer.alloc(0),
      pid: 12345,
      output: [null, isUtf8 ? stdoutStr : Buffer.from(stdoutStr), isUtf8 ? "" : Buffer.alloc(0)],
      signal: null,
    };
  };
  ...
  return [
    ...
    spyOn(nodeChildProcess, "spawnSync").mockImplementation(spawnMock as never) as never,
    spyOn(childProcess, "spawnSync").mockImplementation(spawnMock as never) as never,
    spyOn(nodeChildProcess, "execSync").mockReturnValue(Buffer.alloc(0)) as never,
    spyOn(childProcess, "execSync").mockReturnValue(Buffer.alloc(0)) as never,
  ];
}
