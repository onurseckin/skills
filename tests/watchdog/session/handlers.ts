/**
 * @file handlers.ts
 * Handlers for stats, permissions, rw, and directory traversal in watchdog virtual filesystem.
 */
...
function getNlink(s: VirtualWatchdogState, targetPath: string): number {
  const t = norm(targetPath);
...
export function makeSymlinkStats(s: VirtualWatchdogState, targetPath: string): fs.Stats {
  const t = norm(targetPath);
  const ino = getInode(s, t);
...
export function checkReadPermission(s: VirtualWatchdogState, targetPath: string): void {
