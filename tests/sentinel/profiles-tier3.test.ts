...
describe("Sentinel 20-Role Profiles: Tier 3", () => {
  ...
  describe("mechanic-validator profile", () => {
    test("flags source modifications", () => {
      const violations = mechanicValidatorProfile.evaluate({
        agent_id: "m_val_01",
        role: "mechanic-validator",
        modified_files: ["src/app.ts"],
      });
      expect(violations.some((v) => v.code === "MECHANIC_VALIDATOR_SOURCE_MUTATION")).toBe(true);
    });
  });
  ...
  describe("ui validator profiles", () => {
    ...
    test("ui-mechanic-validator flags source edits", () => {
      const violations = uiMechanicValidatorProfile.evaluate({
        agent_id: "ui_m_01",
        role: "ui-mechanic-validator",
        modified_files: ["src/app.ts"],
      });
      expect(violations.some((v) => v.code === "UI_MECHANIC_SOURCE_MUTATION")).toBe(true);
    });
    ...
    test("ui-validator flags source edits", () => {
      const violations = uiValidatorProfile.evaluate({
        agent_id: "ui_val_01",
        role: "ui-validator",
        modified_files: ["src/app.ts"],
      });
      expect(violations.some((v) => v.code === "UI_VALIDATOR_SOURCE_MUTATION")).toBe(true);
    });
  });
