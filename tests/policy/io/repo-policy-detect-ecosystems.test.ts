...
describe("Repo Policy Ecosystem Auto-Detection (Bun, Cargo, Python, Node, Unknown)", () => {
  ...
  test("detects Bun ecosystem when bun.lock or bun.lockb exists", () => {
    ...
    vfs.writeFileSync(join(dir, "bun.lock"), "", "utf-8");
  ...
  test("detects Bun ecosystem when bun.lockb exists", () => {
    ...
    vfs.mkdirSync(dir, { recursive: true });
    vfs.writeFileSync(join(dir, "bun.lockb"), "", "utf-8");
  ...
  test("detects Cargo ecosystem when Cargo.toml or Cargo.lock exists", () => {
    ...
    vfs.writeFileSync(join(dir2, "Cargo.lock"), "", "utf-8");
  ...
  test("detects Python ecosystem across poetry, pipenv, requirements.txt and setup.py", () => {
    ...
    vfs.mkdirSync(dirPoetry, { recursive: true });
    vfs.writeFileSync(join(dirPoetry, "pyproject.toml"), "[tool.poetry]", "utf-8");
    vfs.writeFileSync(join(dirPoetry, "poetry.lock"), "", "utf-8");
    ...
    vfs.mkdirSync(dirPipenv, { recursive: true });
    vfs.writeFileSync(join(dirPipenv, "Pipfile"), "", "utf-8");
    ...
    vfs.mkdirSync(dirReq, { recursive: true });
    vfs.writeFileSync(join(dirReq, "requirements.txt"), "pytest\n", "utf-8");
    ...
    vfs.mkdirSync(dirSetup, { recursive: true });
    vfs.writeFileSync(join(dirSetup, "setup.py"), "# setup\n", "utf-8");
  ...
  test("detects Node ecosystem across pnpm, yarn, npm, and lockfiles", () => {
    ...
    vfs.mkdirSync(dirPnpm, { recursive: true });
    vfs.writeFileSync(join(dirPnpm, "package.json"), "{}", "utf-8");
    vfs.writeFileSync(join(dirPnpm, "pnpm-lock.yaml"), "", "utf-8");
    ...
    vfs.mkdirSync(dirYarn, { recursive: true });
    vfs.writeFileSync(join(dirYarn, "yarn.lock"), "", "utf-8");
    ...
    vfs.mkdirSync(dirNpm, { recursive: true });
    vfs.writeFileSync(join(dirNpm, "package-lock.json"), "{}", "utf-8");
  ...
  test("detects unknown ecosystem when no marker files exist", () => {
    ...
    vfs.mkdirSync(dirUnknown, { recursive: true });
