...
describe("ActivityRecord", () => {
  ...
  test("output tracks stdout and stderr bytes and lastOutputAt", () => {
    const started = "2026-08-24T10:00:00.000Z";
    const record = new ActivityRecord(testDir, "C-12345", 1, started, 0);
...
describe("Gate Path Validation and Operands", () => {
  ...
  test("inside determines filesystem path containment", () => {
    ...
    expect(inside(repo, "/workspace/project/tests/engine/runner/runner.test.ts")).toBe(true);
  ...
  test("runner index and fixture exports are valid", async () => {
    const { createTestAgentMetadata, RUNNER_SUITES } = await import("./index.ts");
    const meta = createTestAgentMetadata("agent-fix", "implementer");
    expect(meta.agent_id).toBe("agent-fix");
    expect(RUNNER_SUITES.length).toBeGreaterThan(0);
  });
});
