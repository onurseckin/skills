...
describe("Scheduler Conflicts", () => {
  test("scopeConflict correctly identifies overlapping directory and glob scopes", () => {
    ...
    expect(scopeConflict(["tests/engine"], ["src/unit"])).toBe(false);
  });
...
describe("Scheduler Batch Proposal and Ready Set", () => {
  ...
  test("scheduler index and fixture exports are valid", async () => {
    const { createTestTask, createTestRunState, SCHEDULER_SUITES } = await import("./index.ts");
    const task = createTestTask("task-f-1", "ready");
    expect(task.id).toBe("task-f-1");
    const state = createTestRunState({ "task-f-1": task });
    expect(state.schema).toBe("harness.run-state");
    expect(SCHEDULER_SUITES.length).toBeGreaterThan(0);
  });
});
