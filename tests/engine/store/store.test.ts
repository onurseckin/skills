...
describe("Content Normalization", () => {
  ...
  test("canonicalizeJson, canonicalizeYaml, and whitespace normalization work reliably", () => {
    const jsonA = encoder.encode('{"b": 2, "a": 1}');
    const canonA = canonicalizeJson(jsonA);
    expect(canonA).toBeDefined();
    expect(canonicalizeJson(encoder.encode("{ invalid }"))).toBeUndefined();

    const yamlA = encoder.encode("b: 2\na: 1\n");
    expect(canonicalizeYaml(yamlA)).toBeDefined();
    expect(canonicalizeYaml(encoder.encode(":\n - :: invalid"))).toBeUndefined();

    const canonicalWs = canonicalizeEcmaScriptWhitespace("const x = 1;\n\n\n\nconst y = 2;\n");
    expect(canonicalWs).not.toContain("\n\n\n");
  });
  ...
  test("normalizeContent and contentDigest produce matching digests for equivalent JSON", () => {
    ...
    expect(contentEquals(bytes1, bytes2, "json").equal).toBe(true);
  });
});
...
describe("Capsule Transaction and Event Append", () => {
  ...
  test("store index and fixture exports are valid", async () => {
    const { createTestDefect, createTestEventPayload, STORE_SUITES } = await import("./index.ts");
    const defect = createTestDefect("dedup-1");
    expect(defect.dedup_key).toBe("dedup-1");
    const payload = createTestEventPayload("event-1");
    expect(payload.note).toBe("event-1");
    expect(STORE_SUITES.length).toBeGreaterThan(0);
  });
});
