...
describe("CLI real entry point: chat:rooms", () => {
  ...
  it("executes full argv parsing, flag assertion, and entry pipeline", async () => {
    ...
    try {
      await main(["chat:rooms", "--mine", "--as", "test-member", "--json"]);
    } finally {
    ...
    expect(parsed).toEqual({ rooms: [], count: 0 });
  });

  it("throws IDENTITY_UNRESOLVED when --mine is invoked without identity", async () => {
    let caughtCode = "";
    try {
      await main(["chat:rooms", "--mine"]);
    } catch (err: unknown) {
  ...
  it("returns all rooms without requiring identity when --mine is not provided", async () => {
    let captured = "";
    const originalWrite = process.stdout.write;
    process.stdout.write = (chunk: string | Uint8Array): boolean => {
      captured += typeof chunk === "string" ? chunk : chunk.toString();
  ...
  it("renders help markdown cleanly when invoked with --help", async () => {
    ...
    expect(captured.includes("--as")).toBe(true);
  });
