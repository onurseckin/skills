...
| #               | Defect                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| :-------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **165**         | **Proofs over untracked write scopes are vacuous.** `revertWriteScope` restores each scope file to its content at `base`; an untracked file has none, so it is _deleted_ from the scratch tree and the gate measures only the file's absence. Every `tests/unit/*.test.ts` is untracked, so this covers essentially all test-authoring tasks. _(Supersedes 112, which claimed `gate:prove` endangers the working tree — refuted: it is fully sandboxed under `mkdtempSync(tmpdir())` and `rmSync`s the scratch tree in a `finally`.)_ |
...
**Verified live, both polarities.** `scripts-filesize-r2` holds `val_strip-comments-2 → falsifiable=False, exit=0`
beside three honest proofs; `money-contracts-r2` holds `val_money-refund → falsifiable=True, exit=1` over an
untracked scope. Neither measures discrimination — one reads absence as "cannot fail", the other as
"does fail". **Any seal resting on an untracked-scope proof is suspect**, making the false-seal problem
far broader than the single task escalated at 140.
