...
import type { CommandRecord } from "../../src/contracts/commands.ts";
import { realpathSync } from "node:fs";
import { commandFingerprint } from "../../src/workflow/gates/gate-policy.ts";
import { captureGatePathBindings } from "../../src/runner/gate-path-bindings.ts";
import { captureGateEnvironment } from "../../src/runner/gate-environment.ts";
import type { TransactionPort, WorkflowState } from "../../src/workflow/types.ts";
...
export const TEST_GATE_ARGV = ["bun", "test", "tests/runner/output-evidence.test.ts"];
export const SECOND_TEST_GATE_ARGV = ["bun", "test", "tests/runner/gate-path-binding.test.ts"];
...
export function commandRecord(id: string, overrides: Partial<CommandRecord> = {}): CommandRecord {
  ...
  return {
    ...
    gate_id: gateId,
    actor,
    attempt_signing_public_key:
      "MCowBQYDK2VwAyEAebJY5kfIxE+SBdW0wwTb+c0PuvZ21w9gpaa9L86ygKc=",
