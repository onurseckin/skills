...
import { checkCompletion } from "../../src/workflow/completion/check-completion.ts";
import { recordCompletionRemediation } from "../../src/workflow/completion/record-completion-remediation.ts";
...
describe("completion critic lifecycle", () => {
  ...
  test("requires remediation and a fresh critic, preserves history, and bounds rounds", () => {
    ...
    expect(port.read().completion_remediations).toHaveLength(3);
    expect(() => beginCompletenessCritic(port, "critic-4", { clock })).toThrow();
  });
