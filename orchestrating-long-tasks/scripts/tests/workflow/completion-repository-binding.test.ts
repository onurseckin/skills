...
import { beginCompletenessCritic } from "../../src/workflow/completion/begin-completeness-critic.ts";
import { completeRun } from "../../src/workflow/completion/complete-run.ts";
import { recordCompletionReview } from "../../src/workflow/completion/record-completion-review.ts";
import { completionArtifactRequirements } from "../../src/workflow/completion/artifact-verification.ts";
...
import { repositoryBindingFromInspection } from "../../src/packets/repository-inspection.ts";
import { validateRepositoryBinding } from "../../src/workflow/completion/repository-binding.ts";
import {
  at,
  commandRecord,
  repositoryBinding,
...
const integritySha = createHash("sha256").update(canonicalJsonBytes(integrityEvidence)).digest("hex");
...
function readyPort(): TestPort {
  ...
  state.requirements[0] = { id: "R-1", status: "satisfied", disposition: "actionable", evidence: ["task:T-1"] };
  state.gates.push({ id: "G-RUN", command: TEST_GATE_ARGV, cwd: ".", scope: "run", requirement_ids: [], mandatory: true });
  state.commands["C-T"] = commandRecord("C-T", { gate_id: "G-1" });
  state.commands["C-V"] = commandRecord("C-V");
  state.commands["C-RUN"] = commandRecord("C-RUN", { argv: TEST_GATE_ARGV, task_id: null, gate_id: "G-RUN", actor: "coordinator" });
  state.commands["C-REPO"] = commandRecord("C-REPO", { argv: ["git", "status", "--short"], task_id: null, actor: "coordinator" });
...
function input(port: TestPort) {
  return {
    ...
    findings: [],
    requirement_proofs: [
      { requirement_id: "R-1", status: "satisfied" as const, evidence: [{ kind: "command" as const, reference: "C-CRITIC", observation: "verified" }] },
    ],
    residual_risks: [],
...
function criticPacket(state: ReturnType<TestPort["read"]>, token: string) {
  ...
  return buildPacket({
    ...
    agentId: "critic",
    state,
    commonInstructions: { bytes: commonBytes, sha256: createHash("sha256").update(commonBytes).digest("hex") },
...
describe("completion repository-byte binding", () => {
  test("requires the versioned repository binding shape", () => {
    expect(() => validateRepositoryBinding({ ...repositoryBinding, schema: undefined }, "binding")).toThrow("binding is invalid");
    expect(() => validateRepositoryBinding({ ...repositoryBinding, version: 2 }, "binding")).toThrow("binding is invalid");
    expect(() => validateRepositoryBinding({ ...repositoryBinding, git_identity_sha256: undefined }, "binding")).toThrow("binding is invalid");
  });
  ...
  test("binds repository identity into authorization, readiness, and critic packet metadata", () => {
    ...
    port.transact("test", "bind-repository", {}, (draft) => { draft.current_repository_binding = expected; });
    ...
    expect(completionReadinessSnapshot(authorization.state, 1, "critic").repository_binding).toEqual(expected);
  ...
  test("blocks critic packet publication after repository bytes drift", () => {
    ...
    expect(() => criticPacket(port.read(), authorization.token)).toThrow("repository bytes changed before critic packet publication");
  });
