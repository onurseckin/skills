...
import { cleanupAfterAttemptFailure } from "../../src/runner/run-attempt.ts";
    ...
    await expect(
      cleanup({
        ...
        descendants: {
          ...
          terminate: async () => { actions.push("terminate"); return []; },
          proveAbsent: async () => true,
        },
        probeProcess: () => "absent",
        terminateGroup: async () => { actions.push("signal"); return []; },
      }),
