...
} from "../../src/runner/attempt-cleanup-disposition.ts";
import {
  createCommandSigningCapability,
} from "../../src/runner/attempt-disposition-capability.ts";
import {
...
describe("cleanup uncertainty reconciliation", () => {
  ...
  test("rejects a validly signed direct transition from uncertainty to terminal proof", () => {
    ...
    const base = {
      ...
      ownership_token_sha256: ownershipTokenDigest(
  ...
  test("hash-binds cleanup disposition state and its delivered signals", async () => {
    ...
    expect(
      attemptStartedIssues(marker, "C-chain", 1, token, signer.verificationPublicKey),
    ).toEqual([]);
  ...
  test("rejects unsupported signals and terminal proof before record-pending", async () => {
    ...
    expect(() =>
      (controller as never as { markTerminalProof(r: string, p: unknown): void }).markTerminalProof("forged", {
        kind: "strong_absence",
        childSettled: true,
  ...
  test("treats terminal proof as final in the disposition controller", async () => {
    ...
    expect(() => controller.beginCleanupUncertain(["evidence publication failed"])).toThrow(/terminal|final/i);
