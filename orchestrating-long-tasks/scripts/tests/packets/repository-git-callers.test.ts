...
import { ensureHarnessIgnored } from "../../src/cli/git-ignore.ts";
import { ignoredByGit } from "../../src/reporting/doctor.ts";
import { createRepositoryGitCommand } from "../../src/packets/repository-git-command.ts";
...
describe("restricted repository Git callers", () => {
  test("routes init and doctor checks through exact restricted argv and environment", () => {
    ...
    const runRoot = join(repo, ".harness", "run");
    ...
    expect(calls.map(({ argv }) => argv)).toEqual([
      ...
      [...prefix, "-C", repo, "check-ignore", "--quiet", ".harness/probe"],
