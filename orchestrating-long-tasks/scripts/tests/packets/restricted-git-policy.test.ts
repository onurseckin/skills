...
import { canonicalCommandFingerprint } from "../../src/runner/command-id.ts";
import { captureGateEnvironment } from "../../src/runner/gate-environment.ts";
import { restrictedGateGitArgv } from "../../src/runner/restricted-git-gate.ts";
...
} from "../../src/packets/repository-git-command.ts";
...
describe("restricted Git policy", () => {
  ...
  test("executes both accepted Git gates with canonical diff restrictions", () => {
    expect(restrictedGateGitArgv(["/usr/bin/git", "diff", "--check"])).toEqual([
      "/usr/bin/git", ...restrictedPrefix, "diff", "--no-ext-diff", "--no-textconv", "--check",
    ]);
    expect(restrictedGateGitArgv(["/usr/bin/git", "diff", "--cached", "--check"])).toEqual([
      "/usr/bin/git", ...restrictedPrefix, "diff", "--no-ext-diff", "--no-textconv", "--cached", "--check",
    ]);
    expect(restrictedGateGitArgv(["Env.EXE", "Git", "diff", "--check"])).toEqual([
      "Env.EXE", "Git", ...restrictedPrefix, "diff", "--no-ext-diff", "--no-textconv", "--check",
    ]);
    expect(restrictedGateGitArgv(["COMMAND.EXE", "git.EXE", "diff", "--cached", "--check"])).toEqual([
      "COMMAND.EXE", "git.EXE", ...restrictedPrefix, "diff", "--no-ext-diff", "--no-textconv", "--cached", "--check",
    ]);
  });
