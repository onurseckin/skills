...
import { DescendantTracker, type ProcessIdentity } from "./descendant-tracker.ts";
import { addedPipeHandles, OWNERSHIP_ENV, runnerPipeHandles } from "./pipe-ownership.ts";
import type { CommandSigningCapability } from "./attempt-disposition-capability.ts";
import type { AttemptResult, BunSpawnApi, NormalizedCommandOptions, OutputSummary } from "./types.ts";
import {
...
export async function runAttempt(
  ...
  try {
    ...
    startedAt = new Date();
    activityRecord = new ActivityRecord(attemptDir, commandId, attempt, startedAt.toISOString(), options.heartbeatIntervalMs);
    ...
    attemptIntent = startAttemptIntent(attemptDir, commandId, attempt, startedAt.toISOString(), ownershipToken, (identity) => { rootIdentity = identity; }, attemptSigner);
    child = runtime.spawn({ cmd: options.argv, cwd: options.cwd, detached: true, stdout: "pipe", stderr: "pipe", stdin: "ignore", env: options.environment });
    ...
    const { allPumps, monitoring } = startAttemptPumpsAndMonitoring(options, child, stdout!, stderr!, stdoutPath, stderrPath, activity, pumps, pumpAbort, startedAt, () => lastActivity, activityRecord);
    ...
    const { rootProof, exitCode } = await settleAndTerminateAttempt(child, descendants, rootIdentity, options, outcome, processGroupSignals, recordSignal);
    const uniqueSignals = [...deliveredSignals];
    return await finalizeSuccessfulAttempt({
      allPumps, options, commandId, attempt, attemptDir, stdoutPath, stderrPath, activityPath, startedAt, outcome, exitCode, child, uniqueSignals, evidence, outputTail, activityRecord, rootProof, rootIdentity, attemptIntent,
    });
  } catch (error) {
    await handleAttemptFailure({
      error, terminalProofDurable: false, cleanupPrewriteFailed, attemptIntent, child, descendants, rootIdentity, trackerReady, activityRecord, pumps, pumpAbort, options, deliveredSignals, durableSignals, processGroupSignals, persistSignal, startedAt, commandId, attempt, attemptDir, observedExitCode, outputTail,
    });
    throw error;
  } finally {
    pumpAbort.abort();
    try { await descendants?.stop(); } catch {}
