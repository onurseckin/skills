...
export function formatExactAnchorBriefingMarkdown(
  ...
  lines.push(
    `4. **Task Submission**: Submit completed task via \`bun ./orchestrating-long-tasks/scripts/harness.ts task:submit --run <run> --task ${briefing.taskId} --agent <agent> --token <token> --summary "<summary>"\`.`,
  );
