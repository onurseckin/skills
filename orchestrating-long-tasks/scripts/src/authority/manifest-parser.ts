...
export function findSkillRoot(startDir?: string): string {
  ...
  if (moduleDir) {
    ...
    candidates.push(resolve(moduleDir, "../../../orchestrating-long-tasks"));
  }

  // User home directory fallback
  const home = process.env.HOME;
  if (home) {
    candidates.push(resolve(home, ".agents/skills/orchestrating-long-tasks"));
    candidates.push(resolve(home, "repos/skills/orchestrating-long-tasks"));
  ...
  for (const candidate of candidates) {
    ...
    for (let depth = 0; depth < 5; depth++) {
      ...
      // Check if cur/orchestrating-long-tasks has agents/ and roles/
      const sub = join(cur, "orchestrating-long-tasks");
