...
export function generateLeasesReport(runRoot: string): { matrix: LeaseMatrixRow[]; markdown: string } {
  ...
  for (const t of tasks) {
    if (t.lease) {
      matrix.push({ taskId: String(t.id), agentId: String(t.lease.agent), status: String(t.status) });
    }
  }
  ...
  for (const b of branches) {
    for (const sub of b.sub_tasks) {
      if (sub.lease) {
        matrix.push({ taskId: String(sub.id), agentId: String(sub.lease.agent), status: "open (sub_task)" });
      }
    }
  }
...
export function generateDecisionsReport(runRoot: string): { decisions: DecisionAuditRow[]; markdown: string } {
  const loaded = loadRun(runRoot);
  const state = loaded.state as unknown as WorkflowState;
  ...
  const lines = [
    ...
    ...decisions.map(d => `  - \`${d.requirementId}\`: ${d.decision.toUpperCase()} by \`${d.actor}\` (${d.rationale})`)
  ];
