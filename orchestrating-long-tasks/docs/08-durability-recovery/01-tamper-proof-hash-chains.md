...
`orchestrating-long-tasks` uses an **Append-Only Event Sourcing** architecture. Every state mutation across the entire lifecycle (`plan:init`, `plan:add`, `plan:compile`, `queue:pop`, `task:claim`, `task:heartbeat`, `task:submit`, `task:validate-start`, `task:review`, `task:reject`, `run:exec`, `critic:start`, `critic:review`, `run:complete`) is appended as an immutable, discrete record in `.capsules/<run-id>/events.jsonl`.

```text
┌─────────────────────────────────────────────────────────────────┐
│                          EVENT LINE 1                           │
│  sequence: 1, kind: "plan_initialized", hash: "41512719..."     │
└────────────────────────────────┬────────────────────────────────┘
                                 │
                                 ▼ (previous_hash = "41512719...")
┌─────────────────────────────────────────────────────────────────┐
│                          EVENT LINE 2                           │
│  sequence: 2, kind: "task_registered",  hash: "bd5aaed2..."     │
└────────────────────────────────┬────────────────────────────────┘
                                 │
                                 ▼ (previous_hash = "bd5aaed2...")
┌─────────────────────────────────────────────────────────────────┐
│                          EVENT LINE 3                           │
│  sequence: 3, kind: "task_claimed",     hash: "1cb2ca94..."     │
└─────────────────────────────────────────────────────────────────┘
```
...
- `seq`: Strictly ascending integer $i = 1, 2, 3 \dots$
- `previous_event_sha256`: The exact SHA-256 digest of $E_{i-1}$ (or `null` for $E_1$)
- `event_sha256`: SHA-256 digest of $E_i$ (calculated across canonical JSON encoding of all fields excluding `event_sha256`)
...
$$E_i.\text{previous\_event\_sha256} = H_{i-1}$$
