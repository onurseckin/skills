...
3. **Disjoint Write Scope Invariant:**
   - An implementer must strictly confine filesystem modifications to the leased `write_scope` (`task.write_scope`).
...
12. **Supervisor Zero Direct Code Edits / Zero Main-Thread Implementation & Role Boundary Watchdog:**
    - **Supervisor Zero Direct Code Edits & Zero Main-Thread Implementation:** Supervisors (Tier 0 `mind`, Tier 1 `orchestrator`, Tier 2 `coordinator`, Tier 2 `meta-auditor`) and the interactive main thread must NEVER directly modify, edit, or create repository source files in their own thread or take on worker tasks. Explicitly: 0 code edits, 0 unit test runs, 0 PR reviews on the main interactive thread. All code edits must be delegated to leased Tier 3 Implementers.
...
34. **Supervisor Zero Direct Code Edits & Zero Main-Thread Implementation:**
    - Strict prohibition against main thread or supervisory tiers doing code edits or taking on worker tasks (0 code edits, 0 unit test runs, 0 PR reviews on the main interactive thread). All code modifications must be delegated to leased Tier 3 Implementers with exact-anchor briefings and disjoint write scopes.
...
1. **Main-Thread Restraint Guard & Supervisor Zero Direct Code Edits / Zero Main-Thread Implementation:**
   - Interactive main-thread sessions and supervisory tiers (Mind, Orchestrators, Coordinators, Meta-Auditors) must never perform direct task implementations, source code edits, or take on worker tasks in their own thread. Explicitly: 0 code edits, 0 unit test runs, 0 PR reviews on the main interactive thread.
