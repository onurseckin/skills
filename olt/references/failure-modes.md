...
## Autonomous Forensics & Hardened Execution Failure Modes

- `FH-1` — Observed loophole: Cognitive Validator Lockout during Gate Evidence Verification (`role validator may not invoke`). A Cognitive Validator (`can_execute_shell: false`, 0 commands) required to approve a task with a mandatory gate attempts to execute the gate command directly via `run:exec`, hitting mechanical RBAC lockout, or cannot pass without gate proof.
  - Structural countermeasure: Detached Evidence Resolution (`resolveCheckIds` & `gateProofCommand`). Cognitive Validators never execute shell commands. During `task:review`, validators cite the implementer's recorded gate command ID via `--evidence <command-id>`. When `--evidence` is omitted, the harness automatically discovers and falls back to successful implementer gate command records (`task_id` match, `exit_code: 0`, and gate matching) in capsule state without requiring validator shell invocation.
- `FH-2` — Observed loophole: Completeness Critic Authentication Token Invalidation (`completeness critic authentication is invalid`). A Completeness Critic dispatched across context boundaries loses or mangles its ephemeral lease token, failing completion review sign-off.
  - Structural countermeasure: Critic Token Auto-Hydration & Packet Ledger Binding. `critic:review` and `critic:reject` resolve the authoritative critic assignment and published role packet directly from capsule state (`state.completion_critic` and `state.packets`), binding repository evidence commands and cryptographic token digests automatically.
- `FH-3` — Observed loophole: Implementer Submission Command Ownership Failure on Actor Variance (`cannot determine checks for <task>`). An implementer executes required test commands through a host runner or background task with an actor alias or slightly mismatched subagent ID, causing `task:submit` to fail checks resolution.
  - Structural countermeasure: Actor Variance Fallback in `resolveChecks`. Command resolution prioritizes exact `(task_id, actor)` matches, but on actor variance safely falls back to any successful command (`task_id` match, `exit_code: 0`) sorted deterministically by command ID.
- `FH-4` — Observed loophole: Gate-Free and Fast-Track Task Submission Failure. Tasks declaring documentation, planning, or no-op gates (empty string, null, empty array/object) fail submission with `cannot determine checks for <task>`.
  - Structural countermeasure: Gate-Free Auto-Bypass & `--no-op` / `--reason` Flags. `resolveChecks` treats empty gates (null, empty strings, whitespace, empty collections) as valid zero-command checks with `agent_reported` evidence class. For tasks legitimately requiring no write-scope changes or checks, submitting with `--no-op` and `--reason "<why>"` cleanly resolves empty checks and empty files without throwing errors.
- `FH-5` — Observed loophole: CLI Flag Syntax Drift and Alias Rejection (`unknown option: --capsule/--agent`). Autonomous agents using standard mental models invoke `--capsule` instead of `--run` or `--agent` instead of `--actor`, triggering `unknown option` errors and interrupting execution loops.
  - Structural countermeasure: Universal CLI Flag Aliasing (`execute.ts`). The CLI execution layer normalizes `--capsule` <-> `--run` / `--run-id` and `--agent` / `--agent-id` <-> `--actor` across all commands accepting either family, derives `queue-path` from `--run` for queue commands, and supports `task:list --run <capsule-path>`.
- `FH-6` — Observed loophole: Host Source Reverse-Engineering Attempt (`skills/olt/scripts/src`). An agent experiencing a CLI failure attempts to inspect or edit internal harness source files (`skills/olt/scripts/src/**`) rather than resolving the contract through documented CLI options and manifests.
  - Structural countermeasure: Autonomous Transcript Forensics Scanner (`reporting/transcript-forensics/`). Scans agent session transcripts for pattern hits against internal script paths and boundary lockout signatures, clusters occurrences across conversations, and records keyed defects into `.olt/defects.jsonl` under flock lock.
- `FH-7` — Observed loophole: Harness Contract Execution Error & Uncaptured Exit Drops. Subagent crashes or unclassified harness contract violations drop exit codes without persistent diagnostic logging.
  - Structural countermeasure: Non-Blocking Flock-Locked Defect Logging. CLI error handlers and diagnostic scanners ingest unexpected exceptions and boundary breaches directly into `.olt/defects.jsonl` without blocking parent thread execution.
