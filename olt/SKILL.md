...
16. Mandatory 3-Minute Supervisory Scheduler & Algorithmic DAG Optimization: Every long task, multi-phase execution, or autonomous mind loop MUST enforce a recurring 3-minute supervisory scheduler (`schedule` cron `*/3 * * * *`, systemd timer, or floor loop) and provide live ASCII DAG introspection (`dag`) with algorithmic parallelization recommendations to eliminate serial bottlenecks without main-thread or coordinator direct code editing.
