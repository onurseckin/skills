...
export const SERVER_COMMANDS: readonly CommandSpec[] = [
  {
    ...
    domain: "diagnostics",
    summary:
      "Inspect dev server port occupancy, socket bindings, bound processes, and Docker container conflicts.",
    description:
    ...
    flags: [
      ...
      optionalFlag(
        "all",
        "bool",
        "Scan all standard dev server ports (3000, 3001, 4000, 5000, 5173, 8000, 8080, 8888, 9000, 9229).",
      ),
  ...
  },
  {
    ...
    domain: "run",
    summary:
      "Atomically restart a dev server instance with state snapshot preservation and automatic rollback.",
    description:
    ...
    flags: [
      ...
      optionalFlag(
        "timeout",
        "int",
        "Atomic restart lock timeout in milliseconds (default: 10000).",
      ),
      optionalFlag("format", "string", "Output format: 'json' or 'markdown' (default)."),
    ],
    readsStdin: false,
  ...
  },
  {
    ...
    domain: "run",
    summary:
      "Safely reclaim occupied dev ports and terminate zombie or orphaned runtime processes.",
    description:
    ...
    flags: [
      ...
      optionalFlag(
        "force",
        "bool",
        "Immediately send SIGKILL to terminate processes without grace period.",
      ),
      optionalFlag(
        "dry-run",
        "bool",
        "Simulate process reclamation without sending OS termination signals.",
      ),
      optionalFlag(
        "zombies-only",
        "bool",
        "Only terminate detected zombie and orphaned runtime processes.",
      ),
      optionalFlag(
        "grace-period-ms",
        "int",
        "Grace period in milliseconds for SIGTERM before SIGKILL (default: 1000).",
      ),
      optionalFlag("format", "string", "Output format: 'json' or 'markdown' (default)."),
    ],
    readsStdin: false,
