...
import { resolveCapsulesDir, resolveCompletedDefectsPath } from "../../../core/index.ts";
...
import { handleDefectPromotion } from "./promotion.ts";
import { computeDefectAuditSummary } from "./summary.ts";
import { handleGenerateTests } from "./test-gen.ts";
import type {
  ApcaBadgeInfo,
...
export function defectAuditCommand(
  ...
  const { promotedCount, promotedDefects } = handleDefectPromotion(
    allDefects,
    promoteFlag,
    autoPromote,
    completedFileFlag,
  ...
  if (generateTests) {
    const testGen = handleGenerateTests(allDefects, outputTests, dryRun);
    generatedTestsList = testGen.generatedTestsList;
    generatedTestSuite = testGen.generatedTestSuite;
  }

  const summary = computeDefectAuditSummary(allDefects);
