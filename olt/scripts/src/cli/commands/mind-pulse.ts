...
export async function mindPulseCommand(
  ...
  if (!grant) {
    ...
    throw new HarnessError("INVALID_STATE", `agent ${actor} holds role '${grant.role}'; role 'mind' is required for pulse operations`);
  }
  ...
  const pulsesPerDay =
    typeof budgetRecord.pulses_per_day === "number" ? budgetRecord.pulses_per_day : DEFAULT_MIND_BUDGET.pulses_per_day;
  const wallClockPerDay =
    typeof budgetRecord.wall_clock_ms_per_day === "number" ? budgetRecord.wall_clock_ms_per_day : DEFAULT_MIND_BUDGET.wall_clock_ms_per_day;

  if (openPulse !== null && openPulse !== undefined && typeof openPulse === "object") {
    return handleOpenPulseTelemetry({
      run, actor, host, driver, arm, nowMs, state, openPulse, pulseState,
      budgetRecord, baseIntervalMs, pulsesPerDay, wallClockPerDay,
  ...
  const charterSourceRel =
    typeof charterRecord.source_path === "string" ? charterRecord.source_path : "olt/agents/mind.yaml";
  const charterRepoRoots = Array.isArray(charterRecord.repo_roots)
    ? charterRecord.repo_roots.filter((r): r is string => typeof r === "string") : undefined;
  ...
  const pulseDeadlineMs =
    typeof budgetRecord.pulse_deadline_ms === "number" ? budgetRecord.pulse_deadline_ms : DEFAULT_MIND_BUDGET.pulse_deadline_ms;
  ...
  transact(run, actor, "mind-pulse-opened", {
    pulse_id: pulseId, opened_at: openedAt, deadline_at: deadlineAt,
    host, driver, cadence: "infinite_autonomous", closing_permitted: false,
    ...
    workingPulse.open = {
      pulse_id: pulseId, opened_at: openedAt, deadline_at: deadlineAt,
      actor, host, driver, cadence: "infinite_autonomous", closing_permitted: false,
      invariant: CLOSING_FORBIDDEN_FOR_MIND,
    };
    working.pulse = workingPulse as unknown as JsonObject;
  });

  writeLastPulse(run, { at: openedAt, pulse_id: pulseId, outcome: "active", next_wake_at: nextWakeAt });

  const personaReminder = constructSupervisoryPersonaReminder({
    role: "mind", agentId: actor, runId: run, pulseId, tickNumber: nextCounter,
    cadenceMs: scheduledIntervalMs, now: nowMs,
  ...
  try {
    diagResult = await runScriptBackedDiagnostics({
      runRoot: run, repoRoot, state, clock: { now: () => new Date(nowMs) },
    });
  } catch {}
  ...
  const markdown = formatMindPulseOpenedBrief({
    ...
    scheduledIntervalMs, nextWakeAt, pulsesToday: updatedPulsesToday,
    pulsesPerDay, personaReminder,
    ...
    dagBadges, activeRuns: cognitiveTelemetry.activeAgents?.length ?? 0,
    pendingBacklog:
  ...
  return {
    markdown, run_root: run, pulse_id: pulseId, status: "opened", action: "opened",
    actor, host, driver, opened_at: openedAt, deadline_at: deadlineAt,
    scheduled_interval_ms: scheduledIntervalMs, next_wake_at: nextWakeAt,
    cadence: "infinite_autonomous", closing_permitted: false,
    invariant: CLOSING_FORBIDDEN_FOR_MIND, persona_reminder: personaReminder,
    ...
    dag_badges: dagBadges, diagnostics: diagResult,
    budget: {
