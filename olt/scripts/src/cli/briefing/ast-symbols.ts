...
export interface TriviaInfo {
  readonly enclosingStartLine: number;
  readonly declarationStartLine: number;
  readonly docstring?: string | undefined;
}

export function extractNodeTriviaInfo(node: ts.Node, sourceFile: ts.SourceFile): TriviaInfo {
  const fullText = sourceFile.getFullText();
  const declarationPos = node.getStart(sourceFile);
  const declarationStartLine = sourceFile.getLineAndCharacterOfPosition(declarationPos).line + 1;
  ...
  if (ranges !== undefined && ranges.length > 0) {
    ...
    const enclosingStartLine =
      firstComment !== undefined
        ? sourceFile.getLineAndCharacterOfPosition(firstComment.pos).line + 1
  ...
  return {
    enclosingStartLine: declarationStartLine,
    declarationStartLine,
  };
}
