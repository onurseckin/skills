/**
 * Broad Test Concurrency Locking.
 * Enforces single-agent mutual exclusion for broad/full-suite test runs with zero-disk in-memory fallback.
 */
...
let activeLockStore: LockStore = diskLockStore;

export function setLockStore(store: LockStore): void {
  activeLockStore = store;
}

export function resetLockStore(): void {
  activeLockStore = diskLockStore;
}

export function getActiveLockStore(): LockStore {
  return activeLockStore;
}

export interface TestLockOptions {
  readonly store?: LockStore | undefined;
  readonly inMemory?: boolean | undefined;
  readonly lockDir?: string | undefined;
  readonly lockFile?: string | undefined;
  readonly skipSignalHandlers?: boolean | undefined;
}

const DEFAULT_BROAD_LOCK_DIR = ".olt/.locks";
const DEFAULT_BROAD_LOCK_FILE = join(DEFAULT_BROAD_LOCK_DIR, "broad-test.lock");
...
export function acquireTestLock(
  ...
  const store = options?.inMemory
    ...
    : (options?.store ?? activeLockStore);

  const lockFile =
    options?.lockFile ??
    (options?.lockDir ? join(options.lockDir, "broad-test.lock") : DEFAULT_BROAD_LOCK_FILE);
  const lockDir =
    options?.lockDir ?? (options?.lockFile ? dirname(options.lockFile) : DEFAULT_BROAD_LOCK_DIR);
