...
export function createMemoryFSFixture(snapshot?: VirtualFSSnapshot): VirtualMemoryFS {
  const fsInstance = new VirtualMemoryFS();
  if (snapshot) fsInstance.loadSnapshot(snapshot);
  return fsInstance;
}
