...
import { copyDirRecursive } from "./handlers-io.ts";
import { isVirtualPath, normPath, type VirtualFSSpyState } from "./handlers-base.ts";
...
export function mockRead(
  ...
  const data =
    entryWithCache.cachedData ??
    (entryWithCache.cachedData = Buffer.from(state.vfs.readFileSync(entry.path)));
  ...
  data.subarray(pos, pos + readLen).copy(targetBuf, offset, 0, readLen);
