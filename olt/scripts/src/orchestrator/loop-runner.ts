...
import { checkPreCompletionDiagnostics } from "../reporting/doctor/pre-completion.ts";
import type {
    ...
    try {
      for (let round = 1; round <= this.maxRoundsConfigured; round++) {
        ...
        if (
          ...
          const preCompletion = checkPreCompletionDiagnostics({
            runRoot: roundCapsulePath,
            repoRoot: this.repoPath,
            autoHeal: true,
          });
          if (preCompletion.readyForCompletion) {
      ...
      if (this.worktreeIsolation && effectiveTrackId) {
        if (finalStatus === "converged_success") {
          try {
            landTrackToMain({ trackId: effectiveTrackId, repoRoot: this.repoPath });
          } catch {}
        } else {
          try {
            destroyTrackWorktree({
