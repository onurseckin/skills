...
export function categorizeDefect(
  ...
  const rawId = String(item["id"] || "")
    .toLowerCase()
    .replace(/[_-]/g, " ");
  const rawType = String(item["type"] || item["error_code"] || "")
    .toLowerCase()
    .replace(/[_-]/g, " ");
  const rawObs = String(item["observation"] || item["description"] || item["message"] || "")
    .toLowerCase()
    .replace(/[_-]/g, " ");
  const rawRem = String(item["remediation"] || item["prescribed_remediation"] || "")
    .toLowerCase()
    .replace(/[_-]/g, " ");
...
export function aggregateDefectEntries(
  ...
  opts: { readonly maxOccurrences?: number | undefined } = {},
): AggregatedDefect {
