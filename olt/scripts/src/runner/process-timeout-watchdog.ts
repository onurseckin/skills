import type { BunSubprocess } from "./types.ts";
import { DEFAULT_TEST_WALL_TIMEOUT_MS, DEFAULT_TEST_IDLE_TIMEOUT_MS } from "./policy.ts";
...
export class ProcessTimeoutWatchdog {
  ...
  public async enforceSigkill(
    ...
    // Step 1: Send SIGTERM first if graceMs > 0
    if (grace > 0) {
      try {
    ...
    // Step 2: Escalate with SIGKILL enforcement
    try {
