...
import { cleanupFailedAttempt } from "./attempt-cleanup";
import { createAttemptExecutionError } from "./attempt-failure-evidence";
import { probeAttemptProcess } from "./attempt-intent";
import { OutputBudget } from "../receipt/output-budget";
import { pumpOutput } from "../receipt/output-pump";
import { portableArtifactPath } from "../core/artifact-paths";
import { terminateProcessGroup } from "../process/process-group";
import { monitorProcess, type WatchdogOutcome } from "../telemetry/watchdog";
import type { AttemptIntentController } from "./attempt-intent";
...
import type { BunSpawnApi, NormalizedCommandOptions, OutputSummary } from "../types/types";
...
export async function settleAndTerminateAttempt(
  ...
  const needsRootProof = outcome.timeout || outcome.interrupted;
  const probeRoot = (): boolean => {
    if (!rootIdentity) return false;
    try {
      return probeAttemptProcess(rootIdentity) === "absent";
    } catch {
      return false;
    }
  };
  ...
  if (!descendantsAbsent || (needsRootProof && !rootProof)) {
