import type { ActivityRecord } from "../reconciliation/activity-record";
import {
  probeAttemptProcess,
...
export async function cleanupFailedAttempt(
  ...
  const durableSignals: NodeJS.Signals[] = [
    ...(options.signalsRecorded ?? options.signalsSent ?? []),
  ];
  ...
  const persistUndurable = (): void => {
    for (const signal of signals) persist(signal);
  };
  ...
  try {
    await options.descendants?.stop();
  } catch (error) {
    issue(String(error));
  }
  const rootIdentity =
    options.rootIdentity ??
    (await options.trackerReady?.catch((error) => {
      issue(String(error));
      return undefined;
    }));
  if (options.child) {
    if (!rootIdentity) {
      ...
      issue(
        `termination withheld because strong root identity was unavailable; residual pid ${options.child.pid} requires inspection`,
      );
    } else {
      for (let resume = 0; resume < 3; resume += 1) {
        signalPersistenceFailed = false;
        try {
          await (options.terminateGroup ?? terminateProcessGroup)(
            options.child.pid,
            options.graceMs,
      ...
      try {
        rootProof = (options.probeProcess ?? probeAttemptProcess)(rootIdentity);
      } catch (error) {
        issue(String(error));
      }
      if (!rootSettled || rootProof !== "absent") {
        issue(
          `root process absence was not proven for pid ${options.child.pid}: exited=${rootSettled}, identity=${rootProof}`,
        );
      }
    }
  }
  if (
    options.descendants &&
    !issues.some((value) => value.includes("ledger could not be persisted"))
  ) {
    try {
      for (let resume = 0; resume < 3; resume += 1) {
        signalPersistenceFailed = false;
  ...
  try {
    options.activityRecord?.complete("failed");
  } catch (error) {
    issue(String(error));
  }
  const terminalProof =
    rootIdentity && rootSettled && rootProof === "absent" && descendantsAbsent
