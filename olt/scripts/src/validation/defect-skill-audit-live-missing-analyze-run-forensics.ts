...
import type { DefectEntry, DefectSeverity } from "../mind/contracts/index.ts";
...
export const CANONICAL_SKILL_AUDIT_MODULE_PATH =
...
export const REQUIRED_SKILL_AUDIT_FORENSICS_SYMBOLS: readonly string[] = Object.freeze([
  "analyzeRunForensics",
  "SkillAuditorEngine",
  "SKILL_AUDIT_FORENSICS_CATEGORIES",
]);
...
export function analyzeRunForensics(
  events: readonly RunForensicEvent[] = [],
): RunForensicAnalysisResult {
  ...
  let errorCount = 0,
    tokenBurns = 0,
    falseSerializations = 0,
    roleDeviations = 0;

  for (let i = 0; i < safeEvents.length; i++) {
    const evt = safeEvents[i];
    if (!evt) continue;
    const pTok = evt.tokens?.prompt ?? 0,
      cTok = evt.tokens?.completion ?? 0;
    ...
    if (evt.error || evt.type === "ERROR" || evt.kind === "ERROR") {
      ...
      findings.push({
        ...
        description: evt.error ?? "Unhandled error",
        agentId: evt.agentId,
        timestamp: ts,
      });
    }
    if (
      evt.category === "TOKEN_BURNING" ||
      evt.kind === "TOKEN_BURNING" ||
      (totTok > 50000 && pTok > 45000)
    ) {
      tokenBurns++;
      findings.push({
        id: `TB-${i + 1}`,
        ...
        severity: "high",
        title: `Token burning at ${i}`,
        description: `${totTok} tokens (${pTok} prompt)`,
        agentId: evt.agentId,
        timestamp: ts,
      });
    }
    if (evt.category === "FALSE_SERIALIZATION" || evt.kind === "FALSE_SERIALIZATION") {
      ...
      findings.push({
        ...
        severity: "medium",
        title: `False serialization at ${i}`,
        description: "Sequential execution without parallelization",
        agentId: evt.agentId,
        timestamp: ts,
      });
    }
    if (evt.category === "ROLE_BOUNDARY_DEVIATION" || evt.kind === "ROLE_BOUNDARY_DEVIATION") {
      ...
      findings.push({
        ...
        severity: "high",
        title: `Role boundary deviation at ${i}`,
        description: `Agent '${evt.agentId ?? "unknown"}' deviated: ${evt.tool ?? "unauthorized action"}`,
        agentId: evt.agentId,
        timestamp: ts,
      });
    }
  ...
  return {
    ...
    metrics: {
      ...
      errorCount,
      tokenBurnIncidents: tokenBurns,
      falseSerializationIncidents: falseSerializations,
      roleBoundaryIncidents: roleDeviations,
    },
...
export function validateSkillAuditLiveForensicsSymbols(
  sourceCodeOrFilePath?: string,
): SkillAuditLiveForensicsValidationResult {
  let content = "",
  ...
  if (!sourceCodeOrFilePath) {
    ...
    if (!existsSync(targetPath))
      return {
        ...
        valid: false,
        hasAnalyzeRunForensics: false,
        exportedSymbols: [],
        missingSymbols: [...REQUIRED_SKILL_AUDIT_FORENSICS_SYMBOLS],
        issues: [`Skill audit live module does not exist at ${targetPath}`],
        targetPath,
      };
  ...
  const issues = missing.map(
    (sym) =>
      `Required symbol '${sym}' is not declared, imported, or exported in skill audit live module.`,
  );
  return {
    defectRef: DEFECT_REF,
    valid: missing.length === 0 && issues.length === 0,
    hasAnalyzeRunForensics: declared.includes("analyzeRunForensics"),
...
export function assertSkillAuditLiveForensicsPurity(sourceCodeOrFilePath?: string): void {
  ...
  if (!result.valid) {
    throw new SkillAuditLiveSymbolError(
      `Skill audit live forensics symbol purity assertion failed: ${result.issues.join("; ")}`,
      {
        code: UNDEFINED_FUNCTION_REFERENCE,
        defectRef: DEFECT_REF,
        symbolName: result.missingSymbols[0] ?? "analyzeRunForensics",
...
export function auditSkillAuditLiveModuleTree(
  filePathOrTarget?: string | readonly string[],
): SkillAuditLiveModuleTreeAuditResult {
  const targetFiles: string[] = Array.isArray(filePathOrTarget)
    ? [...filePathOrTarget]
    : typeof filePathOrTarget === "string"
      ? [resolve(filePathOrTarget)]
      : [resolve(process.cwd(), CANONICAL_SKILL_AUDIT_MODULE_PATH)];
  ...
  for (const fp of targetFiles) {
    ...
    fileResults.push({
      filePath: fp,
      valid: val.valid,
      hasAnalyzeRunForensics: val.hasAnalyzeRunForensics,
      issues: val.issues,
    });
  }
  ...
  return {
    defectRef: DEFECT_REF,
    errorCode: UNDEFINED_FUNCTION_REFERENCE,
    resolved: invalidCount === 0 && sampleResult.clean,
    totalFiles: targetFiles.length,
    validFiles: validCount,
    invalidFiles: invalidCount,
    files: fileResults,
    verifiedSymbols: REQUIRED_SKILL_AUDIT_FORENSICS_SYMBOLS,
    sampleForensicsAnalysis: sampleResult,
    issues: allIssues,
    timestamp: new Date().toISOString(),
  };
}
...
export function createSkillAuditLiveDefectEntry(
  options: CreateSkillAuditLiveDefectEntryOptions = {},
): DefectEntry {
  ...
  return {
    ...
    observation:
      options.observation ??
      `SkillAuditorEngine calls '${missingSymbol}' which must be resolvable in ${filePath}.`,
    remediation:
      options.remediation ??
      `Import '${missingSymbol}' from '../meta/index.ts' and ensure forensics evaluation engine is intact.`,
    context: {
      file: filePath,
      function: missingSymbol,
      defectReference: DEFECT_REF,
      mechanism: "symbol-resolution",
      command: "skill:audit:live",
      ...options.context,
    },
