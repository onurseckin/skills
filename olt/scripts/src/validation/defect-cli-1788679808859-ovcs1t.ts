export const DEFECT_ID = "defect-cli-1788679808859-ovcs1t";
export const ERROR_CODE = "INVALID_STATE";
export const DEFECT_TITLE =
  "Defect Remediation: cannot pass task-4: no recorded falsifiable gate:prove proof for gate-4 (`bun test tests/cli/defect-routing.test.ts`); run `gate:prove --run <run> --task task-4 --actor <actor>` and record a falsifiable proof against this attempt's claimed base before this review can pass";
...
export function verifyDefectRemediation(
  ...
  const ctx = context ?? {
    ...
    gateCommand: "bun test tests/validation/defect-cli-1788679808859-ovcs1t.test.ts",
    isUiTask: false,
