export const DEFECT_ID = "defect-cli-1788679915509-af2uj1";
export const ERROR_CODE = "INVALID_STATE";
export const DEFECT_TITLE =
  "Defect Remediation: cannot pass task-3: no recorded falsifiable gate:prove proof for gate-3 (`bun test tests/reporting/split-channel-router.test.ts`); run `gate:prove --run <run> --task task-3 --actor <actor>` and record a falsifiable proof against this attempt's claimed base before this review can pass";
...
export function verifyDefectRemediation(
  ...
  const ctx = context ?? {
    ...
    gateCommand: "bun test tests/validation/defect-cli-1788679915509-af2uj1.test.ts",
    isUiTask: false,
