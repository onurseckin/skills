export const DEFECT_ID = "defect-cli-1788679874180-15mfol";
export const ERROR_CODE = "INVALID_STATE";
export const DEFECT_TITLE =
  "Defect Remediation: cannot pass task-1: no recorded falsifiable gate:prove proof for gate-1 (`bun test tests/core/shared/policy-routing.test.ts`); run `gate:prove --run <run> --task task-1 --actor <actor>` and record a falsifiable proof against this attempt's claimed base before this review can pass";
...
export function verifyDefectRemediation(
  ...
  const ctx = context ?? {
    ...
    gateCommand: "bun test tests/validation/defect-cli-1788679874180-15mfol.test.ts",
    isUiTask: false,
