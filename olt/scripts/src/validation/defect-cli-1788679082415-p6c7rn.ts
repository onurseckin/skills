export const DEFECT_ID = "defect-cli-1788679082415-p6c7rn";
export const ERROR_CODE = "AUTHENTICATION_FAILURE";
export const DEFECT_TITLE =
  "Defect Remediation: --role is required to run 'task:claim' but no verified caller identity is available; refusing to auto-fill it from an unauthenticated source (mechanisms: interactive_terminal_fallback).";
...
export function verifyDefectRemediation(
  ...
  const ctx = context ?? {
    ...
    gateCommand: "bun test tests/validation/defect-cli-1788679082415-p6c7rn.test.ts",
    isUiTask: false,
