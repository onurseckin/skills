export const DEFECT_ID = "defect-cli-1788677072532-e1vd9c";
export const ERROR_CODE = "INVALID_STATE";
export const DEFECT_TITLE =
  "Defect Remediation: cannot pass task-7: no recorded falsifiable gate:prove proof for gate-7 (`bun test tests/scripts/sync`); run `gate:prove --run <run> --task task-7 --actor <actor>` and record a falsifiable proof against this attempt's claimed base before this review can pass";
...
export function verifyDefectRemediation(
  ...
  const ctx = context ?? {
    ...
    gateCommand: "bun test tests/validation/defect-cli-1788677072532-e1vd9c.test.ts",
    isUiTask: false,
