export const DEFECT_ID = "defect-cli-1788677296992-clt1yq";
export const ERROR_CODE = "INVALID_STATE";
export const DEFECT_TITLE =
  "Defect Remediation: run is not ready for completeness critic: requirement req-1 has no evidence; requirement req-1 is not satisfied; requirement req-4 has no evidence; requirement req-4 is not satisfied; run gate gate-run-completion lacks an authoritative passing command; task task-1 is validated, not done; task task-1 lacks authoritative gate gate-1; task task-1 lacks code-quality validator command evidence; task task-2 has invalid validator command C-e9b718dc-6f9d-4f3c-8114-654b2ac4ece5; task task-2 lacks authoritative gate gate-2; task task-3 lacks code-quality validator command evidence; task task-4 is validated, not done; task task-4 lacks authoritative gate gate-4; task task-4 lacks code-quality validator command evidence";
...
export function verifyDefectRemediation(
  ...
  const ctx = context ?? {
    ...
    gateCommand: "bun test tests/validation/defect-cli-1788677296992-clt1yq.test.ts",
    isUiTask: false,
