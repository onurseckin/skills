export const DEFECT_ID = "defect-cli-1788679824873-n1313l";
export const ERROR_CODE = "INVALID_STATE";
export const DEFECT_TITLE =
  "Defect Remediation: cannot pass task-2: no recorded falsifiable gate:prove proof for gate-2 (`bun test tests/core/shared/paths-policy.test.ts`); run `gate:prove --run <run> --task task-2 --actor <actor>` and record a falsifiable proof against this attempt's claimed base before this review can pass";
...
export function verifyDefectRemediation(
  ...
  const ctx = context ?? {
    ...
    gateCommand: "bun test tests/validation/defect-cli-1788679824873-n1313l.test.ts",
    isUiTask: false,
