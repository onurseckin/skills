export const DEFECT_ID = "defect-cli-1788679500002-auditorblind";
export const ERROR_CODE = "HOST_TRANSCRIPT_BLINDNESS";
export const DEFECT_TITLE =
  "Defect Remediation: SkillAuditorEngine scans only capsule events.jsonl where caller flag spoofing (--agent implementer-1) masks single-threaded serial execution. It fails to audit host conversation transcripts for invoke_subagent calls, falsely reporting compliant on massive supervisory violations.";
...
export function verifyDefectRemediation(
  ...
  const ctx = context ?? {
    ...
    gateCommand: "bun test tests/validation/defect-cli-1788679500002-auditorblind.test.ts",
    isUiTask: false,
