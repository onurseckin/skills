...
import { AdaptiveTimerController, type IntervalAdjustmentResult } from "./adaptive-timer.ts";
...
export class AutonomicWatchdog {
  ...
  public constructor(config: AutonomicWatchdogConfig = {}) {
    ...
    this.activityTracker = new ActivityTracker(this.bootGateEnforcer);
    ...
    this.adaptiveController = new AdaptiveTimerController(config, this.startedAtMs);
  ...
  private handleIntervalAdjustment(result: IntervalAdjustmentResult): number {
    if (result.changed) {
      this.emitter.emit({
        ...
        previousIntervalMs: result.previousIntervalMs,
        newIntervalMs: result.newIntervalMs,
  ...
  public async tick(currentTime?: string | number | Date): Promise<WatchdogTickReport> {
    ...
    const report: WatchdogTickReport = {
      ...
      timestamp,
      elapsedMs,
      intervalMs: this.adaptiveController.currentIntervalMs,
      health,
    };
  ...
  public async renderCliStatusReport(currentTime?: string | number | Date): Promise<string> {
    ...
    return formatCliStatusReport(health, bootGateTable);
  }
