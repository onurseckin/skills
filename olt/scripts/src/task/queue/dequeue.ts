...
export function assertSingleActiveLease(tasks: readonly TaskQueueItem[], agentId: string, nowMs = Date.now()): void {
  for (const task of tasks) {
    if ((task.status === "IN_PROGRESS" || task.status === "RUNNING" || task.status === "VALIDATING") && task.lease && task.lease.agent_id === agentId) {
...
export function admitTaskUnlocked(
  params: { readonly taskId: string; readonly admittedBy?: string | undefined; readonly nowIso?: string | undefined },
  filePath: string,
): TaskQueueItem {
...
export function admitTask(params: { readonly taskId: string; readonly admittedBy?: string | undefined; readonly customPath?: string | undefined; readonly nowIso?: string | undefined }): TaskQueueItem {
  const p = resolveTaskQueuePath(params.customPath); return withTaskQueueTransaction(p, () => admitTaskUnlocked(params, p));
}

export function claimTaskLeaseUnlocked(
  params: { readonly taskId: string; readonly agentId: string; readonly durationSeconds?: number | undefined; readonly nowIso?: string | undefined },
  filePath: string,
  ...
  const leasedTask: TaskQueueItem = {
    ...task,
    status: "IN_PROGRESS",
    lease: { agent_id: params.agentId, leased_at: nowIso, expires_at: expiresAt, attempt, lease_duration_seconds: durationSec, token },
...
export function claimTaskLease(params: { readonly taskId: string; readonly agentId: string; readonly durationSeconds?: number | undefined; readonly customPath?: string | undefined; readonly nowIso?: string | undefined }): { readonly task: TaskQueueItem; readonly leaseToken: string } {
  const p = resolveTaskQueuePath(params.customPath); return withTaskQueueTransaction(p, () => claimTaskLeaseUnlocked(params, p));
}

export function popNextEligibleTaskUnlocked(
  params: { readonly agentId: string; readonly durationSeconds?: number | undefined; readonly nowIso?: string | undefined },
  filePath: string,
...
export function popNextEligibleTask(params: { readonly agentId: string; readonly durationSeconds?: number | undefined; readonly customPath?: string | undefined; readonly nowIso?: string | undefined }): { readonly task: TaskQueueItem; readonly leaseToken: string } | null {
  const p = resolveTaskQueuePath(params.customPath); return withTaskQueueTransaction(p, () => popNextEligibleTaskUnlocked(params, p));
}

export function popNextEligibleTaskWithCleanup(params: { readonly agentId: string; readonly durationSeconds?: number | undefined; readonly customPath?: string | undefined; readonly completedTasksPath?: string | undefined; readonly nowIso?: string | undefined }): { readonly task: TaskQueueItem; readonly leaseToken: string; readonly prunedCount: number } | null {
...
export function dequeueTask(agentId: string, durationSeconds = DEFAULT_LEASE_DURATION_SECONDS, options?: { readonly customPath?: string | undefined; readonly nowIso?: string | undefined }): TaskQueueItem | null {
...
export function renewTaskLeaseUnlocked(
  params: { readonly taskId: string; readonly agentId: string; readonly leaseToken: string; readonly extensionSeconds?: number | undefined; readonly nowIso?: string | undefined },
  filePath: string,
): TaskQueueItem {
  const queue = readTaskQueueFile(filePath);
  const index = queue.findIndex((t) => t.id === params.taskId);
...
export function renewTaskLease(params: { readonly taskId: string; readonly agentId: string; readonly leaseToken: string; readonly extensionSeconds?: number | undefined; readonly customPath?: string | undefined; readonly nowIso?: string | undefined }): TaskQueueItem {
  const p = resolveTaskQueuePath(params.customPath); return withTaskQueueTransaction(p, () => renewTaskLeaseUnlocked(params, p));
}

export function releaseTaskLeaseUnlocked(
  params: { readonly taskId: string; readonly agentId: string; readonly leaseToken?: string | undefined; readonly nowIso?: string | undefined },
  filePath: string,
): TaskQueueItem {
  const queue = readTaskQueueFile(filePath);
  const index = queue.findIndex((t) => t.id === params.taskId);
  ...
  const nowIso = params.nowIso ?? new Date().toISOString();
  const releasedTask: TaskQueueItem = { ...task, status: "PENDING", lease: null, updated_at: nowIso };
  queue[index] = releasedTask;
  writeTaskQueueUnlocked(queue, filePath);
  return releasedTask;
}

export function releaseTaskLease(params: { readonly taskId: string; readonly agentId: string; readonly leaseToken?: string | undefined; readonly customPath?: string | undefined; readonly nowIso?: string | undefined }): TaskQueueItem {
  const p = resolveTaskQueuePath(params.customPath); return withTaskQueueTransaction(p, () => releaseTaskLeaseUnlocked(params, p));
}

export function startTaskValidationUnlocked(
  params: { readonly taskId: string; readonly agentId?: string | undefined; readonly leaseToken?: string | undefined; readonly nowIso?: string | undefined },
  filePath: string,
): TaskQueueItem {
  const queue = readTaskQueueFile(filePath);
  const index = queue.findIndex((t) => t.id === params.taskId);
...
export function startTaskValidation(params: { readonly taskId: string; readonly agentId?: string | undefined; readonly leaseToken?: string | undefined; readonly customPath?: string | undefined; readonly nowIso?: string | undefined }): TaskQueueItem {
  const p = resolveTaskQueuePath(params.customPath); return withTaskQueueTransaction(p, () => startTaskValidationUnlocked(params, p));
}
