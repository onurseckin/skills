...
export function deserializeTaskQueueItem(raw: Record<string, unknown>): TaskQueueItem {
  const reqStr = (k: string): string => {
    ...
    if (typeof v !== "string" || v.trim().length === 0)
      throw new HarnessError("INTEGRITY", `invalid ${k}`);
    return v;
  };
  ...
  if (raw["source_type"] !== undefined && raw["source_type"] !== sourceType)
    throw new HarnessError("INTEGRITY", "invalid source_type");
  ...
  if (raw["lease"] !== null && raw["lease"] !== undefined) {
    ...
    const { agent_id, token, leased_at, expires_at } = r,
      attempt = r.attempt ?? 1,
      duration = r.lease_duration_seconds ?? 1800;
    if (
      typeof agent_id !== "string" ||
      !agent_id.trim() ||
      typeof token !== "string" ||
      !token.trim() ||
      typeof leased_at !== "string" ||
      !Number.isFinite(Date.parse(leased_at)) ||
  ...
  if (!Number.isFinite(Date.parse(createdAt)) || !Number.isFinite(Date.parse(updatedAt)))
    throw new HarnessError("INTEGRITY", "invalid timestamps");
  return {
    id,
    priority,
    status,
    ...
    write_scope: strArr("write_scope"),
