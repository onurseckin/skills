...
export const TASK_QUEUE_STATUSES: readonly TaskQueueStatus[] = [
  "PENDING", "ADMITTED", "IN_PROGRESS", "RUNNING", "VALIDATING", "COMPLETED", "FAILED", "BLOCKED", "ESCALATED",
];
...
export type TaskQueuePersistenceStage =
  | "before_write" | "before_fsync" | "before_rename" | "after_rename" | "before_directory_fsync";
...
export function validateSourceType(val: unknown): TaskSourceType {
  const valid: TaskSourceType[] = [
    "external_intake", "feedback_intake", "self_evolution",
    "defect_remediation", "direct_prompt", "plan_enhancement",
  ];
  return typeof val === "string" && (valid as string[]).includes(val) ? (val as TaskSourceType) : "self_evolution";
}
...
export function deserializeTaskQueueItem(raw: Record<string, unknown>): TaskQueueItem {
  ...
  const id = reqStr("id"), status = reqStr("status") as TaskQueueStatus, priority = (raw["priority"] ?? "MEDIUM") as TaskPriority;
  ...
  const retryCount = raw["retry_count"] ?? 0, maxRetries = raw["max_retries"] ?? 3;
  ...
  if (raw["lease"] !== null && raw["lease"] !== undefined) {
    ...
    const { agent_id, token, leased_at, expires_at } = r, attempt = r.attempt ?? 1, duration = r.lease_duration_seconds ?? 1800;
  ...
  return {
    id, priority, status, lease, source_type: sourceType, created_at: createdAt, updated_at: updatedAt,
    retry_count: retryCount, max_retries: maxRetries, write_scope: strArr("write_scope"),
