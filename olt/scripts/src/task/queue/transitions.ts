...
import { recordCompletedTask, type CompletedTaskRecord } from "../../mind/archival/completed/index.ts";
...
export function assertValidActiveLease(task: TaskQueueItem, expectedToken?: string): void {
  ...
  if (expectedToken && task.lease.token !== expectedToken) throw new HarnessError("INVALID_STATE", `Lease token mismatch for task '${task.id}'`);
  const expMs = Date.parse(task.lease.expires_at);
  if (Number.isFinite(expMs) && expMs <= Date.now()) throw new HarnessError("INVALID_STATE", `Lease expired for task '${task.id}'`);
}
...
export function assertWriteScopeASTPurity(repoRoot: string, writeScope: readonly string[]): void {
  for (const relPath of writeScope) {
    ...
    if (existsSync(fullPath) && (fullPath.endsWith(".ts") || fullPath.endsWith(".tsx") || fullPath.endsWith(".js"))) {
      ...
      if (content.includes("/*") || content.includes("//")) throw new HarnessError("INTEGRITY", `AST purity invariant violated in ${relPath}`);
    }
  }
}
  ...
  const updated = tasks.map((task) => {
    if (task.lease && (task.status === "IN_PROGRESS" || task.status === "RUNNING" || task.status === "VALIDATING")) {
      ...
      if (Number.isFinite(expMs)) {
        count++;
        return { ...task, lease: { ...task.lease, expires_at: new Date(expMs + frozenDurationMs).toISOString() } };
      }
    }
    return task;
  });
...
export function escalateTaskUnlocked(
  params: {
    readonly taskId: string; readonly reason: string; readonly escalationTier?: string | undefined;
    readonly agentId?: string | undefined; readonly leaseToken?: string | undefined; readonly nowIso?: string | undefined;
  },
  filePath: string,
  ...
  const escalatedTask: TaskQueueItem = {
    ...task, status: "ESCALATED", lease: null, escalated_at: nowIso, error_message: params.reason,
    assigned_tier: params.escalationTier ?? task.assigned_tier ?? "Tier_0_Mind", updated_at: nowIso,
    metadata: { ...(task.metadata ?? {}), escalated_by: params.agentId ?? "system", escalation_reason: params.reason, escalation_timestamp: nowIso },
  };
  ...
  for (let i = 0; i < queue.length; i++) {
    const item = queue[i]!;
    if (item.id === escalatedTask.id || item.status === "COMPLETED" || item.status === "FAILED" || item.status === "ESCALATED") continue;
    if (item.dependencies.includes(escalatedTask.id)) {
      const blockedBy = item.blocked_by.includes(escalatedTask.id) ? item.blocked_by : [...item.blocked_by, escalatedTask.id];
...
export function escalateTask(params: {
  readonly taskId: string; readonly reason: string; readonly escalationTier?: string | undefined;
  readonly agentId?: string | undefined; readonly leaseToken?: string | undefined; readonly customPath?: string | undefined; readonly nowIso?: string | undefined;
}): { readonly task: TaskQueueItem; readonly affectedDependents: readonly string[] } {
  const p = resolveTaskQueuePath(params.customPath); return withTaskQueueTransaction(p, () => escalateTaskUnlocked(params, p));
}

export function failTaskUnlocked(
  params: {
    readonly taskId: string; readonly errorMessage: string; readonly agentId?: string | undefined;
    readonly leaseToken?: string | undefined; readonly canRetry?: boolean | undefined; readonly escalateOnMaxRetries?: boolean | undefined; readonly nowIso?: string | undefined;
  },
  filePath: string,
  ...
  if (canRetry) {
    const retriedTask: TaskQueueItem = {
      ...task, status: task.blocked_by.length > 0 ? "BLOCKED" : "PENDING", retry_count: task.retry_count + 1, lease: null, error_message: params.errorMessage, updated_at: nowIso,
    };
  ...
  for (let i = 0; i < queue.length; i++) {
    const item = queue[i]!;
    if (item.id === failedTask.id || item.status === "COMPLETED" || item.status === "FAILED" || item.status === "ESCALATED") continue;
    if (item.dependencies.includes(failedTask.id)) {
      const blockedBy = item.blocked_by.includes(failedTask.id) ? item.blocked_by : [...item.blocked_by, failedTask.id];
      queue[i] = { ...item, status: "BLOCKED", blocked_by: blockedBy, updated_at: nowIso };
      affectedDependents.push(item.id);
    }
  }
...
export function failTask(params: {
  readonly taskId: string; readonly errorMessage: string; readonly agentId?: string | undefined;
  readonly leaseToken?: string | undefined; readonly canRetry?: boolean | undefined; readonly escalateOnMaxRetries?: boolean | undefined;
  readonly customPath?: string | undefined; readonly nowIso?: string | undefined;
}): { readonly task: TaskQueueItem; readonly retried: boolean; readonly affectedDependents: readonly string[]; readonly escalated?: boolean | undefined } {
  const p = resolveTaskQueuePath(params.customPath); return withTaskQueueTransaction(p, () => failTaskUnlocked(params, p));
}

export function completeTaskUnlocked(
  params: {
    readonly taskId: string; readonly agentId?: string | undefined; readonly leaseToken?: string | undefined;
    readonly nowIso?: string | undefined; readonly proofSummary?: string | undefined; readonly testPath?: string | undefined;
    readonly assertions?: number | string | readonly string[] | null | undefined; readonly runtimeMs?: number | string | null | undefined;
    readonly commitSha?: string | null | undefined; readonly autoArchive?: boolean | undefined;
    readonly completedTasksPath?: string | undefined; readonly autoPrune?: boolean | undefined; readonly receipts?: CompletionReceipts | undefined;
  },
  filePath: string,
  ...
  if (params.autoArchive || params.proofSummary) {
    const proofSummary = params.proofSummary ?? completedTask.description ?? `Completed task ${completedTask.id}`;
    try {
      archivedRecord = recordCompletedTask(
        {
          id: completedTask.id, source: "task_queue", title: completedTask.title, status: "COMPLETED", proof_summary: proofSummary, completed_at: nowIso,
          category: (completedTask.metadata?.["category"] as string | undefined) ?? "CORE_ENGINE",
          test_path: params.testPath ?? (completedTask.metadata?.["test_path"] as string | undefined),
          assertions: params.assertions ?? (completedTask.metadata?.["assertions"] as number | string | readonly string[] | null | undefined),
          runtime_ms: params.runtimeMs ?? (completedTask.metadata?.["runtime_ms"] as number | string | null | undefined),
          commit_sha: params.commitSha ?? (completedTask.metadata?.["commit_sha"] as string | undefined),
...
export function completeTask(params: {
  readonly taskId: string; readonly agentId?: string | undefined; readonly leaseToken?: string | undefined;
  readonly customPath?: string | undefined; readonly nowIso?: string | undefined; readonly proofSummary?: string | undefined;
  readonly testPath?: string | undefined; readonly assertions?: number | string | readonly string[] | null | undefined;
  readonly runtimeMs?: number | string | null | undefined; readonly commitSha?: string | null | undefined;
  readonly autoArchive?: boolean | undefined; readonly completedTasksPath?: string | undefined; readonly autoPrune?: boolean | undefined; readonly receipts?: CompletionReceipts | undefined;
}): { readonly completedTask: TaskQueueItem; readonly unblockedTasks: readonly TaskQueueItem[]; readonly archivedRecord?: CompletedTaskRecord | undefined } {
  const p = resolveTaskQueuePath(params.customPath); return withTaskQueueTransaction(p, () => completeTaskUnlocked(params, p));
}
