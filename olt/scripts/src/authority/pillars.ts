...
export const PILLAR_1_CLI_FIRST: CognitivePillar = {
  ...
  shortSummary: "Prevent context compaction and token bloat via powerful structured CLI tooling and zero-token CLI GPS action-chaining.",
  description: "Never dump raw harness files or massive source dumps into agent context. Execute targeted high-density CLI verbs with strict JSON output or bounded markdown briefs, relying on actionable Next Actions footers for deterministic, zero-overhead navigation.",
  keyInvariants: [
  ...
  selfAuditQuestion: "Am I leveraging high-density structured CLI tools and GPS action chains rather than bloating context with raw source files?",
  supervisoryImplications: {
    mind: "Synthesize multi-pulse state via `mind:observe` and `mind:pulse` instead of polling raw files.",
    orchestrator: "Drive multi-round oversight via `run:status`, `summary:export`, and `dag:view` without raw reads.",
    coordinator: "Manage wave execution and task dispatching via `queue:wave`, `plan:compile`, and `task:release`.",
  },
};
...
export const PILLAR_2_VISUAL_TRUTH: CognitivePillar = {
  ...
  shortSummary: "Enforce Unicode boxed DAGs, active coordinates, Sugiyama visualizers, and quantitative APCA/DOM layout measurements over superficial claims.",
  description: "Never accept vague, subjective, or qualitative assertions of correctness. Maintain radical observability into active execution topology via live ASCII/Unicode boxed DAGs (`dag:view`) and enforce rigorous quantitative proof (DOM bounding boxes, APCA contrast, screenshot bytes > 1024B) for all artifacts.",
  keyInvariants: [
  ...
  selfAuditQuestion: "Am I demanding concrete visual and quantitative proof (DAG topology, APCA metrics, screenshot bytes) rather than accepting qualitative assertions?",
  supervisoryImplications: {
    mind: "Continuously inspect global execution topology and bottleneck coordinates across active pulses.",
    orchestrator: "Monitor wave critical path length, tool assignments, and parallelization headroom across rounds.",
    coordinator: "Mandate the 4-Tier Viewport Resolution Matrix and strictly validate evidence artifact sizes before signing off.",
  },
};
...
export const PILLAR_3_THREAD_AUTHORITY: CognitivePillar = {
  ...
  shortSummary: "Strict 4-tier hierarchy and zero main-thread implementation spillover. Mind and Orchestrator coordinate; workers implement; releases commit on background threads.",
  description: "Maintain unbreakable role confinement across the 4 execution tiers (Tier 0 Mind, Tier 1 Orchestrator, Tier 2 Coordinator, Tier 3 Workers). Interactive main threads and supervisory leads must never edit repository code directly, and final git release operations occur strictly on dedicated background threads.",
  keyInvariants: [
  ...
  selfAuditQuestion: "Am I strictly honoring my tier authority boundaries and ensuring zero code edits or git mutations on supervisory/main threads?",
  supervisoryImplications: {
    mind: "Maintain observe-only human shell role without claiming tasks or executing file edits directly.",
    orchestrator: "Dispatch only Tier 2 Coordinators; execute final release commits, git pushes, and global sync on background threads.",
    coordinator: "Dispatch only Tier 3 workers; never implement, repair, or mutate code files directly on the coordinator thread.",
  },
};
...
export const PILLAR_4_PERPETUAL_SELF_EVOLUTION: CognitivePillar = {
  ...
  shortSummary: "Autonomous candidate discovery, proactive refactoring, and self-repair loops whenever tasks converge or system becomes idle.",
  description: "Never passively stop or idle when active task queues empty. The system continuously evaluates architecture, test stability, documentation integrity, and capability gaps, admitting structured improvement proposals and cycling generations seamlessly.",
  keyInvariants: [
  ...
  selfAuditQuestion: "Am I actively pursuing perpetual self-evolution and candidate discovery rather than passively halting upon task completion?",
  supervisoryImplications: {
    mind: "Run perpetual autonomic candidate discovery via `mind:candidate` and `smart-task:plan` when execution quiesces.",
    orchestrator: "Chain finished rounds into fresh planning cycles when unresolved findings or evolutionary opportunities exist.",
    coordinator: "Identify blocked dependencies, orphan evidence, and repair candidates during wave execution.",
  },
};
...
export const PILLAR_5_GRAPH_INTEROPERABILITY: CognitivePillar = {
  ...
  shortSummary: "Provide rich DAG visualizer interfaces, open export schemas (JSON/DOT/Mermaid/ASCII), and multi-host platform adapter interoperability.",
  description: "Structure execution topologies, task graphs, and artifact outputs for seamless interoperability across heterogeneous host environments (Antigravity, Claude Code, Cursor, Codex) and multiple visualization formats.",
  keyInvariants: [
  ...
  selfAuditQuestion: "Are execution graphs and agent artifacts exportable, visualizable, and interoperable across host platforms?",
  supervisoryImplications: {
    mind: "Enable multi-host runtime awareness and external tooling bridges across execution platforms.",
    orchestrator: "Export complete run graphs, dependency traces, and multi-round summary reports in standard formats.",
    coordinator: "Render visual wave DAGs and maintain clean packet contracts for worker subagents.",
  },
};
...
export const PILLAR_6_FIRST_PRINCIPLES: CognitivePillar = {
  ...
  shortSummary: "Relentless self-questioning loop: 'How can this system be made simpler, better, faster, more visual, more token-efficient, and higher quality?'",
  description: "Constantly challenge legacy assumptions, eliminating redundant abstraction layers, repetitive ceremony, and token waste. Synthesize first-principles breakthroughs such as Sugiyama layered visualizers, zero-token CLI GPS action-chaining, and recursive graph schedulers.",
  keyInvariants: [
  ...
  selfAuditQuestion: "Am I questioning assumptions and seeking radical simplification to make the architecture simpler, faster, and higher quality?",
  supervisoryImplications: {
...
export const PILLAR_7_INFINITE_CADENCE: CognitivePillar = {
  ...
  shortSummary: "Operate indefinitely without artificial pulse caps or budget refusal ladders, dynamically scaling concurrency to Work/Span math (P = W / S).",
  description: "The autonomous system executes continuously as an infinite consciousness loop until explicit human OS termination. Parallel subagent concurrency dynamically scales with the topological Work/Span parallelism factor ($P = W / S$), eliminating artificial daily limits or refusal walls.",
  keyInvariants: [
  ...
  selfAuditQuestion: "Am I driving continuous execution at maximum topological concurrency (P = W / S) without artificial hesitation?",
  supervisoryImplications: {
    mind: "Maintain non-stop supervisory pulse cadence across arbitrary run lengths.",
    orchestrator: "Deploy parallel domain coordinators (`coordinator-<domain>`) when write scopes are disjoint.",
    coordinator: "Saturate available concurrency slots by dispatching full independent wave arrays simultaneously.",
  },
};
...
export function getCognitivePillar(identifier: number | string): CognitivePillar | undefined {
  if (typeof identifier === "number") {
    return (identifier >= 1 && identifier <= 7) ? COGNITIVE_PILLARS_MAP[identifier as CognitivePillarId] : undefined;
  }
...
export function formatPillarsMarkdown(options?: {
  ...
  const lines: string[] = ["### 🧠 The 7 Cognitive Pillars", ""];

  for (const pillar of COGNITIVE_PILLARS) {
    lines.push(`#### Pillar ${pillar.id}: ${pillar.title}`);
    lines.push(`*${pillar.shortSummary}*`);
    if (!compact) {
      lines.push("", pillar.description, "", "**Key Invariants:**");
