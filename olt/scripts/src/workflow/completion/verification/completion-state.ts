...
import { completionHistoryIssues } from "../provenance/index.ts";
import { completionReviewIssues } from "../issues/index.ts";
import { sameRepositoryBinding } from "../provenance/index.ts";
import { taskTwoKeyValidatorPairingIssues } from "../critic/index.ts";
...
function validatorProofIssues(state: WorkflowState, task: TaskRecord): string[] {
  ...
  return applicableValidatorDomains(task.write_scope, reqTexts).flatMap((domain) => {
    ...
    return validation.checks.flatMap(({ command_id: id }) => {
      ...
      return valid || hasFreshPassingCommand
        ? []
...
export function completionIssues(
  ...
  for (const task of Object.values(state.tasks).sort((a, b) => a.id.localeCompare(b.id))) {
    ...
    if (task.status === "done") issues.push(...taskTwoKeyValidatorPairingIssues(task));
