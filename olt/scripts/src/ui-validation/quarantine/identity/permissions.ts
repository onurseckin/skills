...
export function simulatePermissionBoundary(
    ...
    for (const persona of personas) {
      ...
      if (persona.role === "guest") {
        if (routeRequiredPermissions.includes("public_read") && routeRequiredPermissions.length === 1) {
          expectedResult = "ALLOW";
        } else {
          expectedResult = "DENY_REDIRECT_LOGIN";
        }
      } else if (hasWildcard || (!isWildcardRequired && hasDirectPermission)) {
        if (persona.permissions.some((p) => p.endsWith("_read") && !p.endsWith("_write")) && targetRouteOrAction.includes("export")) {
    ...
    const securityScore = totalEvaluations > 0 ? Math.round((passedCount / totalEvaluations) * 100) : 100;
