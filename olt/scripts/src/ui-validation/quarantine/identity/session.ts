...
export function detectSessionDegradation(
  this: any,
    ...
    const {
      ...
      activePersona,
      token,
      loginPathPatterns = ["/login", "/auth", "/signin", "/session-expired", "?error=session_expired"],
    } = params;
    ...
    if (statusCode === 403) {
      return {
        ...
        reason: "HTTP 403 Forbidden encountered: persona lacks required permissions or session revoked.",
      };
    }
