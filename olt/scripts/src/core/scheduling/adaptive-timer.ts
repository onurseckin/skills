...
export class AdaptiveTimerController {
  ...
  public constructor(config: AdaptiveConfigInput = {}, initialStartedAt?: number | string | Date) {
    ...
    this.adaptiveEnabled = config.enabled !== undefined
      ? config.enabled
      : typeof config.adaptive === "boolean"
        ? config.adaptive
        : (nested?.enabled ?? true);
    const heartbeatMs = config.heartbeatIntervalMs ?? DEFAULT_WATCHDOG_HEARTBEAT_INTERVAL_MS;
    const minBound = nested?.minIntervalMs ?? config.minIntervalMs ?? DEFAULT_ADAPTIVE_MIN_INTERVAL_MS;
    const maxBound = nested?.maxIntervalMs ?? config.maxIntervalMs ?? Math.max(DEFAULT_ADAPTIVE_MAX_INTERVAL_MS, heartbeatMs);
    this.minIntervalMsState = Math.min(minBound, maxBound);
    this.maxIntervalMsState = Math.max(minBound, maxBound);
    this.backoffFactorState = nested?.backoffFactor ?? config.backoffFactor ?? DEFAULT_ADAPTIVE_BACKOFF_FACTOR;
    this.activityBoostState = nested?.activityBoost ?? config.activityBoost ?? DEFAULT_ADAPTIVE_ACTIVITY_BOOST;
    const initial = nested?.initialIntervalMs ?? config.initialIntervalMs ?? config.heartbeatIntervalMs ?? DEFAULT_WATCHDOG_HEARTBEAT_INTERVAL_MS;
    this.currentIntervalMsState = Math.min(Math.max(initial, this.minIntervalMsState), this.maxIntervalMsState);
  }

  public get currentIntervalMs(): number { return this.currentIntervalMsState; }
  public get minIntervalMs(): number { return this.minIntervalMsState; }
  public get maxIntervalMs(): number { return this.maxIntervalMsState; }
  public get backoffFactor(): number { return this.backoffFactorState; }
  public get activityBoost(): number { return this.activityBoostState; }
  public isAdaptive(): boolean { return this.adaptiveEnabled; }
  ...
  public configureAdaptiveTimers(config: Partial<AdaptiveTimerConfig>): void {
    ...
    if (config.maxIntervalMs !== undefined && Number.isFinite(config.maxIntervalMs) && config.maxIntervalMs > 0) {
    ...
    if (config.backoffFactor !== undefined && Number.isFinite(config.backoffFactor) && config.backoffFactor > 1) {
      this.backoffFactorState = config.backoffFactor;
    }
    if (config.activityBoost !== undefined && Number.isFinite(config.activityBoost) && config.activityBoost > 0) {
      this.activityBoostState = config.activityBoost;
    }
    const target = config.initialIntervalMs !== undefined && Number.isFinite(config.initialIntervalMs)
      ? config.initialIntervalMs
      : this.currentIntervalMsState;
    this.currentIntervalMsState = Math.min(Math.max(target, this.minIntervalMsState), this.maxIntervalMsState);
  }
  ...
  public boostActivity(
    ...
    const factor = multiplier !== undefined && Number.isFinite(multiplier) && multiplier > 0
      ? multiplier
      : this.activityBoostState;
    const target = Math.max(this.minIntervalMsState, Math.round(this.currentIntervalMsState * factor));
  ...
  public decayIdle(
    ...
    const factor = multiplier !== undefined && Number.isFinite(multiplier) && multiplier > 1
      ? multiplier
      : this.backoffFactorState;
    const target = Math.min(this.maxIntervalMsState, Math.round(this.currentIntervalMsState * factor));
    return this.applyAdjustment(target, reason, now);
  }
  ...
  public resetInterval(
    ...
    const target = Math.min(Math.max(intervalMs ?? defaultIntervalMs, this.minIntervalMsState), this.maxIntervalMsState);
    const previousIntervalMs = this.currentIntervalMsState;
    ...
    return { previousIntervalMs, newIntervalMs: target, changed, state: this.getAdaptiveState(), reason: "manual_reset" };
  }
}
