...
export function auditSupervisorZeroCodeEdits(
  ...
  for (const cmd of commandList) {
    const actor = typeof cmd.actor === "string" ? cmd.actor : typeof cmd.agent_id === "string" ? cmd.agent_id : undefined;
    const role = inferAgentRole(actor, typeof cmd.role === "string" ? cmd.role : undefined, roleMap);
    const commandLine = typeof cmd.command_line === "string" ? cmd.command_line : typeof cmd.command === "string" ? cmd.command : "";
    const tool = typeof cmd.tool === "string" ? cmd.tool : typeof cmd.tool_name === "string" ? cmd.tool_name : undefined;

    if (isSupervisorRole(role)) {
      if (tool && CODE_EDIT_TOOLS.has(tool)) {
        violations.push({ agentId: actor ?? "unknown", role, toolOrCommand: tool });
      }
      if (commandLine && (commandLine.includes("write_to_file") || commandLine.includes("replace_file_content"))) {
  ...
  if (Array.isArray(ctx.events)) {
    for (const event of ctx.events) {
      if (event && typeof event === "object") {
        ...
        const role = inferAgentRole(agentId, typeof payload.role === "string" ? payload.role : undefined, roleMap);
        const toolName =
  ...
  return [
    {
      ...
      compliant: true,
      severity: "INFO",
      message: "Supervisor Zero Code Edits invariant satisfied: no supervisory role modified code files.",
    },
  ];
}
  ...
  if (Array.isArray(ctx.events)) {
    for (const event of ctx.events) {
      if (event && typeof event === "object") {
        ...
        const agentId = typeof payload.agent_id === "string" ? payload.agent_id : actor;
        const role = inferAgentRole(agentId, typeof payload.role === "string" ? payload.role : undefined, roleMap);
        const commandLine = typeof payload.command_line === "string" ? payload.command_line : typeof payload.command === "string" ? payload.command : "";
  ...
  return [
    {
      ...
      compliant: true,
      severity: "INFO",
      message: "Supervisor Zero Test Runs invariant satisfied: no supervisory role executed test runner commands directly.",
    },
  ];
}
 ...
 */
export function auditThreeStrikeMechanicalContainment(ctx: InvariantContext): InvariantAuditResult[] {
  const strikesState = (ctx.state?.strikes ?? (ctx.state?.mind as Record<string, unknown> | undefined)?.strikes) as
  ...
  if (strikesState && typeof strikesState === "object") {
    for (const [agentId, data] of Object.entries(strikesState)) {
      if (data && typeof data === "object") {
        const count = typeof (data as Record<string, unknown>).count === "number" ? ((data as Record<string, unknown>).count as number) : 0;
        const status = typeof (data as Record<string, unknown>).status === "string" ? ((data as Record<string, unknown>).status as string) : "active";
  ...
  return [
    {
      ...
      compliant: true,
      severity: "INFO",
      message: "Three-Strike Mechanical Containment invariant satisfied: all strike quotas and escalations properly bounded.",
    },
  ];
}
...
export function auditAntiMakeworkGenuineValue(ctx: InvariantContext): InvariantAuditResult[] {
  ...
  const consecutiveZeroDelta = typeof pulse.consecutive_zero_delta === "number" ? pulse.consecutive_zero_delta : 0;
