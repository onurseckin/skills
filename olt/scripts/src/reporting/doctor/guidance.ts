...
export function generateRemedialGuidance(options: GuidanceGenerationOptions): DoctorGuidanceResult {
  const actions: DoctorRemedialAction[] = [];
  ...
  for (const f of findings) {
    ...
    if (
      ...
      const act: DoctorRemedialAction = {
        ...
        description:
          "Relocate loose files or static package runtime files to scratch/ or .olt/ to comply with Invariant 30.",
        rationale:
          "Invariant 30 strictly enforces zero loose root files and static package purity.",
      };
      actions.push(act);
      ...
      const act: DoctorRemedialAction = {
        ...
        description:
          "Task requires 5 cognitive pushbacks and 5 adversarial probes before completion.",
        rationale:
          "Pushback quotas ensure comprehensive adversarial validation before task closure.",
      };
      actions.push(act);
      guidance.push(`[${f.code}] ${act.description} -> Run: \`${act.command}\``);
    } else if (
      f.code === "EXPLICIT_ANY" ||
      f.code === "ANY_TYPE_ASSERTION" ||
      f.code === "COMPILER_SUPPRESSION_DIRECTIVE"
    ) {
      seenCodes.add(f.code);
      const act: DoctorRemedialAction = {
        issueCode: f.code,
        ...
        description:
          "Remove explicit 'any' type assertions and compiler suppressions (@ts-ignore/@ts-expect-error) from code.",
        rationale:
          "AST purity mandates zero 'any' types and zero compiler suppressions across harness code.",
      };
      actions.push(act);
      ...
      const act: DoctorRemedialAction = {
        ...
        description:
          "Clean up orphaned worktree directories, merged track branches, and dead-PID worktree locks.",
        rationale:
          "Worktree health ensures isolated git working trees do not leak stale locks or dead branches.",
      };
      actions.push(act);
      guidance.push(`[${f.code}] ${act.description} -> Run: \`${act.command}\``);
      ...
      const act: DoctorRemedialAction = {
        ...
        command: `bun harness.ts doctor --fix`,
        description:
          "Rebuild mailbox cursor from valid inbox envelopes and prune orphaned mailboxes.",
      ...
      const act: DoctorRemedialAction = {
        ...
        command: `git add -A`,
        description:
          "Stage all modified artifacts immediately to persist loose Git objects for reflog safety.",
        rationale:
          "Sub-domain completion git staging invariant guarantees recoverability across crashes.",
      };
      actions.push(act);
      guidance.push(`[${f.code}] ${act.description} -> Run: \`${act.command}\``);
    }
  ...
  if (orphanEvidence.length > 0 && !seenCodes.has("ORPHAN_EVIDENCE")) {
    ...
    const act: DoctorRemedialAction = {
      ...
      description:
        "Record dispositions for undispositioned orphan evidence before sealing run completion.",
      rationale:
        "Every piece of orphan evidence must be explicitly dispositioned prior to run:complete.",
    };
    actions.push(act);
  ...
  if (missingRunGates.length > 0 && !seenCodes.has("MISSING_RUN_GATES")) {
    ...
    const act: DoctorRemedialAction = {
      ...
      description:
        "Execute all run-scoped gate verifications before triggering terminal completion.",
  ...
  if (
    options.criticStatus === null ||
    options.criticStatus === "expired" ||
    options.criticStatus === "assigned"
  ) {
