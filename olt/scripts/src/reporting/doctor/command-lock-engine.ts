...
const isWholeSuite = (argv: readonly string[]): boolean => {
  ...
  if (f === "npm" || f === "pnpm" || f === "yarn") {
    if (s === "test" || s === "t" || (s === "run" && (argv[2] ?? "").toLowerCase().startsWith("test"))) {
  ...
  if (Array.isArray(options.events)) {
    for (const e of options.events) {
      if (e && typeof e === "object") {
        ...
        if (eventName === "command-executed" || eventName === "command-recorded" || eventName === "test-executed" || eventName === "command") {
