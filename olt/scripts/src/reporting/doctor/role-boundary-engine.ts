...
const SUPERVISOR_ROLES = new Set(["mind", "orchestrator", "coordinator", "supervisor", "lead", "architect"]);
 ...
 */
export function checkRoleBoundaryInterlock(options: RoleBoundaryInterlockOptions = {}): DoctorCheckEngineResult {
  ...
  if (Array.isArray(rawGrants)) {
    for (const grant of rawGrants) {
      if (grant && typeof grant === "object") {
        ...
        const toolsUsed = Array.isArray(g.tools_used) ? g.tools_used.filter((t): t is string => typeof t === "string") : [];
        if (id && role) {
  ...
  if (Array.isArray(options.events)) {
    for (const event of options.events) {
      if (event && typeof event === "object") {
        ...
        const role = resolveRole(agentId, typeof payload.role === "string" ? payload.role : undefined);
        const toolName = typeof payload.tool === "string" ? payload.tool : typeof payload.tool_name === "string" ? payload.tool_name : undefined;
        ...
        if (eventName === "task-satisfied" || eventName === "task-approved" || eventName === "gate-approved") {
          const taskImplementer = typeof payload.implementer_id === "string"
  ...
  if (rawTasks && typeof rawTasks === "object") {
    for (const [key, val] of Object.entries(rawTasks)) {
      if (val && typeof val === "object") {
        ...
        const implementer = typeof task.assigned_agent === "string" ? task.assigned_agent : undefined;
        const validator = typeof task.validator_agent === "string" ? task.validator_agent : typeof task.validator_id === "string" ? task.validator_id : undefined;
