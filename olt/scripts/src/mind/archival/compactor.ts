...
function openStableArchivedObjectivesDirectoryChain(
  ...
  try {
    ...
    throw new HarnessError(
      "PATH_SAFETY",
      `${label} cannot be securely traversed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
function atomicWriteArchivedObjectives(
  ...
  try {
    ...
    throw new HarnessError(
      "INTEGRITY",
      `archived objectives ledger mutation failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  } finally {
