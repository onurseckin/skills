import type { DefectDeliberationRound } from "./deliberation.ts";
...
export function formatDeliberationReport(
  ...
  const lines: string[] = [
    ...
    ...round.hypotheses.map(
      (h) => `- [${h.defect_id}] (${h.category}) confidence ${h.confidence.toFixed(2)}: ${h.root_cause}`,
    ),
    "",
    "#### Remediation Actions",
    ...round.actions.map(
      (a) => `- [${a.action_id}] for ${a.defect_id} (${a.action_type}): ${a.description} -> gate: \`${a.prescribed_test}\``,
    ),
    "",
