...
export function diagnoseMacroDag(
  ...
  const totalWorkMs = nodes.reduce(
    (sum: number, n: MacroDagTaskNode) => sum + (n.durationEstimateMs ?? defaultDuration),
    0,
  );
  ...
  for (const node of nodes) {
    ...
    if (node.status === "failed") {
      bottlenecks.push({
        ...
        suggestedMitigation:
          "Dispatch an implementer repair attempt or prune unrecoverable branch.",
      });
    }
  }
