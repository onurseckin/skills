...
export function resolveLastPulsePath(capsuleRoot: string): string {
  return join(capsuleRoot, "last_pulse.json");
}

export type LastPulsePayload = LastPulseRecord;

export function pulseProducedActivity(record: LastPulseRecord | null | undefined): boolean {
  return Boolean(
    record && record.pulse_id && record.outcome && record.outcome !== "idle_quiescent",
  );
}
