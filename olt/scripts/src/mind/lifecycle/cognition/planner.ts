...
import { extractTasksFromState } from "./gap-analysis.ts";
import { PROACTIVE_QUESTION_CATALOG } from "./types.ts";
import type {
  DiscoveredSubtask,
...
import type { QuestionCycleInput, MindPulseContext } from "./state.ts";
...
export function harvestPlanEnhancementsDuringPulse(
  ...
  for (const task of tasks) {
    ...
    if (task.write_scope.length > 2) {
      ...
      task.write_scope.forEach((scopePath: string, index: number) => {
