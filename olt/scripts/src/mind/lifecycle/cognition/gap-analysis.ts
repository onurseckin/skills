...
import { MIN_COGNITIVE_SCORE, MAX_COGNITIVE_SCORE } from "./types.ts";
import type { CognitiveAuditFinding, CognitiveAuditResult, CognitiveScoreVector } from "./types.ts";
import type { SystemStateMetrics, DimensionalWeights } from "./state.ts";
import { DEFAULT_DIMENSIONAL_WEIGHTS } from "./state.ts";
...
export interface ExtractedTask {
  ...
  readonly gate_command?: string | undefined;
}

export function extractTasksFromState(state: unknown): ExtractedTask[] {
  if (!state || typeof state !== "object") return [];
  const s = state as Record<string, unknown>;
  const rawTasks: Record<string, unknown>[] = [];
  if (Array.isArray(s.tasks)) {
    for (const t of s.tasks) {
      if (typeof t === "object" && t !== null) rawTasks.push(t as Record<string, unknown>);
    }
  } else if (s.tasks && typeof s.tasks === "object") {
    for (const t of Object.values(s.tasks)) {
      if (typeof t === "object" && t !== null) rawTasks.push(t as Record<string, unknown>);
    }
  }
  return rawTasks.map((t, idx) => ({
    id: typeof t.id === "string" && t.id.length > 0 ? t.id : `task-${idx}`,
    status: typeof t.status === "string" ? t.status : "unknown",
    dependencies: Array.isArray(t.dependencies)
      ? t.dependencies.filter((d): d is string => typeof d === "string")
      : [],
    write_scope: Array.isArray(t.write_scope)
      ? t.write_scope.filter((w): w is string => typeof w === "string")
      : [],
    gate_command: typeof t.gate_command === "string" ? t.gate_command : undefined,
  }));
}
