...
export function layoutSugiyamaDag(
  ...
  if (nodes.length === 0) {
    return { totalNodes: 0, totalLayers: 0, totalCrossings: 0, criticalPathSpan: 0, asciiDiagram: "(Empty DAG)", layers: [] };
  }
  ...
  const totalWidth = layerCount * (boxWidth + hSpacing);
  const totalHeight = maxNodesInLayer * (boxHeight + vSpacing) + 2;
  const canvas = new AsciiCanvasMatrix(totalWidth, totalHeight);
  ...
  const criticalPathSpan = nodes.reduce((acc, node) => acc + (node.badges?.spanMinutes ?? 1), 0);
  return {
