...
To provide visual clarity in terminal interfaces without resorting to external web viewers or heavy GUI dependencies, `olt` incorporates an in-engine **Sugiyama 4-Phase Hierarchical DAG Renderer** (`dag`, aliased as `graph:sugiyama`).
...
Running `dag` generates an interactive, high-fidelity ASCII/Unicode topology map directly in the terminal:

```bash
bun harness.ts dag --run .olt/capsules/<slug> --box-style rounded
```
...
```bash
bun harness.ts plan:add --run .olt/capsules/<slug> --actor planner --id task-curriculum \
...
```bash
bun harness.ts plan:add --run .olt/capsules/<slug> --actor coordinator --id task-api \
