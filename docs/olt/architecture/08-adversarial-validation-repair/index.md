...
2. **Dual-Channel Interlock**: Certification requires simultaneous agreement between the Cognitive Channel (pure AST static logic auditing) and the Mechanical Channel (deterministic test runner exit status via `task:check` / `ui-headless-validator`).
3. **Cognitive Validator Command Hard-Lock (0 Commands)**: Cognitive validators are mechanically stripped of all shell command execution privileges ($\text{Commands}(\text{Validator}) \equiv \emptyset$), preventing test tampering and exit code forgery.
4. **UI Validation Decoupling (Headless vs Optical Split)**: UI surfaces undergo sequential dual validation: `ui-headless-validator` (Playwright automation, DOM assertions, screenshot capture) followed by `ui-optical-validator` (4-viewport headful visual review across the 8 Optical Dimensions with 0 shell commands).
5. **Forensic AST Heuristics**: The Meta-Auditor evaluates diffs across seven automated heuristics ($\mathcal{H}_{1 \dots 7}$), trapping stubs, mock pollution, exception swallowing, and type suppressions.
6. **Monotonic Repair Loops**: Rejections emit machine-readable JSON structured findings. Repair cycles must strictly shrink the active defect set ($\mathcal{D}_{k+1} \subset \mathcal{D}_k$) within a bounded 5-round envelope ($k \le 5$).
...
│   │ Philosophy & Dual-Channel Gates    │ ═══════════►│ Command Hard-Lock & UI Split       │      │
│   └─────────────────┬──────────────────┘             └─────────────────┬──────────────────┘      │
│                     │                                                  │                         │
│                     ▼                                                  ▼                         │
│   ┌────────────────────────────────────┐             ┌────────────────────────────────────┐      │
...
### Pillar 2: Cognitive Validator Command Hard-Lock & UI Split

Enforces mechanical RBAC interlocks that strip cognitive validators of all shell execution tools ($\text{Commands}(\text{Validator}) \equiv \emptyset$). By decoupling cognitive reasoning from test execution, validators perform deep AST structural audits while mechanical execution is delegated to `task:check`. UI validation is split cleanly between `ui-headless-validator` (Playwright test harness) and `ui-optical-validator` (4-viewport visual inspection across the 8 Optical Dimensions).
...
Replaces vague conversational critique with machine-readable Draft 2020-12 JSON schemas. Enforces monotonic convergence where each repair iteration must strictly reduce the active defect set without introducing regressions ($\mathcal{D}_{k+1} \subset \mathcal{D}_k$), bounded by a maximum of 5 iterations.
...
│ 08-02 Cognitive Validator Command Hard-Lock      │ Security      │ 0 CLI commands & UI split     │
...
### [08-02: Cognitive Validator Command Hard-Lock & UI Validator Split](08-02-cognitive-validator-command-hard-lock.md)

Details the mechanical role interlock barring cognitive validators from executing terminal commands ($\text{Commands}(\text{Validator}) \equiv \emptyset$) to prevent test fabrication, detached evidence resolution, and the UI split between `ui-headless-validator` and `ui-optical-validator`.
...
graph TD
    subgraph Chapter08 ["Chapter 08: Adversarial Validation & Monotonic Repair"]
        A["08-01 Adversarial Philosophy"] --> B["08-02 Validator Command Hard-Lock & UI Split"]
...
3. **UI Dual Sequential Gate**: UI changes must pass both `ui-headless-validator` and `ui-optical-validator` (inspecting actual image artifacts across all 4 mandatory viewports).
4. **100% Heuristic AST Compliance**: Diffs must achieve $\mathcal{S}_{\text{forensic}}(\Delta) \equiv 1.000$ across all seven forensic heuristics.
5. **Strict Defect Monotonicity**: Repair cycles must guarantee $\mathcal{D}_{k+1} \subset \mathcal{D}_k$ and $|\mathcal{D}_{k+1}| < |\mathcal{D}_k|$ with zero regression introduction.
6. **Bounded Repair Envelope**: Repair iterations are bounded by $k \le 5$. Unresolved defects beyond round 5 trigger immediate Critic replanning.
7. **Immutable Ledger Persistence**: All Socratic probe records, findings, and receipts must be permanently stored in `.olt/capsules/<slug>/evidence/`.
