...
- **[`references/cli-capabilities.md`](../../../olt/references/cli-capabilities.md)** — every command, with its
  aliases, flags (type, required, repeatable, default), stdin rule, remainder rule, exit codes and
  runnable examples.
- **[`references/cli-capabilities.json`](../../../olt/references/cli-capabilities.json)** — the same content
...
- `--role` on `task:claim` is mandatory and names the capability contract in
  `olt/roles/<role>.md`; `--role plan-validator` on `agent:register` is the same
