...
In OLT, agents are forbidden from validating code by asserting subjective confidence (e.g., *"I have verified the code and it works"*). Validation is valid **if and only if** it is backed by an immutable shell execution receipt captured by the harness via `run:exec`.
