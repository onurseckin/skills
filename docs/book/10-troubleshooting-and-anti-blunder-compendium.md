...
_How OLT Prevents This_: Writes use atomic temporary file replacement (`fs.renameSync`) with synchronous disk flushes (`fs.fdatasyncSync`).
...
_How OLT Prevents This_: Locks store owner PID and millisecond heartbeat timestamps. If a lock holder process dies or fails to pulse for $> 30$ seconds, the lock is automatically reaped.
...
_How OLT Prevents This_: Task leases carry strict timeouts (default 20m). Upon lease expiry past the grace window, `queue:next` re-queues the task as `retry_ready`.
...
| **24** | `MC-2`        | **Repair Self-Approval**: Implementer skipping validation on a repair because "the critic already approved".                                                                                                                                                                              | Strict validation barrier: every repair task requires an independent paired validator.                                                                                                      |
