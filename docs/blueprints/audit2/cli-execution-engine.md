...
**Rule Definition:** _Agents interact with the harness exclusively through clean colon commands. Commands return concise, structured markdown briefs ($\le 30$ lines) designed for token efficiency and high signal._
...
- **Edge Case Breach (Pre-Execute Bubbling):** If an error occurs _before_ `execute()` is called (e.g., inside `stdinBytes()` size limits or prompt extraction logic), the error bubbles to the `import.meta.main` catch block in `harness.ts`, which bypasses the markdown envelope and forcibly prints `{"ok": false, "error": { ... }}` as JSON.
