...
- **Current Implementation**:
  ...
  - `task:reject` allows intra-task repair rounds on the *same* pre-existing task, incrementing `repair_rounds` up to 5.
- **Gaps Identified**:
  - **Scope Limitation to Existing Tasks**: Intra-task repair only works for defects discovered by a validator *during* that specific task's lifecycle. It cannot handle cross-cutting bugs discovered later during whole-run integration or critic audit.
