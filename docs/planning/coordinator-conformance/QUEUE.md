...
which makes a formatter run indistinguishable from a scope violation. **D1-D5 sit immediately after
the evidentiary floor and before #7.** They were audited 2026-08-21 against the owner's own report
that final verification reaches the main thread instead of a coordinator (`DELEGATION-AUDIT.md`), and
they rank above #7-#23 because they are the run's _delegation_ honesty — whether a dead agent is ever
noticed, whether a critic's findings can become work, whether the main thread has anyone to hand off
to — which the evidentiary floor (can a byte-hash be trusted) and everything below it (telemetry
reporters, artifact rules, per-validator metrics, ...) all presuppose is already sound. Everything
below that is ranked by how much of the run's honesty depends on it. #23 is last by construction.
...
| D1  | **The owner's reported failure: a clean termination is undetectable** — **NOT-STARTED, audited 2026-08-21**             | new (`DELEGATION-AUDIT.md` G4)    | Ranked first of D1-D5 because nothing else in this group matters while it stays absent. `dead-agent-detector.ts:13` types exactly one reason code, confirmed by reading it directly: `expired_lease_no_submission` — every detection path keys off a lapsed lease deadline (`recover-stale.ts:29,52,69`), never a clean exit. Worse, `reclaimDeadAgents` (`dead-agent-detector.ts:55-74`) builds its event list only from task leases and branch sub-leases: a validator or completeness critic that starts and never returns a verdict produces zero dead-agent events even after its own deadline lapses, reproduced live in the audit (`events: []`, `escalatedNow: []`, `changesRequested: []`, task silently reset to `submitted`). This is the exact mechanism behind the owner's report — a verifier's report reaches the main thread with no system-level signal that the verifier itself ever went missing. Full evidence and the cheapest first fix (name the vanished agent in `reclaimDeadAgents`) in DELEGATION-AUDIT.md G4 and the "D1-D5" section below. **Not fixed this wave** — this pass owns `QUEUE.md` only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| D2  | **Registration is optional, so every role rail is opt-out** — **NOT-STARTED, audited 2026-08-21**                       | new (`DELEGATION-AUDIT.md` G2/G7) | Confirmed by reading it directly: `assertGrantedCommand` (`command-authority.ts:65-75`) contains `if (!grant) return;` — the moment the acting agent is absent from the ledger, every role-command check is skipped, not refused. Reproduced live in the audit: an unregistered agent claimed a task while the ledger stayed `[]`. `contracts/agents.ts:19` confirmed to give grants exactly two states, `"active" \| "released"` — no state means "died" or "reclaimed", so a vanished agent's grant is indistinguishable from a live one or a clean release at completion time. `CHANNEL.md` calls registration non-negotiable; no code agrees. The audit's own cheapest fix: refuse a mutating command from an unregistered actor, which turns G2 and most of G7 from "enforced if the agent cooperates" into actually enforced, in one guard clause. Full evidence in DELEGATION-AUDIT.md G2/G7. **Not fixed this wave.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| D3  | **A completeness critic's findings are structurally incapable of becoming tasks** — **NOT-STARTED, audited 2026-08-21** | new (`DELEGATION-AUDIT.md` G3)    | Confirmed both refusals on the real `critic:reject` → `plan:replan` path by reading the code directly. First: `plan-replan-findings.ts:60` reads `record.revalidation_gate`, a field the critic pipeline never writes — `review-input.ts:54` writes `finding.revalidation` instead, a name mismatch, not a missing feature. Second: supplying `--gate` by hand still fails at `revision-guard.ts:134-138`, because a repair task inheriting a finished task's requirement would change that task's already-locked gate set — and `tests/unit/cli/critic-start-review.test.ts:245-247` (confirmed present) asserts this refusal as intended behaviour, so the block is currently specified, not accidental. Confirmed this pass: `roles/completeness-critic.md`'s `commands:` list (`critic:review`, `critic:reject`, `run:exec`, `run:status`, `summary:view`, `finding:get`, `report:get`, `evidence:get`, `evidence:screenshots`, `agent:report`) does not include `plan:replan` — the critic cannot invoke the one command that would turn its own finding into work. Its only outlet is the markdown at `run-formatter.ts:73-74`, "Yielding to Coordinator… Coordinator runs `plan:replan`" — prose in a return value that lands on whoever spawned the critic, which is the owner's bug verbatim when that spawner is the main thread (see D4). Full evidence in DELEGATION-AUDIT.md G3. **Not fixed this wave.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| D4  | **The main thread has no orchestrator to hand off to** — **NOT-STARTED, audited 2026-08-21**                            | new (`DELEGATION-AUDIT.md` G1)    | Confirmed: no `orchestrator` role exists — `contracts/packets.ts`'s role enumeration lists ten roles (`completeness-critic`, `coordinator`, `implementer`, `plan-validator`, `planner`, `repairer`, `sub-implementer`, `sub-investigator`, `sub-validator`, `validator`) and `orchestrator` is not one; `agent:register --role orchestrator` is refused at runtime per the audit. `agents/orchestrator.yaml:6-7` declares `role: "coordinator"` — the "meta-orchestrator" is a coordinator persona sitting at tier 1, not a distinct role tier 1 spawns. Concretely, the skill's own primary entry point instructs the main thread to do the work itself: `orchestrate-formatter.ts:22-30`'s printed next-steps read "nothing here is optional and nothing is done for you" and walk straight into `plan:enhance`/`plan:add`/`plan:compile`/`queue:wave` — there is no "spawn and register a coordinator first" step anywhere in `SKILL.md`, `references/run-playbook.md`, or the `orchestrate` handler. This is the structural reason the main thread absorbs a verifier's findings: it was never told anyone else exists to hand them to. Full evidence in DELEGATION-AUDIT.md G1. **Not fixed this wave.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| D5  | **`triggerAutoWake` reports success for a dispatch it never makes** — **NOT-STARTED, audited 2026-08-21**               | new (`DELEGATION-AUDIT.md` G5)    | Ranked last of D1-D5 because it only misfires after D1 already detected something — a corrective that lies about succeeding is bad, but strictly less bad than never detecting the death at all. Confirmed by reading it directly: `watchdog.ts:180`'s `triggerAutoWake` emits an `auto_wake` event and returns `{ succeeded: true, ... }` at line 232 without dispatching or injecting anything; its only production caller is its own `setInterval` inside `loop-runner`, behind the `orchestrator:run` command, which by its own registry description refuses without a host-injected round executor. `references/host-adapters.md:164`'s claim that the runtime "re-dispatches it without human intervention" is false by construction — the harness never dispatches agents (a hard rule this skill enforces elsewhere). Full evidence in DELEGATION-AUDIT.md G5. **Not fixed this wave.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
